...
class Activity_red_envelope extends UA_Controller
{	
	...
	public function zindex($page=1)
	{
		...
		$config['base_url'] = '/admin/a/activity_red_envelope/zindex/'.$urlstr.'&page=';
	...
	public function zzindex($page=1)
	{
		...
		$data = array();
		$this->load->library ( 'Page' ); // 加载分页类
		$config['base_url'] = '/admin/a/activity_red_envelope/zzindex/'.$urlstr.'&page=';
		$config ['pagesize'] = self::pagesize;
		$config ['page_now'] = $page;		
...
//		$query_sql1=" select wm.*,u.loginname,u.mobile,u.equipment_id equip_id from activity_red_envelope_rec as wm left join u_member as u on(wm.to_member_id=u.mid)  where ".$where_sql." ORDER BY  wm.addtime desc LIMIT {$from},{$page_size}";
                $query_sql1=" select wm.*,u.loginname,u.mobile,u.device_id equip_id from activity_red_envelope_rec as wm left join u_member as u on(wm.to_member_id=u.mid)  where ".$where_sql." ORDER BY  wm.addtime desc LIMIT {$from},{$page_size}";
		$t = $this ->db ->query($query_sql2) ->row_array();
		$total = $t['total'];				
		$wx_flow_activity_member_data = $this ->db ->query($query_sql1) ->result_array();		
		$config ['pagecount'] = $total;
		...
		$this->page->initialize ( $config );			
		$this->load_view ( 'admin/a/activity_red_envelope/index_list_1',$data);
	}	
	...
	public function index($page=1)
	{
		...
		$data = array();
		$this->load->library ( 'Page' ); // 加载分页类
		$config['base_url'] = '/admin/a/activity_red_envelope/index/'.$urlstr.'&page=';
		$config ['pagesize'] = self::pagesize;
		$config ['page_now'] = $page;		
