...
class Sc_activity_city_pic extends UA_Controller {
	public $controllerName = '深窗活动轮播图配置';
	public function __construct() {
		parent::__construct ();
		$this->load_model('admin/a/sc_activity_model/sc_activity_city_pic_model','activity_city_pic');
	}
	...
	function index(){
		...
		$this->load_view ( 'admin/a/cfg_activity/sc_activity_city_pic_view',$data);
	}

	public function getPicList() {
		$data=$this->activity_city_pic->getPicData();
	...
	public function add() {
		...
		$status = $this ->activity_city_pic ->insert($dataArr);
		if (empty($status)) {
			$this->callback->set_code ( 4000 ,"添加失败");
			$this->callback->exit_json();
		} else {
			$this ->log(1,3,'深窗活动轮播图配置','增加轮播图，名称：'.$title);
	...
	public function edit() {
		...
		$status = $this ->activity_city_pic ->update($dataArr ,array('id' =>intval($postArr['id'])));
		if (empty($status)) {
	...
	public function commonFunc($postArr ,$type)
	{
		...
		if(empty($postArr['link'])){
			$this->callback->set_code ( 4000 ,"请填写连链接名称");
			$this->callback->exit_json();
		}
		return array(
	...
	public function getOneData () {
		...
		$whereArr=array('acp.id='=>$id);
		$data=$this->activity_city_pic->getPicData($whereArr);
		if (empty($data)) {
			echo false;
			exit;
		} else {
	...
	function delete(){
		$id = intval($this->input->post("id"));
		$status = $this ->activity_city_pic ->update(array('status'=>0),array('id'=>$id));
		if (empty($status)) {
