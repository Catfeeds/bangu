...
class Sc_high_quality_line extends UA_Controller {
	public $controllerName = '深窗活动精品预售线路配置';
	public function __construct() {
		parent::__construct ();
		$this->load_model('admin/a/sc_activity_model/Sc_high_quality_line_model','high_quality_line');
	}
	...
	function index(){
		...
		$this->load_view ( 'admin/a/cfg_activity/sc_high_quality_line_view',$data);
	}

	public function getQuaList() {
		$data=$this->high_quality_line->getQuaData();
	...
	public function add() {
		...
		$status = $this ->high_quality_line ->insert($dataArr);
		if (empty($status)) {
			$this->callback->set_code ( 4000 ,"添加失败");
			$this->callback->exit_json();
		} else {
			$this ->log(1,3,'深窗活动精品预售线路配置','增加广告，名称：'.$title);
	...
	public function edit() {
		...
		$status = $this ->high_quality_line ->update($dataArr ,array('id' =>intval($postArr['id'])));
		if (empty($status)) {
	...
	public function getOneData () {
		...
		$data=$this->high_quality_line->getQuaData($whereArr);
		if (empty($data)) {
			echo false;
			exit;
		} else {
	...
	function delete(){
		$id = intval($this->input->post("id"));
		$status = $this ->high_quality_line ->update(array('status'=>0),array('id'=>$id));
		if (empty($status)) {
