...
class Line extends UA_Controller
{
	...
	public function index()
	{
		...
		$this->view ( 'admin/a/cfg_mobile/line_list' ,$data);
	}
	...
	public function getCommonData($type=1)
	{
		...
		if ($type == 1)
		{
			return $this->line_model->getALineData ( $whereArr ,$specialSql ,false ,$orderBy);
		}
		else 
		{
...
// 		$data['pageData1'] = $this->gift_manage->get_gift_list( array('linelistID'=>$lineId),$where,$this->getPage () );
	...
	public function vr_num()
	{
        ...
        /**
         * @method getDetailjson
         * @desc 根据线路id获取线路相关信息
         * @param id 线路id
         */
        public function getDetailjson(){
            $id = $this->input->get_post('id', TRUE);
            $sql = "SELECT displayorder FROM u_line WHERE id = $id";
            $ret = $this->db->query($sql)->row_array();
            $order = isset($ret['displayorder']) ? $ret['displayorder'] : 0;
            $data['id'] = $id;
            $data['order'] = $order;
            echo json_encode($data);
        }
        ...
        public function edit(){
            ...
            $showorder = intval($this->input->get_post('showorder', TRUE));
            $sql = "UPDATE u_line SET displayorder = $showorder WHERE id = $id";
            $this->db->query($sql);
            $this->callback->setJsonCode ( 2000 ,'操作成功');
        }
}