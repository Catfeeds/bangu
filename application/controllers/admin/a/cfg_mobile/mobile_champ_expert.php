...
class Mobile_champ_expert extends UA_Controller {
	public $controllerName = '手机端冠军管家';
	public function __construct()
	{
		parent::__construct ();
		$this->load_model('admin/a/mobile_champ_expert_model','champ_expert_model');
	}
	public function index()
	{
		$this->view ( 'admin/a/cfg_mobile/mobile_champ_expert_list');
	}
	...
	public function getMHotExpertData()
	{
		...
		$data = $this ->champ_expert_model ->getMhotExpert($whereArr);
	...
	public function add()
	{
		...
		$status = $this ->champ_expert_model ->insert($dataArr);
		$u_data = array(
	...
	public function edit() {
		...
		$status = $this ->champ_expert_model ->update($dataArr ,array('id' =>intval($postArr['id'])));
		$u_data = array(
				'big_photo'=>$dataArr['smallpic'],  //封面
				'small_photo'=>$dataArr['pic']      //小头像
		);
	...
	public function commonFunc($postArr ,$type)
	{
		...
		{
			if ($type == 'add') {
				$data = $this ->champ_expert_model ->row(array('expert_id' =>$postArr['expertId']) ,'arr' ,'' ,'id');
			} else {
				$data = $this ->champ_expert_model ->row(array('expert_id'=>$postArr['expertId'] ,'id !='=>$postArr['id'] ));
			}
			if (!empty($data))
			{
	...
	public function getDetailjson ()
	{
		...
		$data=$this ->champ_expert_model ->getMobileExpertDetail($id);
		echo json_encode($data);
	}
	...
	function delete(){
		$id = intval($this->input->post("id"));
		$status = $this ->champ_expert_model ->delete(array('id'=>$id));
		if (empty($status)) {
