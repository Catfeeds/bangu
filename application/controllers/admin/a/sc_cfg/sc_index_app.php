...
class Sc_index_app extends UA_Controller {
	public $controllerName = '深窗首页应用配置';
	public function __construct() {
		parent::__construct ();
		$this->load_model('admin/a/sc_cfg_model/sc_index_app_model','index_app');
	}

	function index(){
		$this->load_view ( 'admin/a/sc_cfg/sc_index_app');
	}
	...
	public function getDataList() {
		...
		//获取数据
		$data = $this ->index_app ->getData($whereArr ,$page_new ,sys_constant::A_PAGE_SIZE);
	...
	public function add() {
		...
		$status = $this ->index_app ->insert($navArr);
		if (empty($status)) {
			$this->callback->set_code ( 4000 ,"添加失败");
			$this->callback->exit_json();
		} else {
			$this ->log(1,3,'深窗首页应用配置','首页应用，名称：'.$name);
	...
	public function edit() {
		...
		$status = $this ->index_app ->update($dataArr ,array('id' =>intval($postArr['id'])));
		if (empty($status)) {
	...
	public function commonFunc($postArr ,$type){
		if (empty($postArr['name'])){
			...
			$navData = $this ->index_app ->row(array('name' =>$postArr['name']));
	...
	public function getOneData () {
		...
		$data=$this->index_app->row($whereArr);
		echo json_encode($data);
	}
	...
	function delete(){
		$id = intval($this->input->post("id"));
		$status = $this ->index_app ->delete(array('id'=>$id));
		if (empty($status)) {
