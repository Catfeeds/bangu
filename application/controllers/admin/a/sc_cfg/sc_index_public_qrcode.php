...
class Sc_index_public_qrcode extends UA_Controller {
	public $controllerName = '深窗首页二维码模块配置';
	public function __construct() {
		parent::__construct ();
		$this->load_model('admin/a/sc_cfg_model/sc_index_public_qrcode_model','public_qrcode');
	}

	function index(){

		$this->load_view ( 'admin/a/sc_cfg/sc_index_public_qrcode');
	}
	...
	public function getDataList() {
		...
		//获取数据
		$data = $this ->public_qrcode ->getData($whereArr ,$page_new ,sys_constant::A_PAGE_SIZE);
	...
	public function add() {
		...
		$status = $this ->public_qrcode ->insert($navArr);
		if (empty($status)) {
	...
	public function edit() {
		...
		$status = $this ->public_qrcode ->update($dataArr ,array('id' =>intval($postArr['id'])));
		if (empty($status)) {
	...
	public function commonFunc($postArr ,$type){
		if (empty($postArr['name'])){
			$this->callback->set_code ( 4000 ,"请填写二维码名称");
			$this->callback->exit_json();
		}else if(empty($postArr['pic'])){
			$this->callback->set_code ( 4000 ,"缺少二维码图片");
			$this->callback->exit_json();
		}
		return array(
			'name' =>$postArr['name'],
	...
	public function getOneData () {
		...
		$data=$this->public_qrcode->getData($whereArr);
	...
	function delete(){
		$id = intval($this->input->post("id"));
		$status = $this ->public_qrcode ->delete(array('id'=>$id));
		if (empty($status)) {
