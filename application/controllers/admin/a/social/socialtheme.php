...
class SocialTheme extends UA_Controller {
    const PAGE_SIZE = 10;
    ...
    public function index() {
        $this->view('admin/a/social/socialthemeview');
    }
    ...
    public function get_theme_data(){
        ...
        $sql = "SELECT id, name,is_recommend,app_pic, description, pic1,pic2,pic3, video_url1,video_url2,video_url3,video_pic1,video_pic2,video_pic3, show_order,line1,bg_pic icon, update_time FROM social_theme ORDER BY id DESC ";
        $sqls = $sql.$limit;
    ...
    public function edit_social_theme(){
    	...
    	if ($type == 1){    // 视频
            if ((empty($dataarr['video1'])&&empty($dataarr['video_pic1'])) && (empty($dataarr['video2'])&&empty($dataarr['video_pic2'])) && (empty($dataarr['video3'])&&empty($dataarr['video_pic3']))){
            	...
            	$update_data['pic2']='';
            	$update_data['pic3']='';
            	$this->social_db->update('social_theme', $update_data, array('id' => $id));
            }           
        }else if ($type == 2){  // 图片
...
//         if (empty($dataarr['line'])) $this->callback->setJsonCode ( 4001 ,'请填写关联线路id');
        ...
        $update=$this->social_db->update('social_theme', $data, array('id' => $id));
        if ($update){
...
//         if (empty($dataarr['line'])) $this->callback->setJsonCode ( 4001 ,'请填写关联线路id');
        ...
        $data['update_time'] = time();
        $data['app_pic']=$dataarr['app_pic'];
        $result=$this->social_db->insert('social_theme', $data);
        if ($result){
    ...
    public function del_social_theme(){
        ...
        $id = $id ? (int)$id : 0;
        $ret = $this->social_db->where('id', $id)->delete('social_theme');
        if (!$ret){
    ...
    public function indexVideoInfo(){
    	...
    	$sql_str = "select id,is_recommend from social_theme where id=".$id." ";
    	...
    	if($data['is_recommend']==1){//取消推荐
    		$status = $this->social_db->query("update social_theme set `is_recommend`=0 where id = ".$id."");
    	}else{//推荐
    		$status = $this->social_db->query("update social_theme set `is_recommend`=1 where id = ".$id."");
    	}
    	if($status){
    ...
    public function get_dtl_json(){
        ...
        $id = isset($id) ? $id : 0;
        // 获取该id为$id的一条记录
        $sql = "SELECT id, name,app_pic,video_pic1,video_pic2,video_pic3,description, pic1,pic2,pic3,bg_pic icon,line1, video_url1,video_url2,video_url3, show_order, update_time FROM social_theme WHERE id = ".$id;
    ...
    public function get_posts_by_theme_id($page = 1){
        ...
        $config['base_url'] = '/admin/a/social/socialtheme/get_posts_by_theme_id/' . $urlstr . 'page=';
        $config ['pagesize'] = SocialTheme::PAGE_SIZE;
        $config ['page_now'] = $page;
        $data = array();
        $from = ($page - 1) * 10;
        $query_posts = $this->social_db->query("SELECT * FROM social_theme_post WHERE theme_id = $theme_id ORDER BY last_ans_time DESC, update_time DESC LIMIT {$from}, {$config ['pagesize']}");
        $data['posts'] = $query_posts->result_array();
        $config ['pagecount'] = $this->social_db->query("SELECT id FROM social_theme_post WHERE theme_id = $theme_id")->num_rows();
        ...
        $this->load_view( 'admin/a/social/socialthemepostview_1',$data);
    }
