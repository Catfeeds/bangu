...
class SocialThemePost extends UA_Controller {
    ...
    public function index() {
        $this->view('admin/a/social/socialthemepostview');
    }
...
//        $from = ($page - 1) * $pageSize;
        $sql = "SELECT id, name, description, pic1 pic, video_url1 video_url, show_order, update_time FROM social_theme ORDER BY show_order DESC";
    ...
    public function get_post_count_by_id() {
        ...
        $sql = '';
        if (0 == $id) {
            $sql = "SELECT id FROM social_theme_post LIMIT 20";
        } else {
            $sql = "SELECT id FROM social_theme_post WHERE theme_id = $id";
        }
    ...
    public function get_post_data() {
        ...
        if (0 == $theme_id) {    // 如果没有选择任何主题,则显示5个帖子的内容
            $sql = "SELECT id, title, content, video_pic, video_url, pic1, pic2, pic3 FROM social_theme_post LIMIT 20";
        } else {
            $sql = "SELECT id, title, content, video_pic, video_url, pic1, pic2, pic3 FROM social_theme_post WHERE theme_id = $theme_id";
        }
        $ret = $this->social_db->query($sql)->result_array();
        foreach ($ret as &$v) {
            $post_id = isset($v['id']) ? $v['id'] : 0;
            // 获取帖子的回复
            $sql_reply = "SELECT * FROM social_theme_post_answer WHERE post_id = $post_id";
    ...
    public function get_post_by_id() {
        ...
        $data = $this->social_db->query("SELECT * FROM social_theme_post WHERE id = $post_id")->row_array();
        $data['post_type'] = 0;
...
//        $data['member_id'] = empty($member_id) ? 0 : intval($member_id);
        ...
        $this->social_db->insert('social_theme_post', $data);
...
//        $data['member_id'] = empty($member_id) ? 0 : intval($member_id);
        ...
        $this->social_db->update('social_theme_post', $data);
    ...
    public function del() {
        ...
        // 根据post_id获取theme_id
        $sql_theme = "SELECT theme_id FROM social_theme_post WHERE id = $post_id";
        $theme = $this->social_db->query($sql_theme)->row_array();
        $theme_id = isset($theme['theme_id']) ? $theme['theme_id'] : -1;
        $ret = $this->social_db->where('id', $post_id)->delete('social_theme_post');
        if (!$ret) {
            $this->callback->setJsonCode(4001, '删除失败，请重新尝试');
        } else {
            // 获取评论数
            $sql_ans = "SELECT COUNT(*) cnt FROM social_theme_post_answer WHERE post_id = $post_id";
            ...
            $this->social_db->query("UPDATE social_theme_hot_num SET ans_cnt = ans_cnt - $ans_cnt, praise_cnt = praise_cnt - $praise_cnt WHERE theme_id = $theme_id AND type = 1");
            // 更新评论表,删除此动态的评论记录
            $this->social_db->query("DELETE FROM social_theme_post_answer WHERE post_id = $post_id");
            // 更新点赞表,删除此动态的点赞记录
            $this->social_db->query("DELETE FROM social_praise WHERE type = 1 AND category_id = $post_id");
