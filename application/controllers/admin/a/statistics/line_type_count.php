...
class Line_type_count extends UA_Controller
{
	public $pagesize = 10;
	...
	public function index()
	{
		...
		$sql .= ' where mo.status >=4 and mo.status <=8 and d.level=2 '.$like.' group by d.id limit '.(($page-1)*$this->pagesize).','.$this->pagesize;
		//echo $sql;
		$countData = $this ->db ->query($sql) ->result_array();
		
		//获取总数量
		$sql = 'select count(*) as count from (select mo.id from u_member_order as mo left join u_line as l on l.id=mo.productautoid left join u_line_dest as ld on ld.line_id=l.id left join u_dest_base as d on d.id=ld.dest_id  where mo.status >=4 and mo.status <=8 and d.level=2 '.$like.' group by d.id) as a';
		...
		$config = array(
				...
				'page_now' =>$page,
				'base_url' =>'/admin/a/statistics/line_type_count/index?'.$search.'&page='
		);
		...
		$this ->view('admin/a/statistics/line_type_count' ,$dataArr);
	}
