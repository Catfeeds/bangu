...
class Order_list extends  UB1_Controller{
	...
	public function index() {
		...
		$this->load->view ( "admin/b1/b_order/order_list_view",$data);
...
/*	function through_Oderprice(){
		...
		if($bill_id>0 && $orderid>0){
			...
			$re=$this->apply_order->update_bill_yf($bill_id,$orderid,$arr['id']);
			//var_dump($re);
			if($re){
				if($re==2){
...
/*	function refuse_Oderprice(){
		...
		if($bill_id>0 && $orderid>0){
			...
			$re=$this->apply_order->refuse_bill_yf($bill_id,$orderid,$arr['id']);
			if($re){
				echo  json_encode(array('status'=>1,'msg'=>'操作成功'));
			}else{
				echo  json_encode(array('status'=>-1,'msg'=>'操作失败'));
