...
/* 	function L_line_dest(){
		...
		foreach ($line as $k=>$v){
			...
			foreach ($overcityArr as $key=>$val){	
				if(!empty($val)){
					...
					$destBase= $this ->user_shop_model ->select_rowData('u_destinations',array('id'=>$val));
					...
					if(!empty($destBase)){
						...
						if(!empty($newdest[0]['id'])){  //一对一匹配
							...
							$str=$str."目的地id{$destBase['id']}"."匹配不上";
						}
					}else{ 
	...
	function sel_line_dest(){
		...
		$sql='select l.id,l.features from u_line as l where  l.line_kind=1 and l.addtime>"'.$startdate.'" and l.addtime<="'.$endtdate.'" ';
		$rows = $this->db->query($sql)->result_array();
		foreach ($rows as $k=>$v){
			if(!empty($v['id'])){
				$features=strip_tags($v['features']);
				$this ->user_shop_model ->update_rowdata('u_line',array('features'=>$features),array('id'=>$v['id']));
			}
...
/* 	function replace_live_video(){
        ...
        if(!empty($rows)){
			foreach ($rows as $k=>$v){ 
				if(!empty($v["kindname"])){
				    echo $v["kindname"].'<br>';
					$newdest=$this ->user_shop_model ->select_data("u_dest_base",array("kindname"=>$v["kindname"]));
	...
	function get_line_f(){
		$sql="select id,features from u_line";
		$rows = $this->db->query($sql)->result_array();
		foreach ($rows as $k=>$v){
			if(!empty($v['id'])){
				$features=strip_tags($v['features']);
