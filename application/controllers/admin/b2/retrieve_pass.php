...
class Retrieve_pass extends MY_Controller
{
	...
	{
		$this->load->view ( 'admin/b2/retrievePassword');
	}
	//找回密码验证
	public function do_retrieve_pass()
	{
		...
		/**管家找回密码，不区分账号的状态，状态由登陆时去判断*/
		$expertData = $this ->expert_model ->all(array('login_name' =>$mobile));
		if (empty($expertData))
		{
			$this ->callback ->setJsonCode(4000 ,'您的手机号没有注册');
		}
		...
		$status = $this ->expert_model ->update($expertArr ,$whereArr);
		if ($status == false)
		{
