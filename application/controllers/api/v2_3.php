...
//继承MY_Controller类
class V2_3 extends MY_Controller {
	...
	{
		$sql="select 
					e.id,e.order_amount,e.people_count,e.nickname, e.small_photo,e.login_name,e.sex,e.realname,
					e.satisfaction_rate,e.total_score,e.talk,e.praise_num,e.beizhu,
					a.name as city, g.title as grade,
		            ...
		            (select GROUP_CONCAT(name) from u_area where FIND_IN_SET(id,e.visit_service) >0 )as likeplace 
		     from 
					u_expert AS e 
	...
	protected function F_get_eid($token)
	{
		if($token)
		{
			$this->load->model ( 'common/u_expert_access_token_model', 'at_model' );
			$row = $this->at_model->row( array ('access_token' => $token));
			if(empty($row))
				$e_id="0";
			else
	 ...
	 *
	 */
	public function cfgm_expert_detail2() {
		//1、传值
		$e_id = intval ( $this->input->post ( 'expertid', true ) );
		$token = $this->input->post ( 'number', true );
		...
		$expert_detail['school']=mb_substr(@$beizhu_arr[0],3);
		$expert_detail['workyear']=mb_substr(@$beizhu_arr[2],0,-5);
        ...
        $expert_info['hobby']=isset($expert_more['hobby'])?$expert_more['hobby']:''; //爱好
        $expert_info['pass_way']=isset($expert_more['pass_way'])?$expert_more['pass_way']:'';//去过的地方
        $expert_info['like_food']=isset($expert_more['like_food'])?$expert_more['like_food']:''; //喜欢美食
...
WHERE `tn`.`id` = {$eid} ORDER BY `tnr`.`addtime` desc      limit {$from},{$page_size}  ";
	...
	*/
	...
	public function cfgm_expert_custiome_list() {
		...
		$this->__outmsg ( $data ['customiz_list'] );
		}
		$sql = rtrim ( $sql, "limit {$from},{$page_size} " );
		$query = $this->db->query ( $sql );
		$total_rows = $query->num_rows ()-3; //总条数-3
	...
	public function cfgm_expert_comment_list() {
		...
		if (empty ( $data ['comment_list'] )) {
			$this->__outmsg ( $data ['comment_list'] );
		}
		$sql = rtrim ( $sql, "limit {$from},{$page_size} " );
		$query = $this->db->query ( $sql );
		$total_rows = $query->num_rows ()-3; //总条数-3
	...
	public function cfgm_expert_condition() {
		...
		{
			$this->__outmsg ( $data ['condition_list'] );
		}
		$sql = rtrim ( $sql, "limit {$from},{$page_size} " );
		$query = $this->db->query ( $sql );
		$total_rows = $query->num_rows ()-3; //总条数-3
	...
	*/
	...
	public function cfgm_expert_collect() {
		...
		else
		{
			...
			if($status)
				echo json_encode ( array ('code' => 2000,'msg' => '取消收藏！') );
			else
				$this->__errormsg('操作异常');
		}
	}
	...
	/**
	 ***********************************      即使导游接口         *********************************************************/
	public function ____________________________E()
	{
	...
	public function E_expert_info_show() {
		$token = $this->input->post ( 'number', true );
	...
	public function E_expert_order() {
		$token = $this->input->post ( 'number', true );
		$this->expert_check_token ( $token );
		$this->load->model ( 'common/u_expert_access_token_model', 'eat_model' );
		$user = $this->eat_model->result ( array (
	...
	public function E_tourist_info() {
		$token = $this->input->post ( 'number', true );
		$this->expert_check_token ( $token );
	...
	public function E_customize_manage() {
    	$token = $this->input->post ( 'number', true );
		$this->expert_check_token ( $token );
		$this->load->model ( 'common/u_expert_access_token_model', 'eat_model' );
		$user = $this->eat_model->result ( array (
...
*/
	...
	public function E_phone_password() {
    ...
    * @name：管家退出登录
	...
	public function E_no_apply_line_detail() {
		$this->load->model ( 'admin/b2/line_apply_model', 'line_apply' );
...
// 		$this->expert_check_token ( $token );
 ...
 /**
     ...
     {
     	$country_id=$this->input->post("country_id",true); //国家id
     	//$country_id="2";
     	...
     	$scenic_sql="select
     				 ...
     				 from
	     					...
	     					left join u_area as a on s.area_id=a.id
	     			 where
     						s.status=1 and c.country_id={$country_id}
     	             order by 
     	             	    s.displayorder desc
     	";
     ...
     public function G_scenic_detail()
     {
     	...
     	$output=array();
     	$scenic_sql="select s.* from scenic_spot as s where s.id={$spot_id}";
     	...
     	$output['spot_zb']=$this->F_scenic_zb($geohash,$output['geohash']); //$geohash 5公里范围，$output['geohash'] 不包括当前景区
     ...
     public function G_add_comment()
     {
		...
		$m_id=$this->F_get_mid($token);//根据token获取mid
		//$m_id="291";
		if(!$content||!$spot_id) $this->__errormsg('param missing');
     ...
     protected function F_scenic_zb($geohash,$not_in="")
     {
     	//发送curl
     	...
     	if($not_in!="")
     		$scenic_sql.=" and s.geohash!='{$not_in}'";
     	$scenic_sql .= " order by id desc limit 5";
     	$output=$this->db->query($scenic_sql)->result_array(); //景点
		...
		try {
			...
			switch ($type) {
				...
				case 4 : // 管家注册
					...
					// 验证手机号状态
					if ($usertype == '1') {
						$result_user = $this->db->query ( "select  status  from u_expert  where (mobile={$mobile} or login_name={$mobile}) and status!=3 ORDER BY `id` desc " )->result_array ();
		...
		if ($couActionData [0] ['isrepeat'] == 0) // 不可以重复参与
{
	...
	public function __data($reDataArr, $len = 1) {
