...
class C extends APP_Controller {
    ...
    public function cfgm_home() {
        ...
        $this->db->select("e.id AS eid,e.talk,e.big_photo,e.small_photo,e.small_photo as m_bgpic,e.realname,a.name,e.praise_num,e.avg_score,(CASE WHEN grade=1 THEN '管家' WHEN grade=2 THEN '初级' WHEN grade=3 THEN '中级' WHEN grade=4 THEN '高级' END) AS grade");
...
//        $this->__outmsg($output);
    ...
    public function cfgm_dest_list() {
        ...
        $dest_type = intval($this->input->post('dest_type', true)); //分类（1精选、2国内、3境外、4周边）
...
//                                u_expert AS e JOIN u_expert_collect AS c on c.expert_id = e.id and c.member_id = {$member_id} limit {$from},{$per_page}"; // 改为按成交量将序排列
    ...
    /**
     * 辅助函数
     * 生成u_dest_cfg中id与kindname的对应关系数组
     */
    public function generate_id_name_array() {
     ...
     * 将数字字符串转为对应的中文字符串
     */
    private function generate_chn_array($int_str) {
