...
class Z_live_room extends APP_Controller {
	...
	public function assign_room(){
		...
		if($anchor['is_anchor']==1){
			...
			if(!empty($room)){
				...
				$result = array("code"=>201,"res"=>$room);
			}else{
				...
				if($anchor['room_id']!=0){
					...
					if(!empty($room)){
						...
						$result = array("code"=>202,"res"=>"已经没有空闲房间");
					}
				}
		    }
		}else{

			$result = array("code"=>208,"res"=>"请申请主播");
		}
	...
	function start_live(){
		...
		$live_status = $this->input->post("live_status",true);
		//$this->api_room->start_live($anchor_id,$room_id);
		$anchor_data = array("live_status"=>1);
		$room_data = array( "live_status"=>$live_status);
	...
	public function releaseRoom(){
		...
		$room_data = array( "live_status"=>0, "anchor_id"=>0);//关闭房间播放状态
		$this->api_room->update_live($anchor_id,$anchor_data,$room_id,$room_data);
		////////////////////////////////////////////////////////////////////////////////////
		...
		if($have_video){
			...
			$video_record = self::get_new_video($room['roomid']);
			...
			$video_data = array(
					...
					"time"			=>$video_record['duration'],//$minutes,
					"video"			=>$video_record['firstHlsUrl']
				);
		...
		$result = array("code"=>200,"res"=>"退出房间成功");
		$this->__outmsg($result);
	}
