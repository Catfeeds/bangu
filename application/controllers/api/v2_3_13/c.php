    ...
    public function cfgm_more_expert() {
        ...
        $sql = "(SELECT * FROM (SELECT 
				...
				FROM 	
						...
						left join u_expert_grade as eg on e.grade=eg.grade
				WHERE 	
						la.line_id = {$l_id} AND la. STATUS = 2 AND e.city = {$cityid} AND e.`status`=2 and e.is_kf='N'   {$where}  ) AS hot limit 25) UNION
                (SELECT * FROM (SELECT 
				 		e.id AS expert_id,e.realname,e.nickname, e.big_photo,e.small_photo,e.comment_count,
						e.attention_count,e.sex,e.online,e.expert_dest as expert_destid, e.expert_theme, e.satisfaction_rate, 
						e.people_count, ua.name as cityname, e.title as grade,	    
				...
				FROM 	
						...
						left join u_expert_grade as eg on e.grade=eg.grade
				WHERE 	
						la.line_id = {$l_id} AND la. STATUS = 2 AND e.city = {$cityid} AND e.`status`=2 and e.is_kf='N'   {$where}  ) AS common limit 25)";
        $reDataArr = $this->db->query($sql)->result_array();
        foreach ($reDataArr as $key => $val) {
            foreach ($val as $k => $v) {
                if ($k == "satisfaction_rate") {
...
//                                u_expert AS e JOIN u_expert_collect AS c on c.expert_id = e.id and c.member_id = {$member_id} limit {$from},{$per_page}"; // 改为按成交量将序排列
    ...
    private function home_video_room() {
        $this->live_db = $this->load->database("live", TRUE);
        $nowtime = time();
        $sql_str = '(select anchor_id,room_id,0 as video_id, room_name as name,0 as read_num,peoples as watching_num,pic as cover,starttime as addtime,peoples as peopleby,3 as type,room_dest as dest_name,room_dest_id as dest_id,1 as isroom from live_room  where live_status>0 and status=1  and  createtime>' . ($nowtime - $this->room_timeout) . '  limit 6 )union (select anchor_id,room_id,id as video_id,name,people as read_num,0 as watching_num,pic as cover,addtime,people as peopleby,type,dest_name,dest_id,0 as isroom from live_video where video<>"" and status=1  limit 6 ) order by isroom desc, peopleby desc,addtime desc  LIMIT 6';
        $live_room_data = $this->live_db->query($sql_str)->result_array();
        $num_live_room_data = count($live_room_data);
        $anchor_ids = array();
        if ($num_live_room_data > 0) {
