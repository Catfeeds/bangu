...
class C extends APP_Controller {
    ...
    public function cfgm_home() {
        ...
        $dest_sql = "select cd.dest_id as id,cd.dest_type,ud.kindname as destname,cd.name as description,cd.pic as pic,cd.linenum as num   		from cfgm_hot_dest as cd  		left join u_destinations as ud on ud.id=cd.dest_id 	left join cfg_index_kind as cik on cik.id=cd.dest_type 		where cd.is_show='1' and cd.startplaceid={$cityid}";
...
//		$reDataArr ['line'] ['zb'] = $this->db->query ( $zb_line_sql )->result_array ();
        ...
        $hot_line_sql = "SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id
                    ...
                    where l.status='2' and cx.startplaceid = {$cityid} and cx.is_show=1 and cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime) >= {$time} order by cx.showorder desc limit 6 ";
    ...
    public function cfgm_expert_detail() {
        ...
        $sql2 = "
				...
				from
							...
							left join u_destinations as ud on ud.id=uc.endplace 	
				where 
...
//        $this->__outmsg($output);
    ...
    public function cfgm_destinations() {
        ...
        $sql = "SELECT  
				...
				FROM 
						u_destinations as d 
						LEFT JOIN  u_destinations d2  ON d.pid =d2.id 
				WHERE   
        ...
        ksort($reDataArr);  //排序
        //2、周边
        $zb = $this->db->query("select d.id,d.kindname as name,d.enname,d.simplename ,UPPER(SUBSTRING(d.simplename,1,1 )) AS ch,d.ishot,d.level from cfg_round_trip as t left join u_destinations as d on d.id=t.neighbor_id where t.startplaceid='{$cityid}' and t.isopen=1")->result_array();
        $reDataArr_zb = array();
        if ($zb) {
            foreach ($zb as $k => $v) {
    ...
    public function cfgm_find_destinations() {
        ...
        //"中文"搜索出发城市
        $reDataArr = $this->db->query("select d.id,d.kindname as name,UPPER(SUBSTRING(d.simplename,1,1 )) AS ch from u_destinations as d left join u_destinations as d2 on d.pid=d2.id where d.level=3 and d2.pid='{$type}' and d.kindname LIKE '%" . $dest . "%'")->result_array();
        //首字母搜索出的城市
        $reDataArr2 = array();
        $re = $this->db->query("select d.id,d.kindname as name,UPPER(SUBSTRING(d.simplename,1,1 )) AS ch from u_destinations as d left join u_destinations as d2 on d.pid=d2.id where d.level=3 and d2.pid='{$type}'")->result_array();
        foreach ($re as $key => $value) {
            if ($value['ch'] == $first_word)
                $reDataArr2[] = $value;
        }
    ...
    /**
     * @name：公共接口：目的地三级联动（国家、省、市） u_destinations表
     * @author: 温文斌
     * @param:
     * @return:
     *
    ...
    public function dest_list_three() {
        ...
        $areaData = $this->db->query("select id,kindname as name,pid,level from u_destinations where isopen=1 and level<4  order by level asc,displayorder asc ")->result_array();
        $areaArr = array();  //格式1
        $areaArr2 = array();  //格式2
        ...
        if ($cityid) {
            ...
            $zb = $this->db->query("select d.id,d.kindname as name,d.pid,d.level from cfg_round_trip as t left join u_destinations as d on d.id=t.neighbor_id where t.startplaceid='{$cityid}' and t.isopen=1")->result_array();
        }
    ...
    public function cfgm_line_sort() {
        ...
        $cjy_sql = "select
				   ...
				   from
							u_destinations as dd
				   where
							dd.pid IN (SELECT d.id FROM u_destinations as d where d.pid=1) and dd.`level`=3 and dd.ishot=1
				   limit 15
				    ";
        $gny_sql = "select
							dd.id,dd.kindname as name
				   from
							u_destinations as dd
				   where
							dd.pid IN (SELECT d.id FROM u_destinations as d where d.pid=2) and dd.`level`=3 and dd.ishot=1
				   limit 15
				    ";
        $reDataArr ['cjy'] = $this->db->query($cjy_sql)->result_array();
        $reDataArr ['gny'] = $this->db->query($gny_sql)->result_array();
        //$reDataArr ['cjy'] = $this->db->query ( "select dd.id,dd.kindname as name  from u_destinations as dd where dd.pid IN (SELECT d.id FROM u_destinations as d where d.pid=1) and dd.`level`=3  and dd.isopen=1  limit 15 " )->result_array ();
        //$reDataArr ['gny'] = $this->db->query ( "select dd.id,dd.kindname as name from u_destinations as dd where dd.pid IN (SELECT d.id FROM u_destinations as d where d.pid=2) and dd.`level`=3  and dd.isopen=1   limit 15 " )->result_array ();
        //主题游
        ...
        //$reDataArr ['zby'] = $this->db->query ( "	select u.kindname as name ,u.id from cfg_round_trip as c LEFT JOIN u_destinations as u on u.id = c. neighbor_id where c.startplaceid='235' and u.isopen=1   " )->result_array ();
    ...
    public function cfgm_all_product() {
        $reDataArr = array();
        $internalId = 1; // 国际ID
        $sql_inter_2 = "select * FROM  u_destinations where  pid={$internalId}  order by pid asc ";
        $inter_citys_2 = $this->db->query($sql_inter_2)->result_array();
        $sql_inter_3 = "select * FROM  u_destinations where  level=3  order by pid asc ";
        ...
        $domesticId = 2; // 国内ID
        $sql_dome_2 = "select * FROM  u_destinations where  pid={$domesticId}  order by pid asc ";
        $dome_citys_2 = $this->db->query($sql_dome_2)->result_array();
        $sql_dome_3 = "select * FROM  u_destinations where  level=3  order by pid asc ";
    ...
    public function cfgm_line_list() {
        ...
        if ($areaid) {
            ...
            $area = $this->db->query("select kindname from u_destinations where id='{$areaid}'")->row_array();
//            var_dump("select kindname from u_destinations where id='{$areaid}'".'-----------------------1121--------------');
//            file_put_contents('/tmp/mysql.txt', "select kindname from u_destinations where id='{$areaid}'".'-----------------------1121--------------',FILE_APPEND);
            $title = $area['kindname'];
        }
...
//        file_put_contents('/tmp/mysql.txt', $sql_page.'------1172---------',FILE_APPEND);
    ...
    public function cfgm_dest_list() {
        ...
        $sql_hot = "select
				...
				from
				cfgm_hot_dest as cd
                                left join u_destinations as ud on ud.id=cd.dest_id
				where
...
//            $sql = 'SELECT kindname, description, line_num, pic FROM u_destinations WHERE id not in (select dest_id from cfgm_hot_dest) and level = 3 AND pid in (SELECT id FROM u_destinations WHERE pid = 2) AND isopen = 1 ORDER BY displayorder DESC, line_num DESC';
...
//            $list = array_merge($list, $list2);
            $sql = "(select
				...
				order by cd.showorder desc, cd.linenum desc)
                    UNION
            (SELECT id AS dest_id, kindname, description, line_num, pic FROM u_destinations WHERE id not in (select dest_id from cfgm_hot_dest) and level = 3 AND isopen = 1 ORDER BY displayorder DESC, line_num DESC) limit {$from},{$page_size}";
...
//            file_put_contents('/tmp/mysql.txt', "select * from cfgm_index_roll_pic where kind=4 and status=1".'----------------1236----------',FILE_APPEND);
            ...
            $sql = "(select
				...
				order by cd.showorder desc, cd.linenum desc)
                    UNION
            (SELECT id AS dest_id, kindname, description, line_num, pic FROM u_destinations WHERE id not in (select dest_id from cfgm_hot_dest) and level = 3 AND pid in (SELECT id FROM u_destinations WHERE pid = 2) AND isopen = 1 ORDER BY displayorder DESC, line_num DESC) limit {$from},{$page_size}";
            $list = $this->db->query($sql)->result_array();
...
//            $list = $this->cfgm_hot_dest_model->hot_dest_list(array('dest_type' => '1', 'startplaceid' => $cityid), $from, $page_size);
            $sql = "(select
				...
				order by cd.showorder desc, cd.linenum desc)
                    UNION
            (SELECT id AS dest_id, kindname, description, line_num, pic FROM u_destinations WHERE id not in (select dest_id from cfgm_hot_dest) and level = 3 AND pid in (SELECT id FROM u_destinations WHERE pid = 1) AND isopen = 1 ORDER BY displayorder DESC, line_num DESC) limit {$from},{$page_size}";
            $list = $this->db->query($sql)->result_array();
...
//            $list = $this->cfgm_hot_dest_model->hot_dest_list(array('dest_type' => '3', 'startplaceid' => $cityid), $from, $page_size);
    ...
    public function cfgm_expert_list() {
        ...
        if (!empty($dest_id)) {
            ...
            foreach ($arr as $a => $b) {
                //二级
                $two = $this->db->query("select id,kindname from u_destinations where (id='{$b}' or pid='{$b}') and isopen=1 and (level=2 or level=3)")->result_array();
                foreach ($two as $o => $e) {
                    array_push($dest_array, $e['id']);
                }
                //三级
                $three = $this->db->query("select id,pid,kindname from u_destinations where id='{$b}' and isopen=1 and level=3")->result_array();
      ...
      e.expert_dest as expert_destid,(select GROUP_CONCAT(d.kindname SEPARATOR ' ') as expert_dest from u_destinations as d where FIND_IN_SET(d.id,e.expert_dest)) as expert_dest
...
//					";
    ...
    public function cfgm_my_customize_list() {
        ...
        $sql = "
				SELECT  
						...
						(select GROUP_CONCAT(kindname) from u_destinations where FIND_IN_SET(id,c.endplace) >0 )as dest , 
    ...
    public function cfgm_custoimze_detail() {
        ...
        $data = $this->db->query("
				    SELECT  
						c.id,c.startplace,us.cityname, 
						(select GROUP_CONCAT(kindname) from u_destinations where FIND_IN_SET(id,c.endplace) >0 )as dest , c.addtime,
				        (SELECT COUNT(*) FROM u_customize_reply AS r WHERE r.customize_id=c.id and r.answer_id!=0) AS answer_num,
						(SELECT COUNT(*) FROM u_customize_reply AS r WHERE r.customize_id=c.id) AS expert_num,
    ...
    public function cfgm_customize_city_list() {
        ...
        if (is_numeric($cityId) && !empty($cityId)) {
            ...
            $this->load->model('app/u_destinations_model', 'dest_model');
...
//		              mb_od.addtime DESC,mb_od.status";
        ...
        mb_od.usedate AS day, mb_od.total_price, ls.lineid, mb_od.settlement_price, (mb_od.total_price+mb_od.settlement_price) as all_price,
        CASE WHEN mb_od.ispay = 0 THEN '未支付' WHEN mb_od.ispay = 1 THEN '已首付' WHEN mb_od.ispay = 2 THEN '已支付' END pay_status,
        CASE WHEN mb_od.id IN (SELECT r.order_id FROM u_refund AS r WHERE r.status = 0) AND mb_od.status = -3 THEN '退款审核中' WHEN mb_od.id IN (SELECT r.order_id FROM u_refund AS r WHERE r.status = 1) AND mb_od.status = -4 THEN '退款成功' WHEN mb_od.id IN (SELECT r.order_id FROM u_refund AS r WHERE r.status = 2) AND mb_od.status = -4 THEN '退款失败' WHEN mb_od.ispay = 0 AND TIMESTAMPDIFF(HOUR, mb_od.addtime, NOW())>24 THEN '已经失效' WHEN mb_od.status = -4 THEN '已经取消' WHEN mb_od.status = -3 THEN '取消中' WHEN mb_od.status = -2 THEN '平台拒绝' WHEN mb_od.status = -1 THEN 'B1拒绝' WHEN mb_od.status = 0 THEN '待支付' WHEN mb_od.status = 1 THEN 'B1已确认留位' WHEN mb_od.status = 2 THEN '用户已付款' WHEN mb_od.status = 3 THEN '平台已确认收款' WHEN mb_od.status = 4 THEN 'B1已控位' WHEN mb_od.status = 5 THEN '出行' WHEN mb_od.status = 6 THEN '点评' WHEN mb_od.status = 7 THEN '已投诉' WHEN mb_od.status = 8 THEN '专家已点评' END order_status
        FROM
        u_member_order AS mb_od LEFT JOIN
        u_member AS mb ON mb_od.memberid = mb.mid
        left join u_line as l on mb_od.productautoid = l.id
    ...
    public function cfgm_order_action_do() {
        ...
        if ($ts) { // 投诉
            ...
            $data ['user_name'] = $this->input->pos
            t('user_name', true); // 用户姓名
            $data ['addtime'] = date('Y-m-d H:i:s', time());
            $data ['status'] = 0;
    ...
    public function cfgm_offline_pay() {
        ...
        $this->order_status_model->

        update_order_status_cal($order_id); //统计订单ID状态用
 ...
 => 4000,
        ...
        $token  =  $this-> input-> post( 'number',  true);
                ...
                $this->db->where(array( 'mid'   =>  $mid )); $query = $this->db->get();
                $reDataArr = $query->result_array();
                $this->__outmsg($reDataArr);
                }
...
] = $this->input-> post( 'truename',  true) ; //真名
        ...
        $content  =  $this->input->post("content");
				...
				SELECT 
						...
						e.visit_service as visit_serviceid,(select GROUP_CONCAT(name SEPARATOR ' ') from u_area where level=4 and FIND_IN_SET(id,e.visit_service) >0 )as visit_service, 
						 (select GROUP_CONCAT(d.kindname SEPARATOR ' ') as expert_dest from u_destinations as d where FIND_IN_SET(d.id,substring_index  (e.expert_dest,',',3))) as expert_dest
                ...
                $callback = empty($_REQUEST[ "callback" ] ) ? '' : $_REQUEST['callback'];
                        ...
                        $page
             =  intval(isset($_REQUEST[ 'page'] )  ? $_REQUEST [ 'page'] : '1');
            ...
            $page_size = intval($this->input->post('pagesize', true));
                ...
                $this->__errormsg('param missing');
                $sql = "select 
						la.id,la.expert_id,l.id AS line_id,l.linename,l.linetitle,l.mainpic ,l.linep rice,
						l.satisfyscore,l.all_scor e,l.pe oplecount, l.shownum,l.collectnum,sharecount,
        ...
        $page_size = intval( $this-> input->post('pagesize', true));
				...
				select 
							uc.id as id,uc.question as  question ,uc.budget as budget,uc.pic as litpic ,
				 			uc.total_people as total_people,uc.startdate as startdate,uc.estimatedate as estimatedate,
							uc.startplace as startplace,uc.endplace as endplace,uc.status as status,uca.expert_id as expert_id,
							uca.isuse as isuse, ua.name as area_name,ud.kindname as dest
                _name, ue.nickname as nickname 	 
				from
							u_customize as uc 
							left join u_customize_answer as uca on uc.id=uca.customize_id 	
							left join u_line as l on uc.line_id=l.id 		
 ...
 => $page,
        ...
        $eid = intval( isset( $_REQUEST[ 'eid'] ) ? $_REQUEST [      'eid' ] : '0');
                $page = intval(isset($_REQUEST['page']  ) 
...
(empty($data ['comment_list'])) {
        $this->__wap( $data['comment_list'
...
($_REQUEST['page']) ? $_REQUEST['page'] : '1');
            ...
            $page =  $page == '0'? 1 : $page;
                    ...
                    if (empty($callback))
                    echo json_encode(array('code' => 2000, 'msg' => '收藏成功！')); else
                    ...
                    $this->db->query("update u_expert set `attention_count`=`attention_count`-1 where id = {$eid}");   //更新管家收藏数					
                    if (empty($callback))
                    echo json_encode(array('code' => 2000, 'msg' => '已取消收藏！')); else
            ...
            $mid = isset($_REQUEST['mid']) ? $_REQUEST[ 'mid'] : '0'; //wap端 ：用户id
                    $token = isset($_REQUEST[ 'number' 

]) ? $_REQUEST['number'] : ''; //app端 : 用户session
            //$eid = $this->input->post ( 'eid' ,true);
            //$mid = $this->input->post ( 'mid' ); //wap端 ：用户id
            //$token = $this->input->post ( 'number', true );  //app端 : 用户session
            //$mid="291";
            ...
            $this->__wapmsg('param missing', $callback);
                     ...
                     *
                     */
                    public function cfgm _customi ze_list() {
                    $page  = int val($this->input->post('page', true));
                    $page_size = intval($this->input->post('pagesize', true));
                    $page_size = empty($page_size)  ?  6  : $page_size;
                    $page = $page == '0'  ? 1  : $page;
                    $from = ($page - 1) * $page_size;
                    $sql = " SELECT c.id ,c.question,c.pic,c.budget,c.people,	eg.title  AS grade,e.realname,e.nickname,e.small_photo,e.id AS expert_id  FROM u_customize_answer AS ca    LEFT JOIN u_customize AS c ON ca.customize_id= c.id   L EFT JOIN u_expert AS e ON ca.expert_i d=e.id      left join u_expert_grade as eg on e.grade=eg.grade   WHERE ca.isuse=1 and c.status = 3 and e.status=2    order by c.id desc     limit {$from},{$page_size}  ";
                    $query = $this->db->query($sql);
                    $data ['line_list'] = $query->result_array();
                    if (empty($data ['line_list'])) {
                ...
                ost('content'); //反馈内容
                    ...
                    $status = $th
                is->db->insert("u_feekback", $insert_data);
                    if ($status)
                    $this->__data('反馈已提交'); else
     	...
     	";
                    ...
                    if ($re)
                    $this->__data(array('msg' => 'success')); else
                    $this->__errormsg('操作异常');
                    }
                    ...
                    $this->__errormsg('设备id不能为空');
                    $output = array();
                    $scenic_sql = "select s.*,b.city as area_name,(select id from scenic_spot_praise_record where spot_id=s.id and equipment_id='{$equipment_id}' and add_day='{$add_day}') as is_zan from scenic_spot 
                as s left join scenic_spot_belong as b on b.id=s.belong_id left join u_area as a on a.id=s.area_id where s.id={$spot_id}";
                    ...
                    $output['pic_arr'] = $this->db->query("select p.scenic_spot_id,p.pic from scenic_spot_pic as p where p.scenic_spot_id={$spot_id} limit 6")->result_a
                rray(); //景点图集
     	...
     	")->result_array();
     		...
     		select 
     			rp.pic
     		from scenic_spot_review as r left
                 join  scenic_spot_review_pic as rp on rp.review_id = r.id
     	...
     	")->num_rows();
                ...
                b'] = array();
                    ...
                    $zb3 = $this->geoh
                ash_sort($output['latitude'], $output['longitude'], $zb4);  //对周边景点从近到远排序
                    }
                    ...
                    $destinations = $this->db->query("select id,pid from u_destinations where kindname like '%{$area_name}%'")->row_array();
                ...
                if (!$spot_id)
                    ...
                    if ($total_nums == "0") {
                    $sql = "select
		     					 l.id,l.linename,lineprice,l.satisfyscore,l.peoplecount,l.com ment_c ount,l.mainpic,l.overcity
     		...
     		order by 
                    ...
                    $result = $this->db->insert("scenic_sp
                ot_review", $data);
                    $id = $this->db->insert_id();   //评论的id
                    $output = $data;
                    ...
                    $equipment_id = $this->input->post('equipment_id', true); // 设备id (每个设备每天只能点赞一次)
                      //$this->check_token ( $token );
                    $m_id
                 = $this->F_get_mid($token); //根据token获取mid
                    //$m_id="291";
                    //$comment_id="4";
                    //$flag="1";
                   ...
                   if (empty($is_praise)) {  //是否对评论已赞
                    $praise_num =
                 $comment_detail['praise'] + 1;
                    $down = $comment_detail['down'] + 1;

                    $this->db->trans_begin(); //事务开启
                    $t
                his->db->where(array('id' => $comment_id));
                    if ($flag == "1")
                    $status = $this->db->update("scenic_spot_review", array('praise' => $praise_num)); else
                ...
                ;
                    ...
                    //周边游
                    $sql_zb = "select neighbor_id as id,d.pic as pic,d.kindname as name FROM  cfg_round_trip as t left
                 join u_destinations as d on d.id=t.neighbor_id where  t.isopen=1 and d.isope
                n=1 and startplaceid='{$cityid}' order by t.id desc";
                    ...
                    $dest = $this->db->query("select id,kindname as name from u_destinations where kindname like '%{$area_name}%'")->row_array();
                    if (empty($zb)) {
                ...
                arr;
                    ...
                    $scenic_sql = "select
     				 ...
     				 from
     						scenic_spot as s
     						left join u_destinations as d on s.city_id=d.id
     				where
...
//发送curl  
                    ...
                    //周边景区
                     $out put = $this->F_scenic_zb($geohash, $limit = "1000");  // $limit 不做限制
...
//							u_destinations as d ON d.id = h.id
//				     where
//							d.isopen=1  order by h.showorder desc, h.linenum DESC limit 11";  // 将by id改为by displayorder
                    //$dest_sql = "SELECT * FROM cfgm_hot_dest WHERE startplaceid = $cityid OR startplaceid in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = $cityid) OR startplaceid = 391 ORDER BY showorder DESC, linenum DE
                SC LIMIT 11";
...
//						status=2 and nickname like '%{$content}%' order by addtime desc limit 5";
                    $expert_sql = "SELECT e.id, e.nickname, e.big_photo, e.small_photo, e.m_bgpic FROM cfgm_ho
                t_expert AS h JOIN u_expert AS e on h.expert_id = e.id where e.nickname like '%{$content}%' and e.city={$cityid} ORDER BY h.showorder DESC, e.order_count DESC limit 5";
                 ...
                 public function line_home_search() {
                    ...
                    //$dest_sql = "select cd.dest_id as id,cd.dest_type,ud.kindname as destname,cd.name as description,cd.pic as pic,cd.linenum as num from cfgm_hot_dest as cd left join u_destinations as ud on ud.id=cd.dest_id 	left join cfg_index_kind as cik on cik.id=cd.dest_type 		where cd.is_show='1' ";
                    $dest_sql = "select cd.startplaceid as start_id, cd.dest_id as id,cd.dest_type,ud.kindname as destname,cd.name as description,cd.pic as pic,cd.linenum as num from cfgm_hot_dest as cd left join u_destinations as ud on ud.id=cd.dest_id 	left join cfg_index_kind as cik on cik.id=cd.dest_type 		where cd.is_show='1' ";
                    //$time = date("Y-m-d"); // 依据现阶段时间迭代
                    $time = time();
                    ...
                    /* $line_sql = "SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert a
                s expert_id,e.small_photo,e.nickname   	
                      FROM
                      cfgm_hot_line AS cx
                      RIGHT JOIN
...
] = $this->db-> query( $roll_pic_sql)->result_array();
...
//                $line_hot_sql = $line_sql . " order by cx.id desc limit {$from},{$page_size}";
                    ...
                    FR OM cfgm_hot_line AS cx LEFT JOIN u_line AS l ON cx.line_id=l.id 
                    where  l.status='2' and cx.startplaceid = {$cityid} and cx.is_show=1 and cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime) >= {$time} order by cx.showorder desc 
                        union
                    (select id as id, line_classify as dest_type, mainpic as pic, linetitle, linename, satisfyscore, bookcount, peoplecount, comment_count, lineprice, saveprice, recommend_expert as expert_id 
						 FROM u_line where status = 2 order by displayorder desc) limit {$page}, {$page_size}"; // 加入cx.status=1
...
//                $line_guonei_sql = $line_sql . " and cx.dest_type=2 order by cx.showorder limit {$from},{$page_size}";
                    ...
                    LEFT JOIN u_line AS l ON cx.line_id=l.id where cx.dest_type = 2 AND l.status = 2 AND l.producttype=0 AND l.line_kind = 1 AND l.startcity={$cityid}
                        AND cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime) >= {$time} order by l.displayorder DESC, l.bookcount DESC)
                    union
                    ...
                    left join u_destinations AS ud ON ud.id = uld.dest_id where ud.id = 2 and ul.status = 2 AND ul.producttype = 0 AND ul.line_kind = 1 order by ul.displayorder DESC, ul.bookcount DESC) limit {$from}, {$page_size}";
...
//                $line_chujing_sql = $line_sql . " and cx.dest_type=1 order by cx.showorder limit {$from},{$page_size}";
                    ...
                    FROM  	cfgm_hot_line AS cx 
                    LEFT JOIN u_line AS l ON cx.line_id=l.id where cx.dest_type = 1 AND l.status = 2 AND l.producttype=0 AND l.line_kind = 1 AND  l.startcity={$cityid} 
                            AND cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime) >= {$time} order by l.displayorder DESC, l.bookcount DESC)
                    union
                    (select ul.id as id, ul.line_classify as dest_type, ul.mainpic as pic, ul.linetitle as linetitle, ul.linename as linename, ul.satisfyscore, ul.bookcount,ul.peoplecount, ul.comment_count, ul.lineprice,ul.saveprice,ul.recommend_expert as expert_id
                    from u_line AS ul 
                    left join u_line_dest AS uld ON ul.id = uld.line_id
                    left join u_destinations AS ud ON ud.id = uld.dest_id where ud.id = 1 and ul.status = 2 AND ul.producttype = 0 AND ul.line_kind = 1 order by ul.displayorder DESC, ul.bookcount DESC, ul.addtime DESC) limit {$from}, {$page_size}";        // 添加addtime desc
...
//                $line_zhoubian_sql = "SELECT "
                    ...
                    FROM  	cfgm_hot_line AS cx 
                    LEFT JOIN u_line AS l ON cx.line_id=l.id where cx.dest_type = 3 AND l.status = 2 AND l.producttype=0 AND l.line_kind = 1 AND l.startcity={$cityid} 
                         AND cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime) >= {$time} order by l.displayorder DESC, l.bookcount DESC)
                    union
                    (select ul.id as id, ul.line_classify as dest_type, ul.mainpic as pic, ul.linetitle as linetitle, ul.linename as linename, ul.satisfyscore, ul.bookcount,ul.peoplecount, ul.comment_count, ul.lineprice,ul.saveprice,ul.recommend_expert as expert_id
                    from u_line AS ul 
...
//                                u_expert AS e JOIN u_expert_collect AS c on c.expert_id = e.id and c.member_id = {$member_id} limit {$from},{$per_page}"; // 改为按成交量将序排列
                    ...
                    /**
                     * 辅助方法
                     * @method 生成u_destinations中id与kindname的对应关系数组
                     * @return u_destinations中id与kindname的对应关系数组
                     */
                    private function generate_id_name_array() {
                    $sql = "SELECT id, kindname FROM u_destinations";
