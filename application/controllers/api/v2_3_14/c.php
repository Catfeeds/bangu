...
//        print_r(debug_backtrace());
    ...
    public function cfgm_home() {
        ...
        // 由于14版本已经打包成apk上线,所以用别名方式修复大图与小图颠倒的问题,m_bgpic字段以弃用,数据库中该字段值已清空
        $this->db->select("e.id AS eid,e.talk,e.big_photo as small_photo,e.small_photo as big_photo, e.m_bgpic,e.nickname as realname,a.name,e.praise_num,e.avg_score,(CASE WHEN grade=1 THEN '管家' WHEN grade=2 THEN '初级' WHEN grade=3 THEN '中级' WHEN grade=4 THEN '高级' END) AS grade");
    ...
    public function cfgm_home_search() {
        ...
        $expert_sql = "select
			    ...
			    from
					    u_expert
		        where
						is_commit = 1 AND status=2 and city={$cityid} and nickname like '%{$content}%' order by addtime desc limit 4";
...
//            $list = $this->cfgm_hot_dest_model->hot_dest_list(array('dest_type' => '3', 'startplaceid' => $cityid), $from, $page_size);
    ...
    public function cfgm_line_detail() {
        ...
        //分享链接
        $ouput['shareurl'] = "http://m.1b1u.com/line/line_share?lineid=" . $l_id . '&userid='.$m_id;
        //返回数据结果
        $this->__data($ouput);
    }
...
//						status=2 and nickname like '%{$content}%' order by addtime desc limit 5";
        $expert_sql = "SELECT e.id, e.nickname, e.big_photo, e.small_photo, e.m_bgpic FROM cfgm_hot_expert AS h JOIN u_expert AS e on h.expert_id = e.id where e.is_commit = 1 AND e.nickname like '%{$content}%' and e.city={$cityid} ORDER BY h.showorder DESC, e.order_count DESC limit 5";
...
//        }
    ...
    public function cfgm_expert_list_new() {
        ...
        if ($type < 0 or $page < 0 or $per_page < 0 or empty($city_id)) {
...
//                                    FROM cfgm_hot_expert AS h LEFT JOIN u_expert AS e ON h.expert_id = e.id limit {$from},{$per_page}";   // 改为按成交量降序排列,取前20条
                ...
                $recommend_sql = "SELECT e.id, e.big_photo, e.small_photo, e.m_bgpic, e.realname, e.nickname, e.login_name, e.satisfaction_rate, e.expert_dest, e.people_count, e.attention_count 
                                    FROM cfgm_hot_expert AS h LEFT JOIN u_expert AS e ON h.expert_id = e.id WHERE e.big_photo <>'' and e.big_photo is not null AND e.small_photo IS NOT NULL AND e.small_photo <> '' AND e.is_commit = 1 AND e.status = 2 AND e.city = $city_id ORDER BY h.showorder DESC, e.people_count DESC  LIMIT {$from},{$per_page}";
...
//                                FROM u_expert AS e limit {$from},{$per_page}";        // 改为按成交量降序排列
                $all_sql = "SELECT e.id, e.big_photo, e.small_photo, e.m_bgpic, e.realname, e.nickname, e.login_name, e.satisfaction_rate, e.expert_dest, e.people_count, e.attention_count 
                                FROM u_expert AS e WHERE e.big_photo <>'' and e.big_photo is not null AND e.small_photo IS NOT NULL AND e.small_photo <> '' AND e.is_commit = 1 AND e.status = 2 AND e.city = $city_id ORDER BY e.people_count DESC limit {$from},{$per_page}";
