	...
	public function E_expert_info() 
	{
		...
		$sql = "
			SELECT 
		                ...
		                (select GROUP_CONCAT(kindname SEPARATOR '、') from u_destinations where FIND_IN_SET(id,e.expert_dest) >0 )as expert_dest
		    FROM		
						u_expert AS e 
...
// 		$e_id="242";
	...
	{
// 		file_put_contents('bridge_test.log', json_encode($_POST),FILE_APPEND);
		//1、传值
		$token = $this->input->post ( 'number', true );
		...
		$data['small_photo']=$big_photo;
// 		file_put_contents('bridge2_test.log', json_encode($data),FILE_APPEND);
		$one=$this->db->query("select * from bridge_expert_map where expert_id='{$e_id}' and status=0")->num_rows();
	...
	public function E_customize_grab() {
		...
		$sql="
			select * from (select
				   ...
				   (select GROUP_CONCAT(kindname) from u_destinations where FIND_IN_SET(id,c.endplace)>0) as endplace_CH
		    from
					u_customize as c
	...
	public function E_customize_list() {
		...
		(select GROUP_CONCAT(kindname) from u_destinations where FIND_IN_SET(id,c.endplace)>0 and level=3) as endplace_CH,
	...
	public function E_customize_detail() 
	{
		...
		$sql = "
				SELECT 
						...
						(select GROUP_CONCAT(kindname) from u_destinations where FIND_IN_SET(id,c.endplace) >0 )as endplace,c.days,c.people,
...
/**
	...
	public function E_customize_data()
	{
		...
		$sql = "select c.id as c_id,c.trip_way,c.other_service,
				...
				(select GROUP_CONCAT(kindname) from u_destinations where FIND_IN_SET(id,c.endplace) >0 )as endplace
		...
		if(!empty($line_res)){
			...
			foreach ($overcity as $key => $value) {
				...
				if($is_exist_res[0]['isexist']<=0){
					$get_dest_pid_sql = 'SELECT pid FROM u_destinations WHERE id='.$value;
	...
	public function E_expert_register_p2()
	{
		...
		$big_photo = $this->input->post ( 'bj_pic', true ); // 背景
		...
		if (empty ( $big_photo )) 			{$this->__errormsg ( '请上传背景' );}
...
//		}
	...
	public function E_expert_register() {
		...
		$nickname = isset($_REQUEST['nickname'])?$_REQUEST['nickname']:''; // 用户昵称
                if (empty($nickname)){
                    $this->__errormsg('昵称不能为空');
                }
                // 昵称是否包含英文字母
                if (preg_match('/[a-zA-Z]/',$nickname)){
                    $this->__errormsg('昵称中有字母');
                } 
                // 昵称是否已存在
                $sql_nickname = "SELECT id FROM u_expert WHERE nickname = '{$nickname}'";
                if ($this->db->query($sql_nickname)->num_rows() > 0){
                    $this->__errormsg('该昵称已经存在');
                }
		$sex = isset($_REQUEST['sex'])?$_REQUEST['sex']:''; // 性别
		$small_photo = isset($_REQUEST['icon_photo'])?$_REQUEST['icon_photo']:''; // 头像
		$big_photo = isset($_REQUEST['bj_pic'])?$_REQUEST['bj_pic']:''; // 背景
		file_put_contents('get_big_photo.log', $big_photo,FILE_APPEND);
		...
		if(!empty($big_photo )){ $big_photo= $big_photo ;}else{$big_photo='';}
		...
		file_put_contents('touch_big_photo.log', $big_photo,FILE_APPEND);
		...
		file_put_contents('insert_data.log', json_encode($data),FILE_APPEND);
	...
	public function E_update_expert_register()
	{
		...
		(select GROUP_CONCAT(d.kindname) from u_destinations as d where FIND_IN_SET(d.id,e.expert_dest)) as expert_dest_name,e.expert_dest,
	...
	*/
	...
	public function E_update_expert_deal()
	{
		...
		$nickname = isset($_REQUEST['nickname'])?$_REQUEST['nickname']:''; // 用户昵称
                ...
                // 昵称是否已存在
                $sql_nickname = "SELECT id FROM u_expert WHERE nickname = '{$nickname}' AND id <> {$m_id}";
                if ($this->db->query($sql_nickname)->num_rows() > 0){
                    $this->__errormsg('该昵称已经存在');
                }
		$sex = isset($_REQUEST['sex'])?$_REQUEST['sex']:''; // 性别
		...
		if($phone)
		{
                    ...
                    // 魏勇编辑
                    $sql = "SELECT id,email,mobile,status FROM u_expert WHERE (mobile='{$phone}' OR login_name='{$phone}') AND status!=3 AND status <> -1 ORDER BY id DESC";
                    $row = $this->db->query($sql)->result_array();
		...
		if($email)
		{
                    ...
                    // 魏勇编辑
                    $sql = "SELECT id,email,mobile,status FROM u_expert WHERE (email='{$email}' OR login_name='{$email}') and status!=3 AND status <> -1 ORDER BY id DESC";
                    $row = $this->db->query($sql)->result_array();
