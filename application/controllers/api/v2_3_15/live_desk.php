...
*/				
    ...
    public function go_into_room() {
		...
		$this->live_db->query("update live_room set `live_status`=1,`starttime`={$nowtime},`user_do_start_log`=1,`user_do_start_time`={$nowtime} where room_id = ".$roomid."");		
        $returnData = array();
        $this->__outlivemsg($returnData);
    }
    ...
    public function sign_out_room() {
		...
		$this->live_db->query("update live_room set `live_status`=0,`endtime`={$nowtime},`user_do_tui_log`=1,`user_do_tui_time`={$nowtime} where room_id = ".$roomid."");	
        $returnData = array();
        $this->__outlivemsg($returnData);
    }
...
CREATE TABLE `live_anchor_like` (
  ...
  KEY `index1` (`anchor_id`)
	...
	*/
    ...
    //临时用来合并某些视频
	public function tmp_set_new_video(){
		header( "content-type:text/html;charset=utf-8" );
		$roomid = $this->input->get('roomid', true); //直播房间id
		$is = 0;
		$nowtime = time();
		...
		$param = array(
			"roomId"=>$roomid,// 主播室id
			"endRecordingTime"=>$nowtime*1000,// 结束时间戳*1000=1480499880000
			"startRecordingTime"=>(($nowtime*1000)-(24*60*60*3*1000)),// 开播时间戳*1000=1478080680000							
		);
		$returnData1 = self::curlUtils($param,"GetUploadVideo");
		$returnData2 = array();
		if(isset($returnData1['entities']) && isset($returnData1['entities'][0] ) ){
			$startDate = $returnData1['entities'][0]['videoCreateTime'];
			$one = substr($startDate,0,-3);
			$tow = substr($startDate,-3);			
			$videoDuration = $returnData1['entities'][0]['videoDuration'];
			$endDate = ($one + $videoDuration).$tow;
			//通过接口获取录播列表数据
			$param = array(
				"roomId"=>$roomid,// 主播室id
				//"timeError"=>60, //误差时间范围，单位秒，默认为:(endDate-startDate)/10/1000
				"startDate"=> $startDate, // 开始录制时间, 单位：毫秒
				"endDate"=> $endDate, //结束录制时间, 单位：毫秒
				"ingoreTime"=>1,
			);
			$returnData2 = self::curlUtils($param,"SetRecordingTime");					
		}
		if(!empty($returnData2) && $returnData2['status']==200){
			$is =1;
		}	
		if($is == 1){
			echo '<font color=red>合并成功</font>';						
		}else{
			echo '<font color=red>合并失败或者已经合并过了</font>';
			echo '<br/>';		
			echo '第三方（亲加）接口放回数据：';
			echo '<br/>';		
			echo '录播列表接口GetUploadVideo：';			
			print_r($returnData1);
			echo '<br/>';		
			echo '合成接口SetRecordingTime：';			
			print_r($returnData2);			
		}
		exit;
	}	
