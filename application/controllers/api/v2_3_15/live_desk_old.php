	...
	*/
	private function  insert_live_anchor($user_id,$user_type){
		...
		if($user_id>0 && $user_type==0){ //用户注册主播
			...
			$data['name']=$user['nickname']; //真实姓名
			$data['photo']=$user['big_photo']; //头像图片
    ...
    public function recommend_video_room() {
		...
		$sql_str = '(select anchor_id,room_id,0 as video_id, room_name as name,0 as read_num,peoples as watching_num,pic as cover,starttime as addtime,peoples as peopleby,3 as type from live_room  where live_status>0 and status=1  and  createtime>'.($nowtime - $this->room_timeout).' )union (select anchor_id,room_id,id as video_id,name,people as read_num,0 as watching_num,pic as cover,addtime,people as peopleby,type from live_video where video<>"" ) order by peopleby desc,addtime desc '.$sql_page;
    ...
    public function attention_video_room() {
        ...
        $from = ($page - 1) * $page_size; //from
        $sql_page = " LIMIT {$from},{$page_size}";
		$sql_str = 'select lr.* from live_anchor_fans as laf left join ((select anchor_id,room_id,0 as video_id, room_name as name,0 as read_num,peoples as watching_num,pic as cover,starttime as addtime,peoples as peopleby,3 as type from live_room  where live_status>0 and status=1  and  createtime>'.($nowtime - $this->room_timeout).' )union (select anchor_id,room_id,id as video_id,name,people as read_num,0 as watching_num,pic as cover,addtime,people as peopleby,type from live_video where  video<>"" )) as lr on(laf.anchor_id=lr.anchor_id) where laf.status=1 and laf.user_id='.$this->user_id.' order by lr.addtime desc,lr.peopleby desc '.$sql_page;
		$live_room_data = $this->live_db->query($sql_str)->result_array();			
		$num_live_room_data = count($live_room_data);
		$anchor_ids = array();
		if($num_live_room_data>0){
    ...
    public function search_video_room() {
		...
		$sql_str = '(select anchor_id,room_id,0 as video_id, room_name as name,0 as read_num,peoples as watching_num,pic as cover,starttime as addtime,peoples as peopleby,3 as type from live_room  where live_status>0 and status=1 '.$where1.'  and  createtime>'.($nowtime - $this->room_timeout).' )union (select anchor_id,room_id,id as video_id,name,people as read_num,0 as watching_num,pic as cover,addtime,people as peopleby,type from live_video where video<>"" '.$where2.' ) order by peopleby desc,addtime desc '.$sql_page;
		$live_room_data = $this->live_db->query($sql_str)->result_array();			
		$num_live_room_data = count($live_room_data);
		$anchor_ids = array();
		if($num_live_room_data>0){
...
*/
		...
		*/
		...
		if($live_anchor_data['room_id']>0){//表示有专属房间
			...
			if(!empty($live_room_data) && $live_room_data['status']==1 && $live_room_data['type']==2 && ( $live_room_data['live_status']>0 || $live_room_data['createtime']>($nowtime - $this->room_timeout) ) ){//专属房间正在使用中
				if($live_room_data['anchor_id']==$live_anchor_data['anchor_id']){//被自己使用中
					$returnData = array(
				...
				$sql = 'SELECT id FROM live_video WHERE anchor_id='.$live_anchor_data['anchor_id'].' and room_id='.$live_room_data['room_id'].' and room_code="'.$live_room_data['room_code'].'"   limit 1';
				$live_video_data =  $this->live_db->query($sql )->row_array();				
				$this->live_db->trans_begin();//事务		
				//查询是否有空闲的房间然分配空间
				$sql = 'SELECT * FROM live_room WHERE status=1  and ( live_status=0 or  createtime<'.($nowtime - $this->room_timeout).' ) and room_id= '.$live_anchor_data['room_id'].' limit 1 FOR UPDATE';
				$live_room_data =  $this->live_db->query($sql )->row_array();		
				if(!empty($live_room_data)){//有可以使用的房间
					$this->live_db->query("update live_room set `anchor_id`=".$live_anchor_data['anchor_id'].",`user_type`=".$live_anchor_data['type'].",`attr_id`={$categoryid},`line_ids`={$lineid},`room_code`='{$room_code}',`room_name`='{$roomname}',`pic`='{$cover}',`room_dest_id`={$destid},`createtime`={$nowtime},`starttime`=0,`peoples`=0,`umoney`=0,`live_status`=3,`endtime`=0 where room_id = ".$live_room_data['room_id']."");//	
		...
		if($iszhuang==1){//没有专属房间,使用公用房间分配
			...
			$this->live_db->trans_begin();//事务		
			//查询是否有空闲的房间然分配空间
			$sql = 'SELECT * FROM live_room WHERE  type=1 and status=1 and  (createtime<'.($nowtime - $this->room_timeout).' or  live_status=0 )limit 1 FOR UPDATE';
			$live_room_data =  $this->live_db->query($sql )->row_array();		
			if(!empty($live_room_data)){//有可以使用的房间
				$this->live_db->query("update live_room set `anchor_id`=".$live_anchor_data['anchor_id'].",`user_type`=".$live_anchor_data['type'].",`attr_id`={$categoryid},`line_ids`={$lineid},`room_code`='{$room_code}',`room_name`='{$roomname}',`pic`='{$cover}',`room_dest_id`={$destid},`createtime`={$nowtime},`starttime`=0,`peoples`=0,`umoney`=0,`live_status`=3,`endtime`=0 where room_id = ".$live_room_data['room_id']."");//	
				$sql = 'SELECT id FROM live_video WHERE anchor_id='.$live_anchor_data['anchor_id'].' and room_id='.$live_room_data['room_id'].' and room_code="'.$live_room_data['room_code'].'"  limit 1';
    ...
    public function get_current_room_chat() {
		...
		$sql_str = "select nickname,content,addtime from live_room_chat where room_id=".$roomid." and room_code='".$roomcode."' and addtime>'".date("Y-m-d H:i:s",$currenttime)."' limit 20 ";
    ...
    public function get_current_giving_gifts() {
		...
		$sql_str = "select gift_id,worth,addtime from live_gift_record where room_id=".$roomid." and room_code='".$roomcode."' and addtime>'".date("Y-m-d H:i:s",$currenttime)."' limit 20 ";
    ...
    public function sign_out_room() {
		...
		if( !($live_room_data['status']==1 && $live_room_data['type']==2 && ( $live_room_data['live_status']>0 || $live_room_data['createtime']>($nowtime - $this->room_timeout) ) )){//该房间已经过期
			$returnData = array();	
			$this->__outlivemsg($returnData,"该房间已经过期",'3003');						
		}			
		$this->live_db->query("update live_room set `live_status`=0,`endtime`={$nowtime} where room_id = ".$roomid."");	
...
*/		
	...
	public function timed_task_get_new_video(){
		$nowtime = time();
		//把live_room中没有插入到live_video中的数据插入进去
		$sql = 'SELECT * FROM live_room WHERE status=1 and anchor_id>0 and starttime>0 ';
		$live_room_datas =  $this->live_db->query($sql )->result_array();	
		foreach($live_room_datas as $live_room_data){
			$sql = 'SELECT id FROM live_video WHERE  room_id='.$live_room_data['room_id'].' and room_code="'.$live_room_data['room_code'].'"  limit 1';
			$live_video_data =  $this->live_db->query($sql )->row_array();			
			if(empty($live_video_data)){//检查是否已经插入过了
				$old_attr_id =$live_room_data['attr_id'];				
				$endtime = ($live_room_data['endtime']?$live_room_data['endtime']:($live_room_data['starttime']+$this->room_timeout));
				$pic = $live_room_data['pic'];					
				$insert_data = array(
						"anchor_id"	=>$live_room_data['anchor_id'],
		...
		//获取最大的开始时间
		/*$sql = 'SELECT starttime FROM live_video WHERE where type=1 and  order by starttime desc limit 1';
		$live_video_data =  $this->live_db->query($sql )->row_array();
		$max_starttime = 0;
		if(!empty($live_video_data)){
			$max_starttime = $live_video_data['starttime'];
		}*/
		//通过接口获取录播列表数据
		$total = 1000;
		for($i=1;$i<$total;$i++){
			...
			if(!empty($returnData['entities'])){
				foreach($returnData['entities'] as $v){
					if($v['status']==2){//正常的视频
						...
						echo '['.date("Y-m-d H:i:s",$recStartTime).'到'.date("Y-m-d H:i:s",$recEndTime).']';
						$sql = 'SELECT room_id FROM live_room WHERE roomid='.$v['roomId'].'';
						$live_room_data =  $this->live_db->query($sql )->row_array();
						if(!empty($live_room_data)){
							$sql = 'SELECT id,record_id FROM live_video WHERE  starttime<='.$recStartTime.' and endtime>'.$recStartTime.' and room_id='.$live_room_data['room_id'].'  limit 1';
							$live_video_data =  $this->live_db->query($sql )->row_array();							
							if(!empty($live_video_data) && $live_video_data['record_id'] != $v['recordingId'] && empty($live_video_data['record_id'])){
								$this->live_db->query("update live_video set `video`='".$v['firstHlsUrl']."',`starttime`='".$recStartTime."',`endtime`='".$recEndTime."',`pic`='".$v['recFirstFrameUrl']."',`record_id`='".$v['recordingId']."' where id = ".$live_video_data['id']."");
							}							
						}
					}
				}				
