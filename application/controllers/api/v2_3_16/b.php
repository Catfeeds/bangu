...
class B extends APP_Controller {
	...
	public function E_expert_login() 
	{
		...
		if ($res_query) 
		{
			...
			if (md5 ( $password ) == $res_query ['password'] && ($res_query ['status'] == "2" || $res_query ['status'] == "5"  )) 
			{
...
// 		$e_id="242";
	...
	public function E_update_info_deal()
	{
		...
		// 昵称是否包含英文字母
		if (preg_match('/[a-zA-Z]/',$nickname)) { $this->__errormsg('昵称中有字母');}
		...
		$bridge_expert_data= $this->db->query ( "select nickname from bridge_expert where nickname='{$nickname}' and status=1")->row_array();
		$bridge_expert_rows=count($bridge_expert_data);  //管家审核状态的昵称
		$expert_rows=count($expert_arr);
		if (($expert_rows>0&& $expert_arr_my['nickname']!=$nickname) || ($bridge_expert_rows>0) ) {$this->__errormsg ( '昵称已被占用，请重新输入' );}
		...
		unset($expert['id']);
		unset($expert['like_num']); //不插入管家点赞数
		$data=$expert;
...
/**
	...
	public function E_expert_register_p2()
	{
		...
		$querys = $this->db->query ( "select nickname from bridge_expert where nickname='{$nickname}' and status=1" );
		$bridge_expert_rows= $querys->num_rows();
	
		if (empty ( $nickname )) 				{$this->__errormsg ( '请填写昵称' );}
		// 昵称是否包含英文字母
		if (preg_match('/[a-zA-Z]/',$nickname)) { $this->__errormsg('昵称中有字母');}
		if (($expert_rows>0) || ($bridge_expert_rows>0)) 		  {$this->__errormsg ( '昵称已被占用，请重新输入' );}
...
//		}
	...
	public function E_expert_register() {
		...
		$nickname = isset($_REQUEST['nickname'])?$_REQUEST['nickname']:''; // 用户昵称
                ...
                $querys = $this->db->query ( "select nickname from bridge_expert where nickname='{$nickname}' and status=1" );
                $bridge_expert_rows= $querys->num_rows();
                if (($this->db->query($sql_nickname)->num_rows() > 0) || ($bridge_expert_rows>0)){
	...
	*/
	...
	public function E_update_expert_deal()
	{
		...
		if (preg_match('/[a-zA-Z]/',$nickname)) {$this->__errormsg('昵称中有字母');}
		// 昵称是否已存在
		$bridge_expert_row = "SELECT nickname FROM bridge_expert WHERE nickname = '{$nickname}' AND status=1";
		$rows=$this->db->query($bridge_expert_row)->num_rows();
		if (($expert_rows>0&&$expert_arr_my['nickname']!=$nickname) || ($rows>0)) {$this->__wapmsg ( '昵称已被占用，请重新输入',$callback );}
