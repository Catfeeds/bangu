...
class C extends APP_Controller {
    ...
    private $m_url_root = 'http://m.1b1u.com'; //测试环境及生产环境域名替换
    ...
    public function __construct() {
		...
		*/
        //$this->output->enable_profiler(TRUE);   // add for test
        //file_put_contents('./trace.txt', __METHOD__);
		$http_type = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') || (isset($_SERVER['HTTP_X_FORWARDED_PROTO']) && $_SERVER['HTTP_X_FORWARDED_PROTO'] == 'https')) ? 'https://' : 'http://';
		if(strpos($_SERVER['HTTP_HOST'],'www.1b1u.com')===0){
			$this->m_url_root = $http_type.'m.1b1u.com';
			$this->pic_web_url = $http_type.'www.1b1u.com';
		}else if(strpos($_SERVER['HTTP_HOST'],'t.w.1b1u.com')===0){
			$this->m_url_root = $http_type.'t.m.1b1u.com';
			$this->pic_web_url = $http_type.'t.w.1b1u.com';			
		}else {
			$this->m_url_root = $http_type.'pubtest.m.1b1u.com';
			$this->pic_web_url = $http_type.'pubtest.1b1u.com';
		}
    }
...
//        print_r(debug_backtrace());
        // file_put_contents('./trace.txt', '析构方法');
    }
    ...
    public function cfgm_home() {
        $cityid = $this->input->get_post('cityid', true); //城市id
        ...
        $used_for = $this->input->post('used_for', TRUE);  // 为7表示直播首页,为8表示直播管家页
        //$cityid = "235";
        if (!$cityid) {
            $this->__errormsg('city is null');
        }
        $reDataArr = array();
        $roll_pic_sql = "select id,name,pic,showorder from cfgm_index_roll_pic where is_show=1 AND kind = 7 order by showorder";    // kind为7表示直播首页轮播图
        $reDataArr ['roll_pic'] = $this->db->query($roll_pic_sql)->result_array();
        if (empty($reDataArr['roll_pic'])){
            $this->__nullmsg();
        }
        ...
        // 以管家表中的数据为准
        //$this->db->select("e.id AS eid,e.talk,cfg.pic AS big_photo, cfg.smallpic AS small_photo, e.big_photo AS b_photo,e.small_photo AS s_photo, cfg.pic as m_bgpic,e.nickname as realname,a.name,e.praise_num,e.avg_score,(CASE WHEN grade=1 THEN '管家' WHEN grade=2 THEN '初级' WHEN grade=3 THEN '中级' WHEN grade=4 THEN '高级' END) AS grade");
        $this->db->select("e.id AS eid,e.talk,e.big_photo,e.small_photo, e.m_bgpic,e.nickname as realname,a.name,e.praise_num,e.avg_score,(CASE WHEN grade=1 THEN '管家' WHEN grade=2 THEN '初级' WHEN grade=3 THEN '中级' WHEN grade=4 THEN '高级' END) AS grade");
        ...
        $this->db->order_by("cfg.showorder", "desc");
        $query = $this->db->get();
		$expert_data = $query->result_array();
		if(!empty($expert_data)){
			foreach($expert_data as $k => $v){
				$expert_data[$k]['rand'] = 5;//rand(1,5);
			}			
		}
        $reDataArr ['expert'] = $expert_data;
        // 处理图片begin
//        foreach ($reDataArr['expert'] as &$v) {
//            if (empty($v['big_photo'])) {
//                $v['big_photo'] = $v['b_photo'];
//            }
//            if (empty($v['small_photo'])) {
//                $v['small_photo'] = $v['s_photo'];
//            }
//        }
        // 处理图片end
        $hot_dest_sql = "SELECT * FROM (SELECT
				cd.dest_id as id,cd.dest_type,ud.kindname as destname,cd.name as description,cd.pic as pic,(SELECT IFNULL((SELECT all_line_num from u_dest_line_num WHERE dest_id = ud.id and startplace_id = {$cityid}), 0)) AS num
				from
				cfgm_hot_dest as cd
                                left join u_dest_cfg as ud on ud.id=cd.dest_id
				where cd.startplaceid = {$cityid} AND
				cd.is_show='1' and cd.status=1 
                                order by cd.showorder desc, num desc) h WHERE num > 0  limit 6";
...
//        $hot_dest_sql = $dest_sql . " order by cd.showorder desc, cd.linenum desc limit {$from}, {$page_size}";     // 改为按配置和线路数排序
...
//        $time = date("Y-m-d"); // 依据现阶段时间迭代
        ...
        $line_sql = "SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id,e.small_photo,e.nickname   	FROM  	cfgm_hot_line AS cx RIGHT JOIN u_line AS l ON cx.line_id=l.id     	left join cfg_index_kind as cik on cik.id=cx.dest_type left join u_expert as e on e.id=l.recommend_expert  	where e.is_commit = 1 AND l.status='2' and cx.is_show=1 and cx.starttime<='{$time}' and cx.endtime>='{$time}'";
...
//		$reDataArr ['line'] ['zb'] = $this->db->query ( $zb_line_sql )->result_array ();
        ...
        // 391 全国出发  OR (ls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid}))
        $hot_line_sql = "SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id
                    FROM cfgm_hot_line AS cx LEFT JOIN u_line AS l ON cx.line_id=l.id left join u_line_startplace as uls ON uls.line_id = l.id
                    where l.status='2' and cx.startplaceid ={$cityid} and (uls.startplace_id = {$cityid} OR (uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391) and cx.is_show=1 and cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime)+86400 > {$time} order by cx.showorder desc limit 6 ";
        $reDataArr ['line'] = $this->db->query($hot_line_sql)->result_array();
        // if need to this plase take on
        // $this->output->cache(5);
        $arr_live = $this->home_video_room();
		if(!empty($arr_live)){
			foreach($arr_live as $k => $v){
				$arr_live[$k]['rand'] = 5;//rand(1,5);
			}			
		}		
        $reDataArr['info'] = $arr_live;
        $arr_live = $this->home_video_room2();
        $reDataArr['videoinfo'] = $arr_live;		
    ...
    public function cfgm_get_city() {
        $lat = $this->input->get_post('lat', true); //维度
        $lng = $this->input->get_post('lng', true); //经度
		...
		if($type==1){
			$arr = $this->db->query("select * from u_dest_cfg where kindname like '%" . $city . "%'")->row_array();
			/*if(empty($arr)){
				$arr = $this->db->query("select * from u_area where name like '%" . $city . "%'")->row_array();
			}*/
			$id = 0;
			if(!empty($arr))$id = $arr['id'];
			$returnData = array('cityid' => $id, 'cityname' => $city);			
		}else{
			$arr = $this->db->query("select * from u_area where name like '%" . $city . "%'")->row_array();
    ...
    public function cfgm_home_search() {
        ...
        $cityid = $this->input->get_post("cityid", true); //当前城市
        if (empty($cityid)) {
        ...
        $expert_sql = "select
			    ...
			    from
					    u_expert
		        where
						is_commit = 1 AND status=2 and city={$cityid} and nickname like '%{$content}%' order by addtime desc limit 100";
     ...
     *
     */
    public function cfgm_expert_backgroup_pic() {
		$data['type'] = array(
			array('id'=>'1','name'=>'风景'),
			array('id'=>'2','name'=>'魔幻'),
			array('id'=>'3','name'=>'卡通'),
		...
		$data['data'] = array(
			array(
				'type'=>'1',
				'name'=>'封面1',
				'bgpic'=>'/file/expertbgpic/1.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/1.png',
			),
			array(
				'type'=>'1',
				'name'=>'封面2',
				'bgpic'=>'/file/expertbgpic/2.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/2.png',
			),
			array(
				'type'=>'1',
				'name'=>'封面3',
				'bgpic'=>'/file/expertbgpic/3.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/3.png',
			),
			array(
				'type'=>'2',
				'name'=>'封面4',
				'bgpic'=>'/file/expertbgpic/4.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/4.png',
			),
			array(
				'type'=>'2',
				'name'=>'封面5',
				'bgpic'=>'/file/expertbgpic/5.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/5.png',
			),
			array(
				'type'=>'2',
				'name'=>'封面6',
				'bgpic'=>'/file/expertbgpic/6.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/6.png',
			),
			array(
				'type'=>'3',
				'name'=>'封面7',
				'bgpic'=>'/file/expertbgpic/7.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/7.png',
			),
			array(
				'type'=>'3',
				'name'=>'封面8',
				'bgpic'=>'/file/expertbgpic/8.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/8.png',
			),
			array(
				'type'=>'3',
				'name'=>'封面9',
				'bgpic'=>'/file/expertbgpic/9.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/9.png',
			),
			array(
				'type'=>'4',
				'name'=>'封面10',
				'bgpic'=>'/file/expertbgpic/10.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/10.png',
			),
			array(
				'type'=>'4',
				'name'=>'封面11',
				'bgpic'=>'/file/expertbgpic/11.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/11.png',
			),
			array(
				'type'=>'4',
				'name'=>'封面12',
				'bgpic'=>'/file/expertbgpic/12.png',
				'webpic'=>$this->pic_web_url.'/file/expertbgpic/12.png',
			),			
		);
        $this->__outmsg($data);	
	}
     ...
     *
     */
    public function cfgm_edit_expert_backgroup_pic() {	
        $bgpic = $this->input->post('bgpic', true);
		...
		$this->db->query("update u_expert set `big_photo`='".$bgpic."' where id = ".$m_id."");
		$this->__outmsg(array());
	}
    ...
    public function cfgm_expert_detail() {
        ...
        $view = intval(isset($_REQUEST['view']) ? $_REQUEST['view'] : '0');//模板
        $token = $this->input->get_post('number', true);
		$usertype = $this->input->get_post('usertype', true);
		...
		$result_expert_affiliated=$this->db->query('select * from u_expert_affiliated where expert_id='.$e_id)->row_array();
		if(!empty($result_expert_affiliated)){
			$expert_detail['attention_count']=$expert_detail['attention_count']+$result_expert_affiliated['collect_num_vr'];
		}		
		
		
        $expert_detail['zx_link'] = CHAT_URL . "/m_chat.html?mid=&eid=" . $e_id;

		$expert_detail['rand'] = 5;//rand(1,5);
        ...
        foreach ($line_list as $key => $val) {
			...
			if($val['startplace'] !='全国出发'){
				$val['startplace'] = str_replace("市","",$val['startplace']).'出发';
			}			
            $line_list [$key] = $val;
        }
        ...
        // 5、定制列表              将323行ue.m_bgpic改为ue.big_photo  2016-11-16 11:57:33
        $sql2 = "
				select 
							...
							uca.isuse as isuse, ua.name as area_name,ud.kindname as dest_name, ue.nickname as nickname,ue.big_photo as uem_m_bgpic 	 
				from
							u_customize as uc 
				...
				where 
							ue.is_commit = 1 AND uc.status='3' and uca.isuse='1' and ISNULL(uca.replytime)=0 	 and uca.expert_id={$e_id} 
				order by 
							uc.addtime desc";
        ...
        // 6、评价
        $sql4 = " select 
							uc.id,uc.like_num,uc.addtime,uc.expert_content,uc.reply1,uc.reply2,uc.score5,uc.score6,
		...
		$sql_str = "select anchor_id from live_anchor where user_id=".$e_id." ";
		$live_anchor_data_one = $this->live_db->query($sql_str)->row_array();
		if(!empty($live_anchor_data_one)){
			$anchor_id = $live_anchor_data_one['anchor_id'];
			$nowtime = time();
			$sql_str = '(select anchor_id,room_id,0 as video_id, room_name as name,0 as read_num,peoples as watching_num,pic as cover,starttime as addtime,peoples as peopleby,3 as type,room_dest as dest_name,room_dest_id as dest_id,1 as isroom,status,sort from live_room  where live_status=1 and status in(1,2)  and  createtime>'.($nowtime - $this->room_timeout).' and anchor_id='.$anchor_id.' )union (select anchor_id,room_id,id as video_id,name,people as read_num,0 as watching_num,pic as cover,addtime,people as peopleby,type,dest_name,dest_id,0 as isroom,status,sort from live_video where video<>"" and status in(1,3)  and anchor_id='.$anchor_id.' ) order by isroom desc,status desc,sort desc,watching_num desc,peopleby desc,addtime desc LIMIT 10';
		...
		if($view==1){
			$share_url= $this->m_url_root.'/line/expert_share?view=1&expertid='.$e_id.'&userid='.$userid;			
		}else{
			$share_url= $this->m_url_root.'/line/expert_share?expertid='.$e_id.'&userid='.$userid;
		}
/*		
		$share_name ='您需要的不是攻略，而是这个人';	
		$share_pic =trim(base_url(''),'/').$expert_detail['small_photo'];
		$share_content ='帮游1+1，'.$expert_detail['nickname'].'一对一为您服务哦';
		*/
		/*if($userid){
			$sql = 'SELECT * FROM u_expert WHERE id= '.$userid;
			$u_expert_info =  $this->db->query($sql )->row_array();				
			$share_name ='您需要的不是攻略，而是我。'.$u_expert_info['nickname'].'给您拜年了！';	
			$share_pic =trim(base_url(''),'/').$u_expert_info['small_photo'];
			$share_content ='帮游1+1，'.$u_expert_info['nickname'].'为您一对一服务哦，完美旅程需要设计';			
		}else{*/
			$share_name ='新的一年对自己好点儿！新年新旅行，推荐TA给您！';	
			$share_pic =trim(base_url(''),'/').$expert_detail['small_photo'];
			$share_content ='帮游1+1，'.$expert_detail['nickname'].'为您一对一服务哦，完美旅程需要设计';				
		//}	
        //9、返回结果
        $arr = array(
            ...
            'live_room_list' => $live_room_data,			
            'expert_info' => $expert_info,
            'condition' => $condition,
            'sc' => $sc,
            'is_praise' => $is_praise,
            'lianjie' => "http://www.1b1u.com/guanjia/" . $e_id . '.html',
			'share_url'=> $share_url,
    ...
    public function cfgm_startplace() {
        ...
        if ($all_citys) {
            foreach ($all_citys as $key => $val) {
				if($val ['name']=='重庆市'){
					$val ['spell'] = 'chongqingshi'; // 全拼
					$szm_py [$key] = 'C'; // 首字母拼音大写						
				}else{
...
//        $this->__outmsg($output);
    ...
    function upload() {
        ...
        //上传图片路径
        $path = dirname(BASEPATH) . '/file/c/img/';
        $input_name = $filename;
    ...
    public function cfgm_line_list() {
        ...
        file_put_contents('a.txt', '首页搜索测试');
        //1、传值
        $cityid = $this->input->get_post('cityid', true);   // 定位的城市id
        //$cityid = "235";
        ...
        // 魏勇编辑 将默认排序改为人气*20% + 成交量*50% + 满意度*30% 降序
        $order_by = " ORDER BY l.collectnum * 0.2 + l.satisfyscore * 0.3 + l.peoplecount * 0.5 DESC ";
        $result = "";
...
//                $where.="  and FIND_IN_SET({$cityid},ls.startplace_id)>0 "; //城市出发
        ...
        foreach ($data ['line_list'] as $key => $val) {
			...
			if($val['startplace'] !='全国出发'){
				$data ['line_list'] [$key]['startplace1'] = str_replace("市","",$val['startplace']).'出发';
			}else{
				$data ['line_list'] [$key]['startplace1'] = '全国出发';
			}
        }
        //16、返回结果
...
//            $list = array_merge($list, $list2);
            $sql = "(SELECT * FROM (select
				cd.dest_id, ud.kindname, cd.name as description,cd.pic as pic,(SELECT IFNULL((SELECT all_line_num from u_dest_line_num WHERE dest_id = ud.id and startplace_id = {$cityid}), 0)) AS line_num
				from
				cfgm_hot_dest as cd
                                left join u_dest_cfg as ud on ud.id=cd.dest_id
				where cd.startplaceid = {$cityid} AND
				cd.is_show='1' and cd.status=1 
				order by cd.showorder desc, line_num desc) AS hot WHERE line_num > 0)
                    UNION
            (SELECT * FROM (SELECT ud.id as dest_id, ud.kindname, ud.description as description,  ud.pic as pic, (SELECT IFNULL((SELECT all_line_num from u_dest_line_num WHERE dest_id = ud.id and startplace_id = {$cityid}), 0)) AS line_num
                        FROM u_dest_cfg AS ud 
                        LEFT JOIN u_line_dest AS uld ON ud.id = uld.dest_id 
                        LEFT JOIN u_line AS ul ON uld.line_id = ul.id 
                        WHERE ul.startcity = {$cityid} AND ud.id not in (select dest_id from cfgm_hot_dest) and ud.level = 3 AND ud.isopen = 1 
                        ORDER BY line_num DESC) AS common WHERE line_num > 0) limit {$from},{$page_size}";
            $list = $this->db->query($sql)->result_array();
//            file_put_contents('zd.txt', $this->db->last_query());
...
//            file_put_contents('/tmp/mysql.txt', "select * from cfgm_index_roll_pic where kind=4 and status=1".'----------------1236----------',FILE_APPEND);
            ...
            $sql = "(SELECT * 
                            FROM (
                            SELECT distinct cd.dest_id, ud.kindname, cd.name as description, cd.pic, (
                            SELECT IFNULL((
                            SELECT all_line_num 
                            FROM u_dest_line_num 
                            WHERE dest_id = ud.id and startplace_id = {$cityid}), 0)) AS line_num
                            ...
                            AND cd.is_show='1' and cd.status=1 AND cd.dest_type = 2 ORDER BY cd.showorder desc, line_num desc) AS hot where line_num > 0)
                            UNION
                            (SELECT *
                            FROM (
                            SELECT DISTINCT  ud.id AS dest_id, ud.kindname, ud.description, ud.pic, (
                            SELECT IFNULL((
                            SELECT all_line_num
                            FROM u_dest_line_num
                            WHERE dest_id = ud.id AND startplace_id = {$cityid} ), 0)) AS line_num
...
//            $list = $this->cfgm_hot_dest_model->hot_dest_list(array('dest_type' => '1', 'startplaceid' => $cityid), $from, $page_size);
            $sql = "(SELECT * 
                            ...
                            FROM cfgm_hot_dest as cd left join u_dest_cfg as ud on ud.id=cd.dest_id
                            WHERE (cd.startplaceid = {$cityid} OR (cd.startplaceid in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or cd.startplaceid = 391) 
                            AND cd.is_show='1' and cd.status=1 AND cd.dest_type = 1 ORDER BY cd.showorder desc, line_num desc) AS hot where line_num > 0)
                            UNION
                            (SELECT *
                            FROM (
                            SELECT DISTINCT  ud.id AS dest_id, ud.kindname, ud.description, ud.pic, (
...
//            $list = $this->cfgm_hot_dest_model->hot_dest_list(array('dest_type' => '3', 'startplaceid' => $cityid), $from, $page_size);
            $sql = "SELECT * FROM (SELECT DISTINCT d.id AS dest_id, d.kindname, d.description, d.pic, (SELECT IFNULL((
                            SELECT all_line_num 
                            FROM u_dest_line_num 
                            WHERE dest_id = d.id and startplace_id = {$cityid}), 0)) AS line_num
                            ...
                            AND t.isopen=1 AND d.level = 3 ORDER BY d.displayorder DESC) AS alias WHERE line_num > 0 ORDER BY line_num DESC LIMIT {$from},{$page_size}";
            $list = $this->db->query($sql)->result_array();
        }
    ...
    public function cfgm_expert_list() {
      ...
      WHERE
      e.is_commit = 1 AND e.status=2 and e.is_kf='N' {$where}
      group by
      e.id {$order_by}
      ";
    ...
    /**
     * @method cfgm_champ_expert_list
     * @desc    冠军管家列表
     * @author  魏勇
     * @time    2017-1-13 09:13:42
     */
    public function cfgm_champ_expert_list() {
        $champ_sql = "SELECT e.id, e.big_photo, e.small_photo, e.realname, e.nickname, e.login_name, e.satisfaction_rate, e.expert_dest, e.people_count, e.attention_count 
                                    FROM cfgm_champ_expert AS h LEFT JOIN u_expert AS e ON h.expert_id = e.id WHERE e.big_photo <>'' and e.big_photo is not null AND e.small_photo IS NOT NULL AND e.small_photo <> '' AND e.is_commit = 1 AND e.status = 2 ORDER BY h.showorder DESC, e.people_count DESC LIMIT 3";
        $output = $this->db->query($champ_sql)->result_array();
        $this->__outmsg($output);
    }
    ...
    public function cfgm_line_detail() {
        $l_id = intval($this->input->get_post('lineid', true));
        $token = $this->input->post('number', true);
        ...
        $ouput = $this->F_line_detail_more($l_id,1); //线路详情

		$result_line_affiliated=$this->db->query('select * from u_line_affiliated where line_id='.$l_id)->row_array();
		if(!empty($result_line_affiliated)){
			$ouput['data']['collectnum']=$ouput['data']['collectnum']+$result_line_affiliated['collect_num_vr'];
		}		
		//线路评论
		$ouput['comment_list'] = $this->db->query ( "SELECT m.litpic, m.nickname, c.id as commentid,c.level, c.content,c.reply1,c.reply2,c.pictures, c.addtime, c.isanonymous,c.like_num, m.mobile FROM u_comment AS c	LEFT JOIN  u_member as m ON c.memberid=m.mid	WHERE c.line_id ='{$l_id}' order by c.addtime desc LIMIT 100" )->result_array ();
		foreach ($ouput['comment_list'] as $n=>$m)
		{
			$pic_arr=explode(",", $m['pictures']);
			$pic_arr=$this->__doImage($pic_arr);
			$m['pictures']=$pic_arr;
			$ouput['comment_list'][$n]=$m;
		}		
        ...
        //分享到空间、微博时的来链接
        $ouput['lianjie'] = "http://www.1b1u.com/line/" . $l_id . '.html';
        //分享链接
        $ouput['shareurl'] = $this->m_url_root."/line/line_share?lineid=" . $l_id . '&userid='.$m_id;
		$u_expert_info = array();	
		if($m_id){//判断是管家
			$sql = 'SELECT * FROM u_expert WHERE id= '.$m_id;
			$u_expert_info =  $this->db->query($sql )->row_array();					
		}	
/*		
		if(!empty($u_expert_info)){	
			$ouput['share_title']=$u_expert_info['nickname']."发布了新线路，价格低过了50%的同行";
			$ouput['share_describe']=$ouput['data']['linename'];
			$ouput['share_pic']=trim(base_url(''),'/').$u_expert_info['small_photo'];		
		}else{ 
			$ouput['share_title']='宇宙级性价比旅游线路，是朋友我才推荐/分享给好朋友，一般人我都不告诉';
			$ouput['share_describe']=$ouput['data']['linename'];
			//$ouput['share_pic']=trim(base_url(''),'/').$ouput['data']['mainpic'];
			$ouput['share_pic']='http://www.1b1u.com/file/bijini_pic.jpg';
		}
*/	
		if(!empty($u_expert_info)){	
			$ouput['share_title']=$u_expert_info['nickname']."祝您新春大吉！刚刚发布了一个宇宙级性价比新品！";
			$ouput['share_describe']=$ouput['data']['linename'].'，价格低过50%同行，一般人我都不告诉';
			$ouput['share_pic']=trim(base_url(''),'/').$u_expert_info['small_photo'];		
		}else{ 
			$ouput['share_title']=$ouput['data']['brand'].'给您拜年了！分享一个超高性价比旅游线路给您！';
			$ouput['share_describe']=$ouput['data']['linename'].'，价格低过50%同行，一般人我都不告诉';
			//$ouput['share_pic']=trim(base_url(''),'/').$ouput['data']['mainpic'];
			$ouput['share_pic']='http://www.1b1u.com/file/bijini_pic.jpg';
		}
    ...
    public function cfgm_line_suit() {
		...
		if(empty($l_id)){
			$l_id = $this->input->get('suitid', true); //线路id
		}
        //$l_id="918";
        ...
        // 过滤掉过期的套餐
        //$reDataArr = $this->db->query("SELECT distinct id,suitname FROM u_line_suit ls LEFT JOIN u_line_suit_price lsp ON ls.lineid = lsp.lineid where ls.lineid = {$l_id} and lsp.day <= date(now()) ")->result_array();
        $reDataArr = $this->db->query("SELECT distinct id,suitname FROM u_line_suit ls LEFT JOIN u_line_suit_price lsp ON ls.lineid = lsp.lineid where ls.lineid = {$l_id} and lsp.day >= date(now()) ")->result_array();
        if (empty($reDataArr)) {
            $this->__errormsg();
        }
    ...
    public function cfgm_price_date() {
		...
		if(empty($suit_id)){
			$suit_id = $this->input->get('suitid', true); //套餐id
		}		
        //$suit_id="956";	
    ...
    public function cfgm_suit_line_detail() {
        ...
        //8、管家
        $sql = "select e.id AS expert_id,e.nickname,e.small_photo,CASE WHEN la.grade=1 THEN '管家' WHEN la.grade=2 THEN '初级专家' WHEN la.grade=3 THEN '中级专家' WHEN la.grade=4 THEN '高级专家' end grade  from u_expert AS e left join u_line_apply AS la on e.id=la.expert_id where e.is_commit = 1 AND e.status=2 and la.status=2 and e.is_kf='N' and la.line_id={$l_id}";
        $query = $this->db->query($sql);
...
// 		$reDataArr ['vol_old'] = $query2->result_array (); // 优惠卷已过期
     ...
     * 
     * */
    private function user_register($mobile, $password, $channel, $type = "1") {
        $this->db->trans_begin();
        //注册
...
/*		
		...
		if(!empty($wx_flow_activity_member_data)){
			$wx_flow_activity_member_data = $wx_flow_activity_member_data['num'];
		}						
		if(!empty($wx_flow_activity_member_data) && $wx_flow_activity_member_data<20000){//激活人数小于20k的可以参与活动
			//不用判断用户是否已经领取
			$wx_flow_activity_member_data = array();
			$flow_activity_uid = 0;//表示没有推荐人
			$time1 = time();
			$data1 = array(
				'flownums'=>0,
				'addtime' => $time1,
			...
			$data2 = array(
				...
				'member_get_flow' => 0,
				'member_get_money' => 0,
				'to_member_get_flow' => 50,										
			);										
			$this->db->insert ( 'wx_flow_activity_member', $data1 );
			$this->db->insert ( 'wx_flow_activity_rec', $data2 );																	
		}
*/
        //发送短信
        if ($type == "1") {
    ...
    public function cfgm_user_register() {
        ...
        if (($code_mobile == $mobile) && ($code_number == $code)) {
            ...
            if (empty($result)) {
                $status = $this->user_register($mobile, $password, $channel);
                if ($status['status'])
                    $this->__data('注册成功');
                else
    ...
    public function cfgm_user_login() {
        ...
        if ($result) {
            if (md5($password) == $result [0] ['pwd']) {
				...
				if($result[0]['mid']){
					$uid = $result[0]['mid'];
					$str_sql = "select id,status,utype from wx_flow_activity_member where member_id=".$uid;
					$wx_flow_activity_member_data = $this->db->query($str_sql)->row_array();
					if(!empty($wx_flow_activity_member_data) && $wx_flow_activity_member_data['utype']==2 && $wx_flow_activity_member_data['status']==0){
						...
						$wx_flow_activity_rec_data = $this->db->query($str_sql)->row_array();
						if(!empty($wx_flow_activity_rec_data)){
							...
							$this->db->query("update wx_flow_activity_member set status=1 where member_id = {$uid}");//自己激活状态更改
							...
							if($member_id>0){
								...
								if(!empty($wx_flow_activity_member_data)){
									if($wx_flow_activity_member_data['utype']==1){//判断为直推人
										$this->db->query("update wx_flow_activity_member set `money`=`money`+{$member_get_money} where member_id = {$member_id}");//推荐人增加钱																
									}							
								}						
							}					
						}						
				...
				//判断没登录过就加积分
				if (!$result[0]['if_login'])
				{
					$integral_config=$this->config->item('integral_config');
					$up_arr=array(
							'logintime'=>date('Y-m-d H:i:s',$login_time),//登录时间，后期会废弃该字段
							'login_time'=>$login_time,
							'if_login'=>1  //是否登录过，1为是，0为否
					);
					$content='登录获得';
					$this->db->trans_begin();//事务开启
					//更新用户积分及记录积分日志
					$this->record_integration($result [0] ['mid'],$integral_config['login'],$content,1,$login_time,1,$up_arr);
					$this->db->trans_complete(); //事务结束
					//事务
					if ($this->db->trans_status() === FALSE)
					{
						$this->db->trans_rollback();
						$this->__errormsg('登录异常,请重新登录');
					}
					else
					{
						$this->db->trans_commit();
						$lastData['point']=$integral_config['login'];
						$this->__successmsg($lastData);
					}
				}	
				$this->__successmsg($lastData);
            }else {
...
//6（2）、下订单
    ...
    public function cfgm_walk_count() {
        ...
        // 将第2870行,status=1改为status=0,2871行mb_od.status=1 or mb_od.status=4改为mb_od.status=0 or mb_od.status=4,2873行添加条件AND mo.status = -3 AND mo.ispay = 3
        $sql = "SELECT 
					(SELECT count(*) FROM u_member_order AS mb_od LEFT JOIN u_member AS mb ON mb_od.memberid = mb.mid WHERE mb.mid ={$m_id} AND ispay=0 AND (mb_od.status=0) AND mb_od.ispay=0 AND TIMESTAMPDIFF(HOUR,mb_od.addtime,NOW())<24) AS wait_pay,        
					(SELECT count(*) FROM u_member_order AS mb_od LEFT JOIN u_member AS mb ON mb_od.memberid = mb.mid WHERE mb.mid ={$m_id} AND (mb_od.status=0 or mb_od.status=4) AND ispay>0) AS wait_walk,
					...
					(SELECT count(*) FROM u_refund AS r LEFT JOIN u_member_order AS mo ON r.order_id=mo.id WHERE r.refund_type=0 AND r.status=0 AND r.refund_id={$m_id} AND mo.status = -3 AND mo.ispay = 3) AS wait_tk 
				FROM 
        ...
        // 加入判断是否为主播
        $this->live_db = $this->load->database("live", TRUE);
        $sql_is_anchor = "SELECT la.user_type FROM live_anchor AS la WHERE la.status = 2 AND la.user_id = {$m_id}";
        $query_anchor = $this->live_db->query($sql_is_anchor);
        if ($query_anchor->num_rows() > 0){
            $reDataArr['is_anchor'] = 1;
        }else{
            $reDataArr['is_anchor'] = 0;
        }
...
//        $type="2";
        ...
        if ($type == '1') {
            $sql = "SELECT 
							lc.line_id,l.linename,l.lineday,l.lineprice,l.mainpic ,l.collectnum, l.bookcount,l.satisfyscore
					FROM 
...
//							    ec.addtime desc limit {$from},{$page_size}       
//					";
            $sql = "SELECT e.id, e.big_photo, e.small_photo, e.m_bgpic, e.realname, e.nickname, e.login_name, e.satisfaction_rate, e.expert_dest, e.people_count, e.attention_count 
                                    FROM
                                u_expert AS e JOIN u_expert_collect AS c on c.expert_id = e.id and c.member_id = {$m_id} WHERE e.status = 2  ORDER BY c.addtime DESC limit {$from},{$page_size}";
            $reDataArr = $this->db->query($sql)->result_array();
            // 管家擅长目的地处理
    ...
    public function cfgm_custoimze_detail() {
        ...
        $data ['answer'] = $this->db->query("  
				...
				WHERE 
						e.is_commit = 1 AND r.customize_id ={$cid} ")->result_array();
        $this->__outmsg($data);
    }
    ...
    public function cfgm_custoimze_answer_detail() {
        ...
        $answer ['expert'] = array();
        if (!empty($expert_id)) {
            $answer ['expert'] = $this->db->query("select   realname,mobile,weixin,nickname,id from u_expert  where is_commit = 1 AND id={$expert_id} ")->row_array();
        }
    ...
    public function cfgm_user_order() {
        ...
        $code = $this->input->post('code', true);
        if ($code == 1) { // 待支付
            $where = "AND ( mb_od.status=0 ) AND ispay=0 AND TIMESTAMPDIFF(HOUR,mb_od.addtime,NOW())<24";   // 将mb_od.status=1改为mb_od.status==0
        } elseif ($code == 2) { // 待出行
            $where = "AND ( mb_od.status=0 OR mb_od.status=4 ) AND ispay>0";                                // 将mb_od.status=1改为mb_od.status==0
        } elseif ($code == 3) { // 待评价
    ...
    public function cfgm_user_order_detail() {
        ...
        // 判断是国内订单还是国外订单   魏勇添加
        $overcity = isset($arr['order']['overcity']) ? $arr['order']['overcity'] : 0;
        $arr_city = explode(',', $overcity);
        if (in_array(1, $arr_city)){
            $arr['order']['chujing'] = 1;
        }else if (in_array(2, $arr_city)){
            $arr['order']['chujing'] = 0;
        }
        // 游客信息列表
        ...
        $arr ['expert'] = $this->db->query("		SELECT id, mobile, big_photo, small_photo, nickname  	FROM (`u_expert`)  	WHERE is_commit = 1 AND `id` =  {$arr['order']['expert_id']}   	ORDER BY `id` ")->row_array();
    ...
    public function cfgm_offline_pay() {
        ...
        // 写入订单支付详情图片表   魏勇添加  解决已收款管理中找不到流水单扫描件的问题
        $detai_id = $this->db->insert_id();
        $pic_data['detail_id'] = $detai_id;
        $pic_data['pic'] = $receipt_img;
        $this->db->insert('u_order_detail_pic', $pic_data);
    ...
    public function cfgm_update_info() {
        ...
        if (!empty($data['nickname']) && !empty($data['email']) && !empty($data['truename']) && !empty($data['sex']) && !empty($data['address']) && !empty($data['postcode'])) {
            //完善资料增加积分
            $str_sql = "select id from u_points_log where ptype=1 and member_id=" . $mid;
    ...
    /**
     * @name：得到某线下的“更多管家”
     * @author: 温文斌  魏勇编辑    显示所有热门管家，不管是否申请该线路,删除了与u_line_apply表的关联
     * @param: number=凭证；data
     * @return:
     *
     */
    public function cfgm_more_expert() {
        ...
        $sql = "(SELECT * FROM (SELECT 
						...
						e.attention_count,e.sex,e.online,e.expert_dest as expert_destid, e.expert_theme, e.satisfaction_rate, 
				...
				FROM 	
                                                cfgm_hot_expert AS che
						LEFT JOIN u_expert AS e ON e.id = che.expert_id
						LEFT JOIN u_line_apply AS la ON e.id = la.expert_id 
						left join u_area as ua on ua.id=e.city  
						left join u_expert_grade as eg on e.grade=eg.grade
				WHERE 	
						e.is_commit = 1 AND la.line_id = {$l_id} AND la.STATUS = 2 AND e.city = {$cityid} AND e.`status`=2 and e.is_kf='N'   {$where}  ) AS hot limit 25) UNION
                (SELECT * FROM (SELECT 
				 		e.id AS expert_id,e.realname,e.nickname, e.big_photo,e.small_photo,e.comment_count,
						e.attention_count,e.sex,e.online,e.expert_dest as expert_destid, e.expert_theme, e.satisfaction_rate, 
						e.people_count, ua.name as cityname, e.title as grade,	    
				...
				FROM 	
						...
						left join u_expert_grade as eg on e.grade=eg.grade
				WHERE 	
						e.is_commit = 1 AND la.line_id = {$l_id} AND la.STATUS = 2 AND e.city = {$cityid} AND e.`status`=2 and e.is_kf='N'   {$where}  ) AS common limit 25)";
        $reDataArr = $this->db->query($sql)->result_array();
        foreach ($reDataArr as $key => $val) {
            foreach ($val as $k => $v) {
                if ($k == "satisfaction_rate") {
    ...
    public function cfgm_expert_custiome_list() {
        ...
        $sql = "   
				...
				from
							...
							left join u_dest_cfg as ud on ud.id=uc.endplace 	
				where 
							ue.is_commit = 1 AND uc.status='3' and uca.isuse='1' and ISNULL(uca.replytime)=0 and uca.expert_id={$eid} 
				order by 
							uc.addtime desc
	...
	//用户不登录可以管家点赞
    public function cfgm_expert_like_do() {
        $likeid = $this->input->get_post('likeid', true); //点赞目标
        $liketype = $this->input->get_post('liketype', true); //点赞对象类型,1管家点赞,2管家评论点赞,3线路点赞		
		if(empty($likeid) || empty($liketype)){
			$this->__errormsg('参数错误!');			
		}
		$num =1;
		$like_data = array();
		if($liketype==1){//1管家点赞
			$this->db->query("update u_expert set `like_num`=like_num+1 where id = ".$likeid."");
			$sql_str = "select like_num from u_expert where id=".$likeid;
			$like_data = $this->db->query($sql_str)->row_array();
		}else if($liketype==2){//2管家评论点赞
			$this->db->query("update u_comment set `like_num`=like_num+1 where id = ".$likeid."");
			$sql_str = "select like_num from u_comment where id=".$likeid;
			$like_data = $this->db->query($sql_str)->row_array();
		}else if($liketype==3){//3线路点赞
			$this->db->query("update u_line set `like_num`=like_num+1 where id = ".$likeid."");
			$sql_str = "select like_num from u_line where id=".$likeid;
			$like_data = $this->db->query($sql_str)->row_array();			
		}
		if(!empty($like_data)){
	...
	//用户登录后管家点赞
    public function cfgm_expert_like_do_my() {
        $likeid = $this->input->get_post('likeid', true); //点赞目标
        $liketype = $this->input->get_post('liketype', true); //点赞对象类型,1管家点赞,2管家评论点赞,3线路点赞
        $token = $this->input->get_post('number', true);		
		if(empty($likeid) || empty($liketype) || empty($token)){
			$this->__errormsg('参数错误!');			
		}
        //登录用户
        $m_id = $this->F_get_mid($token);		
		if(empty($m_id)){//用户没有登录
			$this->__errormsg('您还没有登录');			
		}
		$returnData = array();
		$num =1;
		$sql_str = "select * from u_expert_like where expert_id=".$m_id." and like_id=".$likeid.' and like_type='.$liketype;
		$u_expert_like_data = $this->db->query($sql_str)->row_array();
		if(!empty($u_expert_like_data)){
			/*if($u_expert_like_data['status']==1){//取消点赞
				$this->db->query("update u_expert_like set `status`=0 where id = ".$u_expert_like_data['id']."");				
				$returnData = array(
					'code'=>'0',
				);					
			}else{
				$this->db->query("update u_expert_like set `status`=1 where id = ".$u_expert_like_data['id']."");				
				$returnData = array(
					'code'=>'1',
				);				
			}*/
			...
			$this->db->insert('u_expert_like', $insert_data);
			if($liketype==1){//1管家点赞
				$this->db->query("update u_expert set `like_num`=like_num+1 where id = ".$likeid."");
				$sql_str = "select like_num from u_expert where id=".$likeid;
				$like_data = $this->db->query($sql_str)->row_array();				
    ...
    public function cfgm_expert_travels_list() {
        ...
        $query = $this->db->query($sql);
        $data ['line_list_total'] = $query->num_rows();
        $sql = " SELECT nickname from u_expert where is_commit = 1 AND id={$eid} ORDER BY id desc  limit 1  ";
        $query = $this->db->query($sql);
    ...
    public function cfgm_expert_travel_note_detail() {
        ...
        $reDataArr ['expert'] = $this->db->query(
				...
				FROM 		
							...
							LEFT JOIN u_expert_grade AS eg ON e.grade=eg.grade 
				WHERE e.is_commit = 1 AND tn.id={$note_id}  
				"
                )->row_array();
     ...
     * @author: 张允发
     * @since 2017.2.6
     * @param: mid=凭证；scid=线路ID
     * @return:
     *
     */
    public function cfgm_sc_line_wap() {
    	$status = 0;
    	$m_id = intval(isset($_REQUEST['mid']) ? $_REQUEST['mid'] : '0');
    	$callback = empty($_REQUEST["callback"]) ? '' : $_REQUEST['callback'];
		if (empty($m_id)){
			$token = $this->input->post('number', true);
			$this->check_token($token);
		...
		if (!$m_id)
			$this->__wapmsg('参数缺失', $callback);
    	//$m_id="291";
    	$sc['line_id'] =intval(isset($_REQUEST['scid']) ? $_REQUEST['scid'] : '0');
    	if (!$sc['line_id'])
    		$this->__wapmsg('参数缺失', $callback);
    	if ($sc['line_id']) {
    		$query = $this->db->query("select line_id from u_line_collect where line_id={$sc['line_id']} and member_id={$m_id}");
...
//							d.isopen=1  order by h.showorder desc, h.linenum DESC limit 11";  // 将by id改为by displayorder
        //$dest_sql = "SELECT * FROM cfgm_hot_dest WHERE startplaceid = $cityid OR startplaceid in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = $cityid) OR startplaceid = 391 ORDER BY showorder DESC, linenum DESC LIMIT 17";
        $dest_sql = "SELECT dest_id id, name FROM cfgm_hot_dest WHERE is_show = 1 AND startplaceid = $cityid OR startplaceid in (SELECT startplace_child_id FROM u_startplace_child WHERE is_show = 1 AND startplace_id = $cityid) OR startplaceid = 391 ORDER BY showorder DESC, linenum DESC LIMIT 17";
        $dest_query = $this->db->query($dest_sql);
        $out['dest'] = $dest_query->result_array();
        //2、管家
//        $expert_sql = "select
...
//						status=2 and nickname like '%{$content}%' order by addtime desc limit 5";
        $expert_sql = "SELECT e.id, e.nickname, e.big_photo, e.small_photo, e.m_bgpic FROM cfgm_hot_expert AS h JOIN u_expert AS e on h.expert_id = e.id where e.is_commit = 1 AND e.nickname like '%{$content}%' and e.city={$cityid} ORDER BY h.showorder DESC, e.order_count DESC limit 9";    // limit 5 改为 limit 9
    ...
    public function line_home_search() {
        ...
        //$time = date("Y-m-d"); // 依据现阶段时间迭代
        $time = time();
        $line_sql = "SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id,e.small_photo,e.nickname   	FROM  	cfgm_hot_line AS cx LEFT JOIN u_line AS l ON cx.line_id=l.id  left join cfg_index_kind as cik on cik.id=cx.dest_type left join u_expert as e on e.id=l.recommend_expert where e.is_commit = 1 AND l.status='2' and cx.startplaceid = $cityid and cx.is_show=1 and cx.starttime<='{$time}' and cx.endtime>='{$time}'";
        ...
        if (1 === $page) {
            $roll_pic_sql = "SELECT * FROM cfgm_index_roll_pic WHERE kind =2 AND status = 1 AND is_show = 1 ORDER BY showorder DESC"; // 增加按线路数倒序
...
//            $reDataArr['hot_dest'] = $this->db->query($hot_dest_sql)->result_array();
            switch ($type) {
                case 0:         // 精选
                    $hot_dest_sql = "(SELECT * FROM (select
				cd.startplaceid as start_id, cd.dest_id as id,cd.dest_type,ud.kindname as destname,cd.name as description,cd.pic as pic,(SELECT IFNULL((SELECT all_line_num from u_dest_line_num WHERE dest_id = ud.id and startplace_id = {$cityid}), 0)) AS line_num
				from
				cfgm_hot_dest as cd
                                left join u_dest_cfg as ud on ud.id=cd.dest_id
				where cd.startplaceid = {$cityid} AND
				...
				order by cd.showorder desc, line_num desc) AS hot WHERE line_num > 0)
                    UNION
            (SELECT * FROM (SELECT distinct ul.startcity as start_id, ul.overcity as id, 4 as dest_type, ud.kindname as destname, ud.description as description,  ud.pic as pic, (SELECT IFNULL((SELECT all_line_num from u_dest_line_num WHERE dest_id = ud.id and startplace_id = {$cityid}), 0)) AS line_num
                        FROM u_dest_cfg AS ud 
                        LEFT JOIN u_line_dest AS uld ON ud.id = uld.dest_id 
                        LEFT JOIN u_line AS ul ON uld.line_id = ul.id 
                        WHERE ul.startcity = {$cityid} AND ud.id not in (select dest_id from cfgm_hot_dest) and ud.level = 3 AND ud.isopen = 1 
                ...
                case 1:     // 国内
                    ...
                    $guonei_dest_sql = "(SELECT * 
                            FROM (
                            SELECT distinct cd.dest_id as id,cd.dest_type,ud.kindname as destname,cd.name as description,cd.pic as pic,(
                            SELECT IFNULL((
                            SELECT all_line_num 
                            FROM u_dest_line_num 
                            WHERE dest_id = ud.id and startplace_id = {$cityid}), 0)) AS line_num
                            ...
                            (SELECT *
                            FROM (
                            SELECT DISTINCT  ud.id AS id, 2 AS dest_type, ud.kindname AS destname, ud.description AS description, ud.pic AS pic, (
                            SELECT IFNULL((
                            SELECT all_line_num
                            FROM u_dest_line_num
                            WHERE dest_id = ud.id AND startplace_id = {$cityid} ), 0)) AS line_num
...
//                    $chujing_dest_sql = $dest_sql . " and cd.dest_type=1 and cd.startplaceid={$cityid} order by cd.showorder desc, ud.line_num DESC limit {$from},{$page_size}";
                    $chujing_dest_sql = "(SELECT * 
                            ...
                            (SELECT *
                            FROM (
                            SELECT DISTINCT  ud.id AS id, 1 AS dest_type, ud.kindname AS destname, ud.description AS description, ud.pic AS pic, (
                            SELECT IFNULL((
                            SELECT all_line_num
                            FROM u_dest_line_num
                            WHERE dest_id = ud.id AND startplace_id = {$cityid} ), 0)) AS line_num
                ...
                case 3:     // 周边
                    ...
                    $zhoubian_dest_sql = "SELECT * FROM (SELECT DISTINCT d.id, 3 AS dest_type, d.kindname AS destname, d.description, d.pic, (SELECT IFNULL((
                            SELECT all_line_num 
                            FROM u_dest_line_num 
                            WHERE dest_id = d.id and startplace_id = {$cityid}), 0)) AS line_num
                            FROM cfg_round_trip AS t
...
//                $line_hot_sql = $line_sql . " order by cx.id desc limit {$from},{$page_size}";
                ...
                $line_hot_sql = "(SELECT * FROM (SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id
                    FROM cfgm_hot_line AS cx LEFT JOIN u_line AS l ON cx.line_id=l.id   left join u_line_startplace as uls ON uls.line_id = l.id
                    where  l.status='2' and (uls.startplace_id = {$cityid} or (cx.startplaceid = {$cityid} AND uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391)
                    AND cx.is_show=1 and cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime)+86400 > {$time} order by cx.showorder desc ) as abc)
                        UNION
                    (SELECT * FROM (select distinct ul.id as id, ul.line_classify as dest_type, ul.mainpic as pic, ul.linetitle, ul.linename, ul.satisfyscore, ul.bookcount, ul.peoplecount, ul.comment_count, ul.lineprice, ul.saveprice, ul.recommend_expert as expert_id 
						 FROM u_line as ul
                                                 LEFT JOIN u_line_startplace as uls on ul.id = uls.line_id
                                                 where ul.status=2 and ul.producttype = 0 and ul.line_kind = 1 and (uls.startplace_id = {$cityid} OR (uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391)
                                                 order by displayorder desc) as def) limit {$from}, {$page_size}"; // 加入cx.status=1
...
//                $line_guonei_sql = $line_sql . " and cx.dest_type=2 order by cx.showorder limit {$from},{$page_size}";
                ...
                $line_guonei_sql = "(SELECT * FROM (SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id
                    FROM  	cfgm_hot_line AS cx 
                    LEFT JOIN u_line AS l ON cx.line_id=l.id LEFT JOIN u_line_startplace AS uls ON l.id = uls.line_id 
                    where cx.dest_type = 2 AND l.status = 2 AND l.producttype=0 AND l.line_kind = 1 AND (uls.startplace_id = {$cityid} or (cx.startplaceid = {$cityid} AND uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391) AND cx.is_show = 1
                        AND cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime)+86400 > {$time} order by l.displayorder DESC, l.bookcount DESC) AS abc)
                    UNION
                    (SELECT * FROM (select distinct ul.id as id, ul.line_classify as dest_type, ul.mainpic as pic, ul.linetitle as linetitle, ul.linename as linename, ul.satisfyscore, ul.bookcount,ul.peoplecount, ul.comment_count, ul.lineprice,ul.saveprice,ul.recommend_expert as expert_id
                    from u_line AS ul 
                    left join u_line_dest AS uld ON ul.id = uld.line_id
                    LEFT JOIN u_line_startplace as uls on ul.id = uls.line_id
                    left join u_dest_cfg AS ud ON ud.id = uld.dest_id where ud.id = 2 and ul.status = 2 AND ul.producttype = 0 AND ul.line_kind = 1 and (uls.startplace_id = {$cityid} OR (uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391) order by ul.displayorder DESC, ul.bookcount DESC) as def) limit {$from}, {$page_size}";
...
//                $line_chujing_sql = $line_sql . " and cx.dest_type=1 order by cx.showorder limit {$from},{$page_size}";
                ...
                $line_chujing_sql = "(SELECT * FROM (SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id
                    FROM  	cfgm_hot_line AS cx 
                    LEFT JOIN u_line AS l ON cx.line_id=l.id left join u_line_startplace as uls ON uls.line_id = l.id
                    where cx.dest_type = 1 AND l.status = 2 AND l.producttype=0 AND l.line_kind = 1 AND (uls.startplace_id = {$cityid} or (cx.startplaceid = {$cityid} AND uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391)
                    AND cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime)+86400 > {$time} and cx.is_show = 1 order by l.displayorder DESC, l.bookcount DESC) as abc)
                    UNION
                    (SELECT * FROM (select distinct ul.id as id, ul.line_classify as dest_type, ul.mainpic as pic, ul.linetitle as linetitle, ul.linename as linename, ul.satisfyscore, ul.bookcount,ul.peoplecount, ul.comment_count, ul.lineprice,ul.saveprice,ul.recommend_expert as expert_id
                    from u_line AS ul 
                    left join u_line_dest AS uld ON ul.id = uld.line_id
                    LEFT JOIN u_line_startplace as uls on ul.id = uls.line_id
                    left join u_dest_cfg AS ud ON ud.id = uld.dest_id where ud.id = 1 and ul.status = 2 AND ul.producttype = 0 AND ul.line_kind = 1 and (uls.startplace_id = {$cityid} OR (uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391) order by ul.displayorder DESC, ul.bookcount DESC, ul.addtime DESC) AS def) limit {$from}, {$page_size}";        // 添加addtime desc
...
//                $dest_zhoubian_sql = $dest_sql . ' and cd.dest_type=3 order by cd.id desc';
                $line_zhoubian_sql = "(SELECT * FROM (SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id
                    FROM  	cfgm_hot_line AS cx 
                    LEFT JOIN u_line AS l ON cx.line_id=l.id left join u_line_startplace as uls ON uls.line_id = l.id
                    where cx.dest_type = 3 AND l.status = 2 AND l.producttype=0 AND l.line_kind = 1 AND uls.startplace_id={$cityid} 
                         AND cx.status = 1 and UNIX_TIMESTAMP(cx.starttime) <= {$time} and UNIX_TIMESTAMP(cx.endtime)+86400 > {$time} and cx.is_show = 1 order by l.displayorder DESC, l.bookcount DESC) as abc)
                    union
                    (SELECT * FROM (select distinct ul.id as id, ul.line_classify as dest_type, ul.mainpic as pic, ul.linetitle as linetitle, ul.linename as linename, ul.satisfyscore, ul.bookcount,ul.peoplecount, ul.comment_count, ul.lineprice,ul.saveprice,ul.recommend_expert as expert_id
                    from u_line AS ul 
                    left join u_line_dest AS uld ON ul.id = uld.line_id
                    LEFT JOIN u_line_startplace as uls on ul.id = uls.line_id
                    left join cfg_round_trip AS crt ON crt.neighbor_id = uld.dest_id where crt.isopen = 1 AND ul.status = 2 AND ul.producttype = 0 AND ul.line_kind = 1 AND (uls.startplace_id={$cityid} OR (uls.startplace_id in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) or uls.startplace_id = 391) order by ul.displayorder desc, ul.bookcount DESC) as def) limit {$from}, {$page_size}";
...
//        }
     ...
     * 根据父类id获取获取目的地
     */
    public function get_destinations_bypid() {
        $page = intval($this->input->get_post("page"));//分页	
        $cityid = intval($this->input->get_post("cityid"));//	
        $first_dest = intval($this->input->get_post("first_dest"));//一级目的地
        $second_dest_id = intval($this->input->get_post('second_dest_id', true));  //二级目的地，0时表示热门
		$where ='';
		$pid = 0;
		...
		if($first_dest==3){
			//周边游
			$query_sql = "SELECT * FROM (SELECT DISTINCT d.id, d.kindname,d.pic,d.simplename,d.line_num
								FROM cfg_round_trip AS t
								LEFT JOIN u_dest_cfg AS d ON d.id=t.neighbor_id
								WHERE (t.startplaceid = {$cityid} OR (t.startplaceid in (SELECT startplace_child_id FROM u_startplace_child WHERE startplace_id = {$cityid})) OR t.startplaceid = 391)
								AND t.isopen=1 AND d.level = 3 and d.line_num>0 ORDER BY d.displayorder DESC) AS alias  ".$sql_page;       // 去掉limit 10  魏勇编辑
			$destArr=$this->db ->query($query_sql) ->result_array(); 
			$this->__outmsg($destArr);			
		}
		if($first_dest && $second_dest_id){
			$pid = $second_dest_id;
		}
		if($first_dest && $second_dest_id==0){
			$sql = 'select id,kindname,pid,pic,simplename,line_num from u_dest_cfg  where pid in(select id from u_dest_cfg  where pid='.$first_dest.')  and ishot=1 and line_num>0 '.$sql_page;
			$destArr = $this ->db ->query($sql) ->result_array();
		}else{
			$sql = 'select id,kindname,pid,pic,simplename,line_num from u_dest_cfg  where  line_num>0 and pid='.$pid.$sql_page;
			$destArr = $this ->db ->query($sql) ->result_array();			
		}
        $this->__outmsg($destArr);
    }		
    ...
    public function cfgm_expert_list_new() {
        ...
        $sort = intval($this->input->get_post('sort', true));
        $dest_id = intval($this->input->get_post('dest_id', true));		
        $type = empty($type) ? 1 : $type;
        $page = empty($page) ? 1 : $page;
        $per_page = empty($per_page) ? 6 : $per_page;
        $from = ($page - 1) * $per_page;
		$order = 'e.people_count DESC';
		if($sort==2){//按成交
			$order = 'e.people_count DESC';
		}else if($sort==3){//按人气
			$order = 'e.attention_count DESC';
		}else if($sort==4){//按满意度	
			$order = 'e.satisfaction_rate DESC';
		}
		if(empty($city_id))$city_id=$this->default_cityid;
		$where ='';
		if($dest_id){
			$where = ' and FIND_IN_SET('.$dest_id.', e.expert_dest) ';
		}else{
			$where = ' and e.city = '.$city_id.' ';			
		}
...
//                                    FROM cfgm_hot_expert AS h LEFT JOIN u_expert AS e ON h.expert_id = e.id limit {$from},{$per_page}";   // 改为按成交量降序排列,取前20条
                ...
                $recommend_sql = "SELECT e.id, e.big_photo, e.small_photo, e.m_bgpic, e.realname, e.nickname, e.login_name, e.satisfaction_rate, e.expert_dest, e.people_count, e.attention_count 
                                    FROM cfgm_hot_expert AS h LEFT JOIN u_expert AS e ON h.expert_id = e.id WHERE e.big_photo <>'' and e.big_photo is not null AND e.small_photo IS NOT NULL AND e.small_photo <> '' AND e.is_commit = 1 AND e.status = 2 AND e.city = $city_id ORDER BY e.attention_count * 0.2 + e.people_count * 0.5 + e.satisfaction_rate * 0.3 DESC LIMIT {$from},{$per_page}";
                $reDataArr['expert'] = $this->db->query($recommend_sql)->result_array();
...
//                                FROM u_expert AS e limit {$from},{$per_page}";        // 改为按成交量降序排列
                $all_sql = "SELECT e.id, e.big_photo, e.small_photo, e.m_bgpic, e.realname, e.nickname, e.login_name, e.satisfaction_rate, e.expert_dest, e.people_count, e.attention_count 
                                FROM u_expert AS e WHERE e.big_photo <>'' and e.big_photo is not null AND e.small_photo IS NOT NULL AND e.small_photo <> '' AND e.is_commit = 1 AND e.status = 2 {$where} ORDER BY {$order} limit {$from},{$per_page}";
                $reDataArr['expert'] = $this->db->query($all_sql)->result_array();
                break;
...
//                                u_expert AS e JOIN u_expert_collect AS c on c.expert_id = e.id and c.member_id = {$member_id} limit {$from},{$per_page}"; // 改为按成交量将序排列
                $collect_sql = "SELECT e.id, e.big_photo, e.small_photo, e.m_bgpic, e.realname, e.nickname, e.login_name, e.satisfaction_rate, e.expert_dest, e.people_count, e.attention_count 
                                    FROM
                                u_expert AS e JOIN u_expert_collect AS c on c.expert_id = e.id and c.member_id = {$member_id} WHERE e.big_photo <>'' and e.big_photo is not null AND e.small_photo IS NOT NULL AND e.small_photo <> '' AND e.is_commit = 1 AND e.status = 2 AND e.city = $city_id ORDER BY c.addtime DESC limit {$from},{$per_page}";
                $reDataArr['expert'] = $this->db->query($collect_sql)->result_array();
                break;
        ...
        if (!empty($reDataArr['expert'])) {
            foreach ($reDataArr['expert'] as $k => $v) {
                $reDataArr['expert'][$k]['rand'] = 5; //rand(1,5);
            }
        }

        foreach ($reDataArr['expert'] as &$val) {
            $val['expert_dest'] = $this->generate_chn_array($val['expert_dest']);
        }
        //满意度处理
        foreach ($reDataArr['expert'] as $key => &$val) {
            foreach ($val as $k => $v) {
                if ($k == "satisfaction_rate") {
                    if ($v) {
                        $val [$k] = round($v * 100);
            ...
            $reDataArr['expert'] [$key] = $val;
        }
        //满意度处理end        
        // 管家轮播图
        $sql_roll_pic = "SELECT id,name,pic,showorder from cfgm_index_roll_pic WHERE is_show=1 AND kind = 8 ORDER BY showorder";    // kind为8表示直播管家轮播图
        $reDataArr['roll_pic'] = $this->db->query($sql_roll_pic)->result_array();
        $this->__outmsg($reDataArr);
    }
     ...
     * @method 发布动态
     */
    public function issue_news() {
        $token = $this->input->post('number', true);
        $trend_con = $this->input->post('trend_con', true);     // 内容
        $pic_list = $this->input->post('pic_list', true);
        $local_city = $this->input->post('local_city', true);
        $m_id = $this->F_get_mid($token);
        if (empty($token)) {
            return $this->__errormsg('token is empty');
        }
        if (empty($trend_con)) {
            return $this->_errormsg('trend_con is empty');
        }
        if (empty($local_city)) {
            return $this->_errormsg('local_city is empty');
        }
        $t = time();
        $data = array(
            'expert_id' => $m_id,
        ...
        $id = $this->db->insert('u_expert_condition', $data);
        if (!empty($pic_list)) {
            $pic_arr = explode(',', $pic_list);
            foreach ($pic_arr as $v) {
                $data_pic = array(
                    'condition_id' => $id,
                    'pic' => $v
                );
                $this->db->insert('u_expert_condition_pic', $data_pic);
            }
        }
    }
    ...
    public function get_new_app_version() {
        ...
        // 改为从数据库中读取数据
        $arr = array();
        $sql = "SELECT version_name FROM app_version ORDER BY id DESC";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            $arr = array('version_name' => $row['version_name']);
        } else {
            $arr = $this->restore_app_version();
        }
        echo $callback . "(" . json_encode($arr) . ")";
    }

    /**
     * @desc 抓取app_version并存入数据库
     * @author 魏勇
     * @return array
     */
    private function restore_app_version() {
        $str = file_get_contents('http://sj.qq.com/myapp/detail.htm?apkName=com.r747223875.nfu');
        $preg = '/data-apkUrl="http:\/\/imtt(\S+)/';
        preg_match($preg, $str, $m);
        $version_name = substr(rtrim($m[0], '"'), 13);
        $arr = array('version_name' => $version_name, 'update_time' => time());
        $this->db->insert('app_version', $arr);
        return array('version_name' => $version_name);
    }

    /**
     * @desc查看合同
     * @param int $type 合同类型 为1表示境外,为2表示境内
     */
    public function check_contract() {
        $type = $this->input->get_post('insurance_type', true);
        $content = '';
        if (intval($type) === 1) {
            $content = 'travel_contract_abroad';
        } else if (intval($type) === 2) {
            $content = 'travel_contract_domestic';
        } else {
            $this->__errormsg('类型异常');
        }
        $sql = "SELECT {$content} FROM cfg_web";
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            $this->__outmsg(array('content' => $row[$content]));
        }
    }
     ...
     * @return string msg 提示信息
     */
    private function home_video_room() {
        $this->live_db = $this->load->database("live", TRUE);
        $nowtime = time();
        $sql_str = '(select anchor_id,room_id,0 as video_id, room_name as name,0 as read_num,peoples as watching_num,pic as cover,starttime as addtime,peoples as peopleby,3 as type,room_dest as dest_name,room_dest_id as dest_id,1 as isroom,status,sort,like_num,attr_id,"" as app_index_tui_pic from live_room  where live_status=1 and status in(1,2)  and  createtime>'.($nowtime - $this->room_timeout).'  )union (select anchor_id,room_id,id as video_id,name,people as read_num,0 as watching_num,pic as cover,addtime,people as peopleby,type,dest_name,dest_id,0 as isroom,status,sort,like_num,attr_id,app_index_tui_pic from live_video where video<>"" and status in(1,3)  and type=1 order by status desc ) order by isroom desc,status desc,sort desc,watching_num desc,peopleby desc,addtime desc  LIMIT 6';
        $live_room_data = $this->live_db->query($sql_str)->result_array();
        $num_live_room_data = count($live_room_data);
        $anchor_ids = array();
		$attr_id = array();
        ...
        if ($num_live_room_data > 0) {
            foreach ($live_room_data as $k => $v) {
                if (isset($live_anchor_ids_data[$v['anchor_id']])) {
                    ...
                    $live_room_data[$k]['anchor_sex'] = $live_anchor_ids_data[$v['anchor_id']]['sex'];
                    $live_room_data[$k]['anchor_type'] = $live_anchor_ids_data[$v['anchor_id']]['type'];
					if(isset($category_live_dictionary_data[$v['attr_id']])){
						$live_room_data[$k]['attrname'] = $category_live_dictionary_data[$v['attr_id']];
					}else{
						$live_room_data[$k]['attrname'] = '';
					}
                }
                unset($live_room_data[$k]['addtime'], $live_room_data[$k]['peopleby']);
            }
        }
        return $live_room_data;
    }

	
    private function home_video_room2() {
        $this->live_db = $this->load->database("live", TRUE);
        $nowtime = time();
		$sql_str = 'select anchor_id,room_id,id as video_id,name,people as read_num,0 as watching_num,pic as cover,addtime,people as peopleby,type,dest_name,dest_id,video_height,video_width,like_num,attr_id,app_index_tui_pic from live_video where video<>"" and status in(1,3) and type=2  order by status desc,sort desc,watching_num desc,peopleby desc,addtime desc LIMIT 6';
        $live_room_data = $this->live_db->query($sql_str)->result_array();
        $num_live_room_data = count($live_room_data);
        $anchor_ids = array();
		$attr_id = array();
        ...
        if ($num_live_room_data > 0) {
            foreach ($live_room_data as $k => $v) {
                if (isset($live_anchor_ids_data[$v['anchor_id']])) {
					...
					if(!empty($live_room_data[$k]['app_index_tui_pic'])){
						if (strpos($live_room_data[$k]['app_index_tui_pic'], 'http://') !== 0) {
							$live_room_data[$k]['cover'] = trim(base_url(''), '/') . $live_room_data[$k]['app_index_tui_pic'];
						}						
					}					
                    $live_room_data[$k]['avatar'] = trim(base_url(''), '/') . $live_anchor_ids_data[$v['anchor_id']]['photo'];
                    $live_room_data[$k]['anchor_name'] = $live_anchor_ids_data[$v['anchor_id']]['name'];
    ...
    public function get_user_info() {
        $token = $this->input->get_post('number', true);
        if (empty($token)){
            $this->__errormsg('number不能为空！');
        }
        $m_id = $this->F_get_mid($token);
        $returnData = array();
        // 昵称
        $sql_nickname = "SELECT nickname FROM u_member WHERE mid = {$m_id}";
        $returnData['nickname'] = $this->db->query($sql_nickname)->row_array();
        // 个人签名
        $sql_talk = "SELECT talk FROM u_member WHERE mid = {$m_id}";
        $returnData['talk'] = $this->db->query($sql_talk)->row_array();
        // 故乡
        $sql_country = "(SELECT ua.name FROM u_area AS ua LEFT JOIN u_member_attr AS uma  ON ua.id = uma.country RIGHT JOIN u_member AS um ON uma.member_id = um.mid where um.mid={$m_id})";
        $sql_province = "(SELECT ua.name FROM u_area AS ua LEFT JOIN u_member_attr AS uma  ON ua.id = uma.province RIGHT JOIN u_member AS um ON uma.member_id = um.mid where um.mid={$m_id})";
        $sql_city = "(SELECT ua.name FROM u_area AS ua LEFT JOIN u_member_attr AS uma ON ua.id = uma.city RIGHT JOIN u_member AS um ON uma.member_id = um.mid where um.mid={$m_id})";
        $sql_country_id = "(SELECT uma.country FROM u_member_attr AS uma LEFT JOIN u_member AS um ON uma.member_id = um.mid where um.mid={$m_id})";
        $sql_province_id = "(SELECT uma.province FROM u_member_attr AS uma LEFT JOIN u_member AS um ON uma.member_id = um.mid where um.mid={$m_id})";
        $sql_city_id = "(SELECT uma.city FROM u_member_attr AS uma LEFT JOIN u_member AS um ON uma.member_id = um.mid where um.mid={$m_id})";
//        $hometown_sql = "SELECT concat_ws('/', $sql_country, $sql_province, $sql_city) AS hometown";
        $returnData['hometown']['country']['name'] = $this->db->query($sql_country)->row_array();
        $returnData['hometown']['province']['name'] = $this->db->query($sql_province)->row_array();
        $returnData['hometown']['city']['name'] = $this->db->query($sql_city)->row_array();
        $returnData['hometown']['country']['id'] = $this->db->query($sql_country_id)->row_array();
        $returnData['hometown']['province']['id'] = $this->db->query($sql_province_id)->row_array();
        $returnData['hometown']['city']['id'] = $this->db->query($sql_city_id)->row_array();
        // 个人爱好
//        $hobby1_sql = "(SELECT hobby1 FROM u_member_attr WHERE member_id = {$m_id})";
//        $hobby2_sql = "(SELECT hobby2 FROM u_member_attr WHERE member_id = {$m_id})";
//        $hobby3_sql = "(SELECT hobby3 FROM u_member_attr WHERE member_id = {$m_id})";
//        $hobby4_sql = "(SELECT hobby4 FROM u_member_attr WHERE member_id = {$m_id})";
//        $hobby5_sql = "(SELECT hobby5 FROM u_member_attr WHERE member_id = {$m_id})";
//        $hobbies_sql = "SELECT concat_ws('/', $hobby1_sql, $hobby2_sql, $hobby3_sql, $hobby4_sql, $hobby5_sql) AS hobbies";
//        $returnData['hobbies'] = $this->db->query($hobbies_sql)->row_array();
        $sql_hobby = "SELECT hobby1 FROM u_member_attr WHERE member_id = {$m_id}";
        $returnData['hobbies'] = $this->db->query($sql_hobby)->row_array();
        // 喜欢美食
//        $food1_sql = "(SELECT food1 FROM u_member_attr WHERE member_id = {$m_id})";
//        $food2_sql = "(SELECT food2 FROM u_member_attr WHERE member_id = {$m_id})";
//        $food3_sql = "(SELECT food3 FROM u_member_attr WHERE member_id = {$m_id})";
//        $food4_sql = "(SELECT food4 FROM u_member_attr WHERE member_id = {$m_id})";
//        $food5_sql = "(SELECT food5 FROM u_member_attr WHERE member_id = {$m_id})";
//        $foods_sql = "SELECT concat_ws('/', $food1_sql, $food2_sql, $food3_sql, $food4_sql, $food5_sql) AS foods";
//        $returnData['foods'] = $this->db->query($foods_sql)->row_array();
        $sql_food = "SELECT food1 FROM u_member_attr WHERE member_id = {$m_id}";
        $returnData['foods'] = $this->db->query($sql_food)->row_array();
        // 去过地方
//        $dest1_sql = "(SELECT dest1 FROM u_member_attr WHERE member_id = {$m_id})";
//        $dest2_sql = "(SELECT dest2 FROM u_member_attr WHERE member_id = {$m_id})";
//        $dest3_sql = "(SELECT dest3 FROM u_member_attr WHERE member_id = {$m_id})";
//        $dest4_sql = "(SELECT dest4 FROM u_member_attr WHERE member_id = {$m_id})";
//        $dest5_sql = "(SELECT dest5 FROM u_member_attr WHERE member_id = {$m_id})";
//        $dests_sql = "SELECT concat_ws('/', $dest1_sql, $dest2_sql, $dest3_sql, $dest4_sql, $dest5_sql) AS dests";
//        $returnData['dests'] = $this->db->query($dests_sql)->row_array();
        $sql_dest = "SELECT dest1 FROM u_member_attr WHERE member_id = {$m_id}";
        $returnData['dests'] = $this->db->query($sql_dest)->row_array();
        // 性格类型
        $characters_sql = "SELECT GROUP_CONCAT(ud.description SEPARATOR '/') AS characters FROM u_dictionary AS ud RIGHT JOIN u_member_character AS umc ON FIND_IN_SET(ud.dict_id, umc.characters) LEFT JOIN u_member AS um ON umc.member_id = um.mid WHERE um.mid = {$m_id}";
        $characters_id_sql = "SELECT umc.characters FROM u_member_character AS umc LEFT JOIN u_member AS um ON umc.member_id = um.mid WHERE um.mid = {$m_id}";
        $returnData['characters']['name'] = $this->db->query($characters_sql)->row_array();
        $returnData['characters']['id'] = $this->db->query($characters_id_sql)->row_array();
        // 代表年代
        $decade_sql = "SELECT ud.description AS decade FROM u_dictionary AS ud LEFT JOIN u_member_attr AS uma ON ud.dict_id = uma.years LEFT JOIN u_member AS um ON uma.member_id = um.mid WHERE um.mid = {$m_id}";
        $decade_id_sql = "SELECT uma.years FROM u_member_attr AS uma LEFT JOIN u_member AS um ON uma.member_id = um.mid WHERE um.mid = {$m_id}";
        $returnData['decade']['name'] = $this->db->query($decade_sql)->row_array();
        $returnData['decade']['id'] = $this->db->query($decade_id_sql)->row_array();
        // 星座
        $constellation_sql = "SELECT ud.description AS constellation FROM u_dictionary AS ud LEFT JOIN u_member_attr AS uma ON ud.dict_id = uma.constellation LEFT JOIN u_member AS um ON uma.member_id = um.mid WHERE um.mid = {$m_id}";
        $constellation_id_sql = "SELECT uma.constellation FROM u_member_attr AS uma LEFT JOIN u_member AS um ON uma.member_id = um.mid WHERE um.mid = {$m_id}";
        $returnData['constellation']['name'] = $this->db->query($constellation_sql)->row_array();
        $returnData['constellation']['id'] = $this->db->query($constellation_id_sql)->row_array();
        // 血型
        $blood_sql = "SELECT ud.description AS blood FROM u_dictionary AS ud LEFT JOIN u_member_attr AS uma ON ud.dict_id = uma.blood LEFT JOIN u_member AS um ON uma.member_id = um.mid WHERE um.mid = {$m_id}";
        $blood_id_sql = "SELECT uma.blood FROM u_member_attr AS uma LEFT JOIN u_member AS um ON uma.member_id = um.mid WHERE um.mid = {$m_id}";
        $returnData['blood']['name'] = $this->db->query($blood_sql)->row_array();
        $returnData['blood']['id'] = $this->db->query($blood_id_sql)->row_array();
        // 喜欢去哪玩(境内)
        $prefer_domestic_sql = "SELECT GROUP_CONCAT(ud.description SEPARATOR '/') AS prefer_domestic FROM u_dictionary AS ud LEFT JOIN u_member_play_domestic AS umpd ON FIND_IN_SET(ud.dict_id, umpd.dest_id) LEFT JOIN u_member AS um ON umpd.member_id = um.mid WHERE um.mid = {$m_id}";
        $prefer_domestic_id_sql = "SELECT umpd.dest_id FROM u_member_play_domestic AS umpd LEFT JOIN u_member AS um ON umpd.member_id = um.mid WHERE um.mid = {$m_id}";
        $returnData['prefer_domestic']['name'] = $this->db->query($prefer_domestic_sql)->row_array();
        $returnData['prefer_domestic']['id'] = $this->db->query($prefer_domestic_id_sql)->row_array();
        // 喜欢去哪玩(境外)
        $prefer_abroad_sql = "SELECT GROUP_CONCAT(ud.description SEPARATOR '/') AS prefer_abroad FROM u_dictionary AS ud LEFT JOIN u_member_play_abroad AS umpa ON FIND_IN_SET(ud.dict_id, umpa.dest_id) LEFT JOIN u_member AS um ON umpa.member_id = um.mid WHERE um.mid = {$m_id}";
        $prefer_abroad_id_sql = "SELECT umpa.dest_id FROM u_member_play_abroad AS umpa LEFT JOIN u_member AS um ON umpa.member_id = um.mid WHERE um.mid = {$m_id}";
        $returnData['prefer_abroad']['name'] = $this->db->query($prefer_abroad_sql)->row_array();
        $returnData['prefer_abroad']['id'] = $this->db->query($prefer_abroad_id_sql)->row_array();
        // 喜欢怎么玩
        $prefer_how_play_sql = "SELECT GROUP_CONCAT(ud.description SEPARATOR '/') AS prefer_how_play FROM u_dictionary AS ud LEFT JOIN u_member_play_how AS umph ON FIND_IN_SET(ud.dict_id, umph.way_id) LEFT JOIN u_member AS um ON umph.member_id = um.mid WHERE um.mid = {$m_id}";
        $prefer_how_play_id_sql = "SELECT umph.way_id FROM u_member_play_how AS umph LEFT JOIN u_member AS um ON umph.member_id = um.mid WHERE um.mid = {$m_id} ";
        $returnData['prefer_how_play']['name'] = $this->db->query($prefer_how_play_sql)->row_array();
        $returnData['prefer_how_play']['id'] = $this->db->query($prefer_how_play_id_sql)->row_array();
        // 喜欢跟谁玩
        $prefer_whom_prefer_play_with_sql = "SELECT GROUP_CONCAT(ud.description SEPARATOR '/') AS whom_play_with FROM u_dictionary AS ud LEFT JOIN u_member_play_with AS umpw ON FIND_IN_SET(ud.dict_id, umpw.with_id) LEFT JOIN u_member AS um ON umpw.member_id = um.mid WHERE um.mid = {$m_id}";
        $prefer_whom_prefer_play_with_id_sql = "SELECT umpw.with_id FROM u_member_play_with AS umpw LEFT JOIN u_member AS um ON umpw.member_id = um.mid WHERE um.mid = {$m_id}";
        $returnData['whom_prefer_play_with']['name'] = $this->db->query($prefer_whom_prefer_play_with_sql)->row_array();
        $returnData['whom_prefer_play_with']['id'] = $this->db->query($prefer_whom_prefer_play_with_id_sql)->row_array();
        // 平日休闲方式
        $relax_sql = "SELECT GROUP_CONCAT(ud.description SEPARATOR '/') AS relax FROM u_dictionary AS ud LEFT JOIN u_member_relax AS umr ON FIND_IN_SET(ud.dict_id, umr.relax_id) LEFT JOIN u_member AS um ON umr.member_id = um.mid WHERE um.mid = {$m_id}";
        $relax_id_sql = "SELECT umr.relax_id FROM u_member_relax AS umr LEFT JOIN u_member AS um ON umr.member_id = um.mid WHERE um.mid = {$m_id} ";
        $returnData['relax']['name'] = $this->db->query($relax_sql)->row_array();
        $returnData['relax']['id'] = $this->db->query($relax_id_sql)->row_array();
    ...
    public function submit_user_info() {
        ...
        $m_id = $this->F_get_mid($token);
        if (empty($m_id)){      // weiy add 2017-1-11 19:11:30
            $m_id = 0;
        }
        // 昵称
        $nickname = $this->input->get_post('nickname', true);
        $nickname = empty($nickname) ? '' : $nickname;
        $sql_nickname = "UPDATE u_member SET nickname = '{$nickname}' WHERE mid = {$m_id}";
        $this->db->query($sql_nickname);
        // 同时更改live_anchor表中的昵称
        $this->social_db = $this->load->database("live", TRUE);
        $sql_nickname_of_anchor = "UPDATE live_anchor SET nickname = '{$nickname}' WHERE user_type = 0 AND user_id = {$m_id}";
        $this->social_db->query($sql_nickname_of_anchor);
        // 个人签名
        $talk = $this->input->get_post('talk', true);
        $talk = empty($talk) ? '' : $talk;
        $sql_talk = "UPDATE u_member SET talk = '{$talk}' WHERE mid = {$m_id}";
        $this->db->query($sql_talk);
        // 故乡
        $country_id = $this->input->get_post('country', true);
        $province_id = $this->input->get_post('province', true);
        ...
        $sql_hometown = "INSERT INTO u_member_attr(member_id, country, province, city) VALUES('{$m_id}', '{$country_id}', '{$province_id}', '{$city_id}') ON DUPLICATE KEY UPDATE country = '{$country_id}', province = '{$province_id}', city = '{$city_id}' ";
        $this->db->query($sql_hometown);
        // 爱好
        $hobby = $this->input->get_post('hobby', true);
        $hobby = empty($hobby) ? '' : $hobby;
        $sql_hobby = "INSERT INTO u_member_attr(member_id, hobby1) VALUES('{$m_id}', '{$hobby}') ON DUPLICATE KEY UPDATE hobby1 = '{$hobby}'";
        $this->db->query($sql_hobby);
        // 喜欢的美食
        $food = $this->input->get_post('food', true);
        $food = empty($food) ? '' : $food;
        $sql_food = "INSERT INTO u_member_attr(member_id, food1) VALUES('{$m_id}', '{$food}') ON DUPLICATE KEY UPDATE food1 = '{$food}'";
        $this->db->query($sql_food);
        // 到过的地方
        $dest = $this->input->get_post('dest', true);
        $dest = empty($dest) ? '' : $dest;
        $sql_dest = "INSERT INTO u_member_attr(member_id, dest1) VALUES('{$m_id}', '{$dest}') ON DUPLICATE KEY UPDATE dest1 = '{$dest}'";
        $this->db->query($sql_dest);
        // 性格类型
        $character_id = $this->input->get_post('character_id', true);
        $character_id = empty($character_id) ? 0 : $character_id;
        $sql_character = "INSERT INTO u_member_character(member_id, characters) VALUES('{$m_id}', '{$character_id}') ON DUPLICATE KEY UPDATE characters = '{$character_id}'";
        $this->db->query($sql_character);
        // 代表年代
        $decade = $this->input->get_post('decade', true);
        $decade = empty($decade) ? 0 : $decade;
        $sql_decade = "INSERT INTO u_member_attr(member_id, years) VALUES('{$m_id}', '{$decade}') ON DUPLICATE KEY UPDATE years = '{$decade}' ";
        $this->db->query($sql_decade);
        // 星座
        $constellation = $this->input->get_post('constellation', true);
        $constellation = empty($constellation) ? 0 : $constellation;
        $sql_constellation = "INSERT INTO u_member_attr(member_id, constellation) VALUES('{$m_id}', '{$constellation}') ON DUPLICATE KEY UPDATE constellation = '{$constellation}'";
        $this->db->query($sql_constellation);
        // 血型
        $blood_type = $this->input->get_post('blood_type', true);
        $blood_type = empty($blood_type) ? 0 : $blood_type;
        $sql_blood_type = "INSERT INTO u_member_attr(member_id, blood) VALUES('{$m_id}', '{$blood_type}') ON DUPLICATE KEY UPDATE blood = '{$blood_type}'";
        $this->db->query($sql_blood_type);
        // 喜欢去哪玩(境内)
        $domestic_like_to_go = $this->input->get_post('domestic_like_to_go', true);
        $domestic_like_to_go = empty($domestic_like_to_go) ? '' : $domestic_like_to_go;
        $sql_domestic = "INSERT INTO u_member_play_domestic(member_id, dest_id) VALUES('{$m_id}', '{$domestic_like_to_go}') ON DUPLICATE KEY UPDATE dest_id = '{$domestic_like_to_go}'";
        $this->db->query($sql_domestic);
        // 喜欢去哪玩(境外)
        $abroad_like_to_go = $this->input->get_post('abroad_like_to_go', true);
        $abroad_like_to_go = empty($abroad_like_to_go) ? '' : $abroad_like_to_go;
        $sql_abroad = "INSERT INTO u_member_play_abroad(member_id, dest_id) VALUES('{$m_id}', '{$abroad_like_to_go}') ON DUPLICATE KEY UPDATE dest_id = '{$abroad_like_to_go}'";
        $this->db->query($sql_abroad);
        // 喜欢怎么玩
        $how_to_play = $this->input->get_post('how_to_play', true);
        $how_to_play = empty($how_to_play) ? 0 : $how_to_play;
        $sql_how_to_play = "INSERT INTO u_member_play_how(member_id, way_id) VALUES('{$m_id}', '{$how_to_play}') ON DUPLICATE KEY UPDATE way_id = '{$how_to_play}'";
        $this->db->query($sql_how_to_play);
        // 喜欢和谁玩
        $whom_play_with = $this->input->get_post('whom_play_with', true);
        $whom_play_with = empty($whom_play_with) ? 0 : $whom_play_with;
        $sql_whom_play_with = "INSERT INTO u_member_play_with(member_id, with_id) VALUES('{$m_id}', '{$whom_play_with}') ON DUPLICATE KEY UPDATE with_id = '{$whom_play_with}'";
        $this->db->query($sql_whom_play_with);
        // 平日休闲方式
        $daily_leisure_style = $this->input->get_post('daily_leisure_style', true);
        $daily_leisure_style = empty($daily_leisure_style) ? 0 : $daily_leisure_style;
        $sql_daily_leisure_style = "INSERT INTO u_member_relax(member_id, relax_id) VALUES('{$m_id}', '{$daily_leisure_style}') ON DUPLICATE KEY UPDATE relax_id = '{$daily_leisure_style}'";
        $this->db->query($sql_daily_leisure_style);
    ...
    public function get_home_info(){
        // 获取主题相关信息
        $sql_theme = "SELECT id theme_id, name theme_name, pic1 pics, video_url1 video_url, concern_count FROM social_theme";
