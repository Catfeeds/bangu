...
//            }
            ...
            // 帖子点赞数
            $sql_praise_cnt = "SELECT id FROM social_praise WHERE category_id = {$post_id} AND type = 1";
            $v['praise_count'] = $this->social_db->query($sql_praise_cnt)->num_rows();
    ...
    public function get_post_by_id() {
        ...
        // 统计帖子所属主题的参与数
        $sql_invol = "INSERT INTO social_theme_hot_num(invol_cnt, theme_id, type) VALUES (1, $theme_id, 1) 
                ON DUPLICATE KEY UPDATE invol_cnt = invol_cnt + 1";
        $this->social_db->query($sql_invol);
     ...
     * @param type              关注类型  1表示主题,2表示话题,3表示场景,4表示人,5表示故事
     */
    public function concern() {
        ...
        if ($query->num_rows() < 1) {    // 还没有关注,进行关注
            // 将对应的关注数加1
            if (($type == 1 OR $type == 5) && empty($content_id)) { // 主题/故事
                $sql = '';
                if ($type == 1){
                    $sql = "UPDATE social_theme SET concern_count = concern_count + 1 WHERE id = {$category_id}";
                }else if ($type == 5){
                    $sql = "UPDATE social_story SET concern_count = concern_count + 1 WHERE id = {$category_id}";
                }
...
//                    $this->social_db->trans_rollback();
            ...
            } else if (($type == 1 OR $type == 5) && !empty($content_id)) {   // 帖子
                $sql = '';
                if ($type == 1){
                    $sql = "UPDATE social_theme_post SET watched_count = watched_count + 1 WHERE id = {$content_id} ";
                }else if ($type == 5){
                    $sql = "UPDATE social_story_post SET watched_count = watched_count + 1 WHERE id = {$content_id} ";
                }
            ...
            // 将对应的关注数减1
            if (($type == 1 OR $type == 5) && empty($content_id)) { // 主题
                $sql = '';
                if ($type == 1){
                    $sql = "UPDATE social_theme SET concern_count = concern_count - 1 WHERE id = {$category_id}";
                }else if ($type == 5){
     ...
     * @param type 类型        为1表示主题帖子,为2表示话题回答,3表示故事帖子
    ...
    public function praise_post() {
        ...
        $time = time();
        if (1 == $type OR 3 == $type){    // 帖子每天只能点赞一次
            // 获取上次点赞的时间
            ...
            $sql_praisable = "SELECT update_time FROM social_praise WHERE category_id = {$category_id} AND type = {$type} AND member_id = {$this->anchor_id}";
        ...
        if ($this->social_db->affected_rows() > 0) {
            ...
            if ($type == '1') {
                ...
                $sql_update_count = "UPDATE social_story_post SET praise_count = praise_count + 1 WHERE id = {$category_id}";
                $this->social_db->query($sql_update_count);
                $this->__outlivemsg('', '点赞成功', 2000);
            }
        }
...
//        $data['pic1'] = $pics;
    ...
    public function get_question_detail() {
        ...
        // 问题回答数
        $sql_answer = "SELECT id FROM social_topic_answer WHERE question_id = $question_id";
        $ret['answer_cnt'] = $this->social_db->query($sql_answer)->num_rows();
        
        // 统计帖子所属主题的参与数
        $sql_invol = "INSERT INTO social_theme_hot_num(invol_cnt, theme_id, type) VALUES (1, $topic, 3)  
                ON DUPLICATE KEY UPDATE invol_cnt = invol_cnt + 1";
        $this->social_db->query($sql_invol);
...
//        $sql = "SELECT id video_id, video_name name, video_pic pic, 0 as people, 0 as dest_id FROM social_scene_video LIMIT {$from}, {$page_size}";
    ...
    public function get_daka_dynamics() {
        ...
        foreach ($ret as &$v) {
            ...
            if (1 == $v['type']) {   // 主题
                ...
                // 获取答问数
                $q_id  = isset($question['id']) ? $question['id'] : 0;
                $sql_answer = "SELECT id FROM social_topic_answer WHERE question_id = {$q_id} ";
                $answer = $this->social_db->query($sql_answer);
                $v['answer_cnt'] = $answer->num_rows();
            } else if (3 == $v['type']) {      // 视频
                // 获取大咖视频
     ...
     * @param   page        第几页
     * @param   page_size   每页多少条
     * @param   type        1表示主题,2表示问答,3表示大咖,4表示故事
     * @return  content_type    1表示心情, 2表示问题, 3表示故事
     */
    public function get_concern_by_user() { 
        ...
        if ($type == 1) {        // 主题 和 故事
            ...
            // 获取关注的故事
            $sql_story = "SELECT DISTINCT category_id, update_time FROM social_concern WHERE member_id = {$this->anchor_id} AND type = 5";
            $story = $this->social_db->query($sql_story)->result_array();
            if (!$theme && !$story) {
            ...
            if (empty($theme)){
                ...
                foreach ($theme as $val) {
                    $category_id = isset($val['category_id']) ? $val['category_id'] : 0;
                    $update_time = isset($val['update_time']) ? $val['update_time'] : 0;
                    $where_theme .= " OR (theme_id = {$category_id} AND update_time >= {$update_time}) ";
                }
            }
            $where_theme = 'WHERE ' . trim(trim($where_theme), 'OR');
//            $where_limit = $where_theme." ORDER BY update_time DESC LIMIT {$from}, {$page_size}";
            $sql_post_where = "SELECT id, title, content, video_pic, video_url, pic1, pic2, pic3, member_id, theme_id, praise_count, watched_count, update_time, line_id, 1 content_type FROM social_theme_post " . $where_theme;
            ...
            foreach ($story as $val) {
                $category_id = isset($val['category_id']) ? $val['category_id'] : 0;
                $update_time = isset($val['update_time']) ? $val['update_time'] : 0;
                $where_story .= " OR (theme_id = {$category_id} AND update_time >= {$update_time}) ";
            }
            $where_story = 'WHERE ' . trim(trim($where_story), 'OR');
//            $where_limit = $where_theme." ORDER BY update_time DESC LIMIT {$from}, {$page_size}";
            $sql_story_where = "SELECT id, title, content, video_pic, video_url, pic1, pic2, pic3, member_id, theme_id, praise_count, watched_count, update_time, line_id, 3 content_type FROM social_story_post " . $where_story;
            
            $sql_both = $sql_post_where . ' UNION ' . $sql_story_where . " ORDER BY update_time DESC LIMIT {$from}, {$page_size}";
            // 拼装sql结束
            file_put_contents('def.txt', $sql_both);
            $post = $this->social_db->query($sql_both)->result_array();
            if ($post) {
                foreach ($post as &$v) {
                    // 图片处理
                    ...
                    if (1 == $content_type){
                        ...
                        $sql_theme = "SELECT name FROM social_story WHERE id = {$theme_id}";
                    }
                    $theme = $this->social_db->query($sql_theme)->row_array();
                    $v['theme_name'] = isset($theme['name']) ? $theme['name'] : '';
                    // 获取回帖数
                    ...
                    if (1 == $content_type){
                        ...
                        $sql_answer = "SELECT id FROM social_story_post_answer WHERE post_id = {$post_id} ";
                    }
                    $answer = $this->social_db->query($sql_answer);
                    $v['answer_cnt'] = $answer->num_rows();
                    // 设置内容类型
//                    $v['content_type'] = 1;     // 为1表示帖子    // 转移到sql语句中
                    
                    // 当天是否对帖子点过赞
                    $member_id_1 = empty($this->anchor_id) ? 0 : $this->anchor_id;
                    $post_id_1 = isset($v['id']) ? $v['id'] : 0;
                    $sql_praised_post = '';
                    if (1 == $content_type){    // 心情
                        $sql_praised_post = "SELECT update_time FROM social_praise WHERE category_id = {$post_id_1} AND type = 1 AND member_id = {$member_id_1} ORDER BY update_time DESC LIMIT 1";
                    }else if (3 == $content_type){  // 故事
                        $sql_praised_post = "SELECT update_time FROM social_praise WHERE category_id = {$post_id_1} AND type = 3 AND member_id = {$member_id_1} ORDER BY update_time DESC LIMIT 1";
                    }
                    $query_praised_post = $this->social_db->query($sql_praised_post);
                    if ($query_praised_post->num_rows() < 1) {
                        $v['praised_today'] = 0;
...
//            $sql_theme = "SELECT id, title, content, video_pic, video_url, pic1 pics, member_id, theme_id, praise_count, watched_count, update_time, line_id FROM social_theme_post ".$where_theme;
//            $cnt_theme = $this->social_db->query($sql_theme)->num_rows();
//            $sql_story = "SELECT id, title, content, video_pic, video_url, pic1 pics, member_id, theme_id, praise_count, watched_count, update_time, line_id FROM social_theme_post ".$where_story;
//            $cnt_story = $this->social_db->query($sql_story)->num_rows();
            $sql_cnt = $sql_post_where . ' UNION ' . $sql_story_where;
            $cnt_both = $this->social_db->query($sql_cnt)->num_rows();
            if (0 == $cnt_both){
                $this->__nullmsg();
            }
            $ret['cnt'] = $cnt_both;
            ...
            foreach ($topic as $val){
                $category_id = isset($val['category_id']) ? $val['category_id'] : 0;
                $update_time = isset($val['update_time']) ? $val['update_time'] : 0;
                $where .= " OR (topic_id = {$category_id} AND update_time >= {$update_time}) ";
            }
            ...
            if ($question) {
                foreach ($question as &$v) {
                    ...
                    $sql_answer = "SELECT id FROM social_topic_answer WHERE question_id = {$question_id}";
                    $answer = $this->social_db->query($sql_answer);
                    file_put_contents('abc.txt', $this->social_db->last_query(), FILE_APPEND);
                    $v['answer_cnt'] = $answer->num_rows();
                    // 设置内容类型
            ...
            foreach ($daka as $val){
                $category_id = isset($val['category_id']) ? $val['category_id'] : 0;
                $update_time = isset($val['update_time']) ? $val['update_time'] : 0;
                $where .= " OR (member_id = {$category_id} AND update_time >= {$update_time}) ";
            }
            $where = 'WHERE ' . trim(trim($where), 'OR');
            $where_limit = $where . " ORDER BY update_time DESC LIMIT {$from}, {$page_size}";
            // 被关注大咖的发帖
            $sql_all = "SELECT * FROM
            (SELECT id, title, content, video_pic, video_url, pic1, pic2, pic3, member_id, line_id, theme_id,  0 topic_id, update_time, 0 involve_count, praise_count, watched_count, 0 concern_count, 0 invited_id, 1 content_type FROM social_theme_post
            UNION
            ...
            UNION
            SELECT id, title, content, video_pic, video_url, pic1, pic2, pic3, member_id, line_id, 0 theme_id, topic_id, update_time, involve_count, 0 praise_count, 0 watched_count, concern_count, invited_id, 3 content_type FROM social_story_post
            ) t ".$where_limit;
    ...
    public function get_home_info(){
        ...
        foreach($theme as &$v){
            ...
            $cnt_post = 0;      // 主题的帖子数
            $cnt_answer = 0;    // 主题的回复数
            $cnt_praise = 0;    // 主题中帖子的点赞数
            
            
            // 获取主题的参与数,评论数;参与数为帖子数与回复数之和,评论数为所有帖子的回复数之和
            $theme_id = isset($v['theme_id']) ? $v['theme_id'] : 0;
            // 获取含有视频的帖子
            $sql_post = "SELECT id, title, content, video_pic, praise_count FROM social_theme_post WHERE theme_id = $theme_id AND video_url IS NOT NULL AND video_url <> '' AND video_url <> '0'";
            ...
            // 获取主题下帖子的点赞数
            // 1.获取主题下所有的帖子
            $sql_post_all = "SELECT id FROM social_theme_post WHERE theme_id = $theme_id";
            $post_all = $this->social_db->query($sql_post_all)->result_array();
            foreach($post_all as $value){
                $post_id = isset($value['id']) ? $value['id'] : 0;
                $sql_praise_cnt = "SELECT id FROM social_praise WHERE category_id = {$post_id} AND type = 1";
                $cnt_praise += $this->social_db->query($sql_praise_cnt)->num_rows();
            }
            $v['cnt_praise'] = $cnt_praise;
        }
        $this->__outmsg($theme);
    }
     ...
     * @since  2017-1-18 14:56:27
     */
    public function get_home_info_story(){
        // 获取主题相关信息
        $type = $this->input->post('type', TRUE);
        $type = $type ? (int)$type : 0;
        // TODO 加入排序
        ...
        $page_size = $page_size ? (int)$page_size : 5;
        $from = ($page - 1) * $page_size;
        $sql_story = "SELECT id theme_id, name theme_name, pic1 pics, video_url1 video_url, concern_count FROM social_story LIMIT $from, $page_size";
        $story = $this->social_db->query($sql_story)->result_array();
        if (empty($story)){
            $this->__nullmsg();
        }
        foreach($story as &$v){
            ...
            $cnt_post = 0;      // 主题的帖子数
            $cnt_answer = 0;    // 主题的回复数
            $cnt_praise = 0;    // 主题帖子点赞数
            // 获取主题的参与数,评论数;参与数为帖子数与回复数之和,评论数为所有帖子的回复数之和
            $story_id = isset($v['theme_id']) ? $v['theme_id'] : 0;
            // 获取故事关注数
            $sql_concern_cnt = "SELECT id FROM social_concern WHERE type = 5 AND category_id = {$story_id} AND content_id = 0";
            $v['concern_count'] = $this->social_db->query($sql_concern_cnt)->num_rows();
            // 获取含有视频的帖子
            $sql_post = "SELECT id, title, content, video_pic, praise_count FROM social_story_post WHERE theme_id = $story_id AND video_url IS NOT NULL AND video_url <> '' AND video_url <> '0'";
            $query_post = $this->social_db->query($sql_post);
            $cnt_post += $query_post->num_rows();
            $posts = $query_post->result_array();
            foreach($posts as &$val){
            ...
            // 获取故事下帖子的点赞数
            // 1.获取故事下所有的帖子
            $sql_post_all = "SELECT id FROM social_story_post WHERE theme_id = $story_id";
            $post_all = $this->social_db->query($sql_post_all)->result_array();
            foreach($post_all as $value){
                $post_id = isset($value['id']) ? $value['id'] : 0;
                $sql_praise_cnt = "SELECT id FROM social_praise WHERE category_id = {$post_id} AND type = 3";
                $cnt_praise += $this->social_db->query($sql_praise_cnt)->num_rows();
            }
            $v['cnt_praise'] = $cnt_praise;
        }
     ...
     /**
     * @desc 获取故事详情页描述和线路部分
     * @param type theme_id 故事id
     * @param type page 第几页
     * @param type page_size 每页条数
     */
    public function cfgm_get_story_detail_of_description() {
        $theme_id = $this->input->post('theme_id', TRUE);
        if (empty($theme_id)) {
            $this->__errormsg("theme_id不能为空");
        }
...
//                FROM social_theme WHERE id = {$theme_id}";
        $sql = "SELECT name, icon, concern_count, pic1 as pic, video_url1 as video_urls, description
                FROM social_story WHERE id = {$theme_id}";
        $returnData['desc'] = $this->social_db->query($sql)->row_array();
        if (empty($returnData['desc'])){
            $this->__nullmsg();
        }
        ...
        $returnData['desc']['pic'] = $this->__doImage($arr);
        // 线路
        $sql_line_id = "SELECT line1, line2, line3, line4, line5, line6 FROM social_story WHERE id = $theme_id";
        $arr_line = $this->social_db->query($sql_line_id)->row_array();
        $line1 = $arr_line['line1'] ? $arr_line['line1'] : 0;
        $line2 = $arr_line['line2'] ? $arr_line['line2'] : 0;
        $line3 = $arr_line['line3'] ? $arr_line['line3'] : 0;
...
//            $sql = "SELECT id FROM social_concern WHERE member_id = {$this->user_id} and type = 1 and category_id = {$theme_id}";
            // type为5表示故事
            $sql = "SELECT id FROM social_concern WHERE member_id = {$this->anchor_id} and type = 5 AND category_id = {$theme_id} AND content_id = 0";   // 将user_id改为anchor_id
            $query = $this->social_db->query($sql);
            if ($query->num_rows() < 1) {
                $returnData['is_concerned'] = 0;
            } else {
            ...
            $sql_concern_cnt = "SELECT id FROM social_concern WHERE type = 5 AND category_id = {$theme_id} AND content_id = 0";
            $returnData['desc']['concern_count'] = $this->social_db->query($sql_concern_cnt)->num_rows();
        }
        // 获取主题下面帖子的数量
        $sql_theme = "SELECT COUNT(id) as count FROM social_story_post WHERE theme_id = {$theme_id} ORDER BY NULL";
        $theme = $this->social_db->query($sql_theme)->row_array();
        $returnData['count'] = isset($theme['count']) ? $theme['count'] : 0;        
        // 分享
        $returnData['desc']['share_url'] = $this->share_url . 'dongtai_share/?theme_id=' . $theme_id;
     ...
     * @param type page_size 每页条数
     */
    public function cfgm_get_story_detail_of_post() {
        // 获取主题下的帖子
        $theme_id = $this->input->post('theme_id', TRUE);
        if (empty($theme_id)) {
            $this->__errormsg('theme_id不能为空');
...
//        $sql_post = "SELECT * FROM social_theme_post WHERE theme_id = {$theme_id} ORDER BY update_time DESC";
        $sql_post = "SELECT id, title, content, video_pic, video_url, concat_ws(',', pic1, pic2, pic3) as pics, member_id, theme_id, praise_count, 
            (select count(id) from social_story_post_answer where post_id = stp.id) as watched_count, update_time, line_id, 
                '' as linename, '' as linepic, '' as nickname, '' as litpic
                FROM social_story_post stp WHERE theme_id = {$theme_id} ORDER BY update_time DESC limit {$from}, $page_size";
        $arr_post = $this->social_db->query($sql_post)->result_array();
        // arr_post进行处理，获取member_id对应的昵称以及line_id对应的线路名和图片
        foreach ($arr_post as &$v) {
            // 根据member_id查询用户的昵称,根据line_id查询线路名称和图片
...
//            }
            ...
            // 当天是否对帖子点过赞
            // type为3表示故事帖子
            $sql_praised_post = "SELECT update_time FROM social_praise WHERE category_id = {$post_id} AND type = 3 AND member_id = {$this->anchor_id} ORDER BY update_time DESC LIMIT 1";
            $query_praised_post = $this->social_db->query($sql_praised_post);
            if ($query_praised_post->num_rows() < 1) {
                $v['praised_today'] = 0;
            } else {
     ...
     * @param type $line_id     帖子线路id
     */
    public function cfgm_putup_story_post() {
        if (empty($this->user_id)) {//用户没有登录
            $this->__errormsg('用户没有登录');
        }
        $theme_id = intval($this->input->post('theme_id', TRUE));
        ...
        $ret = $this->social_db->insert('social_story_post', $data);
     ...
     * @param post_id  帖子id
     */
    public function get_story_post_by_id() {
        $post_id = $this->input->post('post_id', TRUE);
        if (empty($post_id)) {
            $this->__errormsg('post_id不能为空');
        }
        $sql_post = "SELECT id, title, content, video_pic, video_url, concat_ws(',', pic1, pic2, pic3) as pics, member_id, theme_id, praise_count, 
            (select count(id) from social_story_post_answer where post_id = stp.id) as watched_count, update_time, line_id, 
                '' as linename, '' as linepic, '' as nickname, '' as litpic
                FROM social_story_post stp WHERE id = {$post_id}";
        ...
        // 获取主题名称
        $theme_id = isset($v['theme_id']) ? $v['theme_id'] : 0;
        $sql_theme = "SELECT name, description FROM social_story WHERE id = $theme_id";
        $theme = $this->social_db->query($sql_theme)->row_array();
        $v['theme_name'] = isset($theme['name']) ? $theme['name'] : '';
        $m_id = isset($v['member_id']) ? $v['member_id'] : 0;    // member_id对应live_anchor表中的anchor_id
        $line_id = isset($v['line_id']) ? $v['line_id'] : 0;
        ...
        if ($this->anchor_id) {   // 如果已登录
            ...
            // 是否是用户自己的帖子
            $member_id = "SELECT member_id FROM social_story_post WHERE id = {$post_id}";
            $query_member_id = $this->social_db->query($member_id);
            if ($query_member_id->num_rows() > 0) {
                $ret = $query_member_id->row_array();
                if ($ret['member_id'] == $this->anchor_id) {
        ...
        // 统计帖子所属主题的参与数
        $sql_invol = "INSERT INTO social_theme_hot_num(invol_cnt, theme_id, type) VALUES (1, $theme_id, 2)  
                ON DUPLICATE KEY UPDATE invol_cnt = invol_cnt + 1";
        $this->social_db->query($sql_invol);
        $this->__outmsg($v);
    }
     ...
     * @param  page_size 每页条数
     */
    public function cfgm_get_story_detail_of_post_answer() {
        $post_id = $this->input->get_post('post_id', TRUE);
        if (empty($post_id)) {
            $this->__errormsg('post_id不能为空');
        }
        ...
        $sql_post_answer = "SELECT content, video_pic, video_url, concat_ws(',', pic1, pic2, pic3) as pics, member_id, praise_count,
            update_time, line_id, '' as linename, '' as linepic, '' as nickname, '' as litpic
            FROM social_story_post_answer WHERE post_id = {$post_id} ORDER BY update_time DESC limit $from, $page_size";
        $ret = $this->social_db->query($sql_post_answer)->result_array();
        foreach ($ret as &$v) {
            $m_id = isset($v['member_id']) ? $v['member_id'] : 0;        // member_id对应live_anchor中的anchor_id
            $line_id = isset($v['line_id']) ? $v['line_id'] : 0;
     ...
     * @param type $post_id     被回复帖子id
     */
    public function cfgm_response_story_post() {
        if (empty($this->user_id)) {//用户没有登录
            $this->__errormsg('用户没有登录');
        }
        $theme_id = intval($this->input->post('theme_id', TRUE));
        ...
        $ret = $this->social_db->insert('social_story_post_answer', $data);
        if (empty($ret)) {
            $this->__errormsg("回复帖子失败!");
        } else {
        	// 对应帖子的参与数加1
        	$sql_incre_post = "UPDATE social_story_post SET watched_count = watched_count + 1 WHERE id = {$post_id}";
        	//获取积分配置
        	$integral_config=$this->config->item('integral_config');
        	$content='回复帖子获得';
        	$this->db->trans_begin();//事务开启
    ...
    /**
     * @method get_theme_hot_num
     * @desc    获取主题/心情的热度值
     */
    public function set_theme_hot_num(){
        // 参与/观看 占比60%   评论占比30%  关注 10%
        // 获取前一天0点的参与数(回复数与点赞数之和)
        // 获取当前的参与数
        
        // 统计心情和故事的浏览数,评论数,关注数,点赞数,热度值
        $sql_hot_num = "SELECT id, 1 type FROM social_theme UNION SELECT id, 2 type FROM social_story";
        $hot_nums = $this->social_db->query($sql_hot_num)->result_array();
        foreach($hot_nums as $v){   // 遍历心情或故事
            $cnt_ans_posts = 0;         // 心情或故事下的回复总数
            $cnt_concern = 0;           // 心情或故事的关注数
            $cnt_praise = 0;            // 心情或故事帖子的点赞数
            $theme_id = isset($v['id']) ? $v['id'] : 0;
            // 获取当前主题或故事下的回复数和点赞数
            $type = isset($v['type']) ? $v['type'] : 0;
            if (1 == $type) {// 心情
                $sql_theme = "SELECT id FROM social_theme_post WHERE theme_id = {$theme_id}";
                $posts = $this->social_db->query($sql_theme)->result_array();
                foreach($posts as $val){// 帖子
                    $post_id = isset($val['id']) ? $val['id'] : 0;
                    $sql_answer = "SELECT id FROM social_theme_post_answer WHERE post_id = {$post_id}";
                    $cnt_ans_posts += $this->social_db->query($sql_answer)->num_rows();
                    // 帖子点赞数
                    $sql_praise_cnt = "SELECT id FROM social_praise WHERE category_id = {$post_id} AND type = 1";
                    $cnt_praise += $this->social_db->query($sql_praise_cnt)->num_rows();
                }
                // 关注数
                $sql_concern_cnt = "SELECT id FROM social_concern WHERE type = 1 AND category_id = {$theme_id} AND content_id = 0";
                $cnt_concern += $this->social_db->query($sql_concern_cnt)->num_rows();
            }else if (2 == $type){// 故事
                $sql_story = "SELECT id FROM social_story_post WHERE theme_id = {$theme_id}";
                $posts = $this->social_db->query($sql_story)->result_array();
                foreach($posts as $val){
                    $post_id = isset($val['id']) ? $val['id'] : 0;
                    $sql_answer = "SELECT id FROM social_story_post_answer WHERE post_id = {$post_id}";
                    // 帖子回复数
            ...
            $time = time(); //FROM_UNIXTIME(1234567890, '%Y-%m-%d %H:%i:%S') 
            $t = date("Y-m-d", $time);
            $sql = "INSERT INTO social_theme_hot_num(type, theme_id,ans_cnt,praise_cnt,concern_cnt, update_time, ans_cnt_day, praise_cnt_day, concern_cnt_day) 
                    VALUES($type, $theme_id,{$cnt_ans_posts},{$cnt_praise},$cnt_concern, '{$t}', {$cnt_ans_posts}, {$cnt_praise}, {$cnt_concern}) 
                ON DUPLICATE KEY UPDATE invol_cnt_day = invol_cnt - invol_cnt_pre, ans_cnt_day = {$cnt_ans_posts} - ans_cnt, praise_cnt_day = {$cnt_praise} - praise_cnt, concern_cnt_day={$cnt_concern} - concern_cnt, update_time = '{$t}'";
            $this->social_db->query($sql);
            // 更新24小时前的参与数,回复数,点赞数,关注数
            $sql_invol_pre = "UPDATE social_theme_hot_num SET invol_cnt_pre = invol_cnt,
                    ans_cnt = {$cnt_ans_posts}, praise_cnt = {$cnt_praise}, concern_cnt = $cnt_concern,
                    hot_num = invol_cnt_day * 0.6 + ans_cnt_day * 0.3 + concern_cnt_day * 0.1
                    WHERE type = {$type} AND theme_id = $theme_id";
            $this->social_db->query($sql_invol_pre);
        }
        
        // 统计话题的浏览数,评论数,关注数,点赞数,热度值
