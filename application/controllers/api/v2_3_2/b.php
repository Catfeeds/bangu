...
class B extends APP_Controller {
	...
	public function E_expert_info() 
	{
		...
		$sql = "
			SELECT 
						e.id AS e_id,e.small_photo,e.realname,e.nickname,e.mobile,e.expert_dest as expert_destid,e.amount,
						e.sex as sexid,CASE WHEN e.sex=0 THEN '女' WHEN e.sex=1 THEN '男' WHEN e.sex=-1 THEN '保密' END AS sex,e.idcard,e.idcardpic,
		                a.name AS city_name ,e.city,(select GROUP_CONCAT(kindname SEPARATOR '、') from u_dest_cfg where FIND_IN_SET(id,e.expert_dest) >0 )as expert_dest
		    FROM		
						u_expert AS e 
	...
	public function E_update_order_price()
	{
		...
		$num=$this->db->query("select id from u_order_price_apply where expert_id='{$e_id}' and order_id='{$orderid}' and status=0")->num_rows();
		...
		{
			$this->__errormsg('价格申请已提交，不能重复申请');
		}
	}
	/**
