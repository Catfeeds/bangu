...
class C extends APP_Controller {
	...
	public function cfgm_home() {
		...
		$dest_sql = "select cd.dest_id as id,cd.dest_type,ud.kindname as destname,cd.name as description,cd.pic as pic,ud.line_num as num   		from cfgm_hot_dest as cd  		left join u_dest_cfg as ud on ud.id=cd.dest_id 	left join cfg_index_kind as cik on cik.id=cd.dest_type 		where cd.is_show='1' and cd.startplaceid={$cityid}";
		...
		$line_sql = "SELECT cx.line_id as id,cx.dest_type,cx.pic as pic,l.linetitle as linetitle,l.linename as linename,l.satisfyscore,l.bookcount,l.peoplecount,l.comment_count,l.lineprice,l.saveprice,l.recommend_expert as expert_id,e.small_photo,e.nickname   	FROM  	cfgm_hot_line AS cx LEFT JOIN u_line AS l ON cx.line_id=l.id     	left join cfg_index_kind as cik on cik.id=cx.dest_type left join u_expert as e on e.id=l.recommend_expert  	where l.status='2' and cx.is_show=1 and cx.starttime<='{$time}' and cx.endtime>='{$time}' and cx.startplaceid={$cityid}";
		$jw_line_sql = $line_sql . " and cx.dest_type=1 order by cx.showorder";
		$gn_line_sql = $line_sql . " and cx.dest_type=2 order by cx.showorder";
		$zb_line_sql = $line_sql . " and cx.dest_type=3 order by cx.showorder";  //合并
	...
	public function cfgm_line_list() {
		...
		//12、sql语句
 		...
 		if(empty( $data['line_list'] )){  $this->__data($data ['line_list'],array('title'=>$title));}
	...
	public function cfgm_hot_area() 
	{
		...
		//1、境外排名第一
		$jw=$this->cfgm_hot_dest_model->hot_dest_list(array('dest_type'=>'1'),"0","1");
		//2、国内排名第一
		$gn=$this->cfgm_hot_dest_model->hot_dest_list(array('dest_type'=>'2'),"0","1");
		//3、周边排名第一
		$zb=$this->cfgm_hot_dest_model->hot_dest_list(array('dest_type'=>'3'),"0","1");
	...
	public function cfgm_expert_list() {
		...
		$sql_init="
				SELECT
						...
						eg.title AS grade,e.attention_count,
	...
	public function cfgm_line_detail() 
	{
		...
		//$l_id="918";
		if(!$l_id) 		$this->__errormsg('线路id不能为空');
		if($usertype!="0"&&$usertype!="1")   $this->__errormsg('usertype参数有误');
...
WHERE `tn`.`id` = {$eid} ORDER BY `tnr`.`addtime` desc      limit {$from},{$page_size}  ";
	...
	public function cfgm_my_collection() 
	{
	    ...
	    //$type="1";
	    if(empty($type))  $this->__errormsg("type值不能为空");
	    // 1、我收藏的线路
		if ($type == '1') 
		{
			$sql = "SELECT 
		...
		else  //2、我收藏的管家
		{
			$sql = " 
					...
					ORDER BY 
							    ec.addtime  limit {$from},{$page_size}       
					";
			$reDataArr = $this->db->query ($sql)->result_array ();
			if (empty ( $reDataArr )) {
				$this->__outmsg ( $reDataArr );
	...
	public function cfgm_my_coupon() 
	{
		...
		if(empty($reDataArr ['result']))  $this->__data($reDataArr ['result']);
	...
	public function cfgm_custoimze_detail_update_status() {
		$this->load->model ( 'app/u_member_model', 'member' );
	...
	public function cfgm_offline_pay() 
	  {
		...
		if (empty ( $offline_num_before )) 		    {$this->__errormsg ( "银行卡前四位数字必填！" );}
		if (empty ( $offline_num_after )) 		    {$this->__errormsg ( "银行卡后四位数字必填！" );}
	...
	public function cfgm_find_city() {
		...
		//$city="深圳";
		if (!$city) $this->__errormsg('城市名称为空');
   ...
   /**
	...
	public function cfgm_expert_travel_note_detail() 
	{
		...
		//$note_id="139";
		is_numeric ( $note_id ) ? ($note_id) : $this->__errormsg ( '管家标识不能为空！' );
		//游记=》管家部分
		$reDataArr ['expert'] = $this->db->query (
				"SELECT 
						     e.id AS expert_id,tn.is_show,e.small_photo AS small_photo,e.nickname AS e_name,
							...
							tn.praise_count AS praise_count,tn.comment_count,tn.content AS content 
				FROM 		
							travel_note AS tn 
	...
	public function cfgm_expert_praise() {
		...
		if(!$eid)  $this->__wapmsg( 'param missing',$callback );
		if(!$token && !$mid) $this->__wapmsg ( 'param missing',$callback );
	...
	public function cfgm_to_suggest()
	{
		...
		//$content="测试";
		if(empty($content))  $this->__errormsg('请填写反馈的内容');
	...
	public function cfgm_version_check()
	{
		...
		//$version = "1.5.7";
		if(empty($version))  $this->__errormsg('版本号不能为空');
 ...
 /**
     ...
     public function G_scenic_list()
     {
     	...
     	//$belong_id="15";
     	if(!$belong_id) $this->__errormsg('belong_id不能为空');
     	$output=array();
     	$scenic_sql="select
     ...
     public function G_scenic_detail()
     {
     	...
     	//$m_id="291";
     	//$spot_id="1032";
     	if(!$spot_id) $this->__errormsg('景区id不能为空');
     	$output=array();
     	$scenic_sql="select s.*,b.city as area_name,(select id from scenic_spot_praise_record where spot_id=s.id and member_id={$m_id}) as is_zan from scenic_spot as s left join scenic_spot_belong as b on b.id=s.belong_id left join u_area as a on a.id=s.area_id where s.id={$spot_id}";
     	$output=$this->db->query($scenic_sql)->row_array(); //景点
     	if(empty($output))  $this->__errormsg('景点不存在');
     	...
     	select 
     			...
     			(select type from scenic_spot_review_praise where review_id=r.id and member_id={$m_id}) as zan_or_cai
     	...
     	order by r.addtime desc limit 5")->result_array(); //景点评论
        ...
        $legnth="5";//5公里范围
     	...
     	if($need_num>0)
     	{
     		...
     		$geohash=substr($output['geohash'],0,1); //将距离放大至无穷
     		$add=$this->F_scenic_zb($geohash,$limit=$need_num,$not_in); //$geohash 5公里范围，不包括“已符合”的景区
     		$add=$this->geohash_sort($output['latitude'], $output['longitude'], $add);  //对周边景点从近到远排序
     		//var_dump($not_in);var_dump($add);
     	}
     	$output['spot_zb']=array_merge($output['spot_zb'],$add);
     	
     	//推荐线路
     	//$area_name="深圳市";
     	//$area_name=mb_substr($area_name, 0, -1);
     	$output['area_name']=$area_name;
     	$destinations=$this->db->query("select id from u_dest_cfg where kindname like '%{$area_name}%'")->row_array();
     	...
     	if(!empty($destinations))
     	{
	     	$output['recommend']=$this->db->query("
	     			...
	     			order by ordertime desc limit 6
     	...
     	$output['recommend']=$this->db->query("
     			    ...
     			    from 
     					    u_line as l
     			    where status=2 and (FIND_IN_SET(2,l.overcity)>0) order by l.peoplecount desc limit 6")->result_array();
     	}
     ...
     public function G_more_comment()
     {
     	...
     	$comment_list=$this->db->query("
     		select 
     				r.*,m.litpic,(select type from scenic_spot_review_praise where review_id=r.id and member_id={$m_id}) as zan_or_cai
     ...
     public function G_add_comment()
     {
		...
		$m_id=$this->F_get_mid($token);//根据token获取mid
		//$m_id="291";
		if(!$content||!$spot_id) $this->__errormsg('评论内容不能为空');
     ...
     public function G_to_praise()
     {
     	...
     	//$comment_id="4";
     	//$flag="1";
     	if(!$comment_id) $this->__errormsg('评论id不能为空');
     	if($flag!="0"&&$flag!="1") $this->__errormsg('参数flag缺失');
     ...
     public function G_destinations_m() 
     {
     	...
     	if(!$cityid)  $this->__errormsg('城市id传入有误');
     	...
     	//周边游
     	$sql_zb= "select neighbor_id as id,d.kindname as name FROM  cfg_round_trip as t left join u_dest_cfg as d on d.id=t.neighbor_id where  t.isopen=1 and d.isopen=1 and startplaceid='{$cityid}' order by t.id desc";
     ...
     public function G_nine_dest() 
     {
     	...
     	$from = ($page - 1) * $page_size;
     	//$dest_id="10433";
     	if(!$dest_id)  $this->__errormsg('目的地id传入有误');
         
     	
     	//目的地对应的9个景区
     	$scenic_sql="select
     						s.id,s.city_id,s.name,mainpic,d.kindname
     				 from
     						scenic_spot as s
     						left join u_dest_cfg as d on s.city_id=d.id
     				where
     ...
     public function G_scenic_zb()
     {
        ...
        //$lat="22.546162";
     	...
     	if(!$lat || !$lng ) $this->__errormsg('经度和维度不能为空');
