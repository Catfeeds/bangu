...
/*			foreach ( $anchor_pic_arr as $key => $val ) {				
	...
	*/
	 ...
	 * @author: xml
	 * @param:视频标签 (attr_id:标签id),$user_id:用户id；$user_type:用户类型 0:用户 ,1:管家 
	 * @return:action_video:正在直播
	 */
	function live_video_most(){
                     $attr_id=$this->input->post('attr_id',true);//标签id
                     $user_id=$this->input->post('user_id',true);
                     $user_type=$this->input->post('user_type',true);
                     $anchor=$this->anchor_model->select_table('live_anchor',array('user_id'=>$user_id,'user_type'=>$user_type,'status'=>1));
                     if(!empty($anchor[0])){
                              $anchor_id=$anchor[0]['anchor_id'];
                     }else{
		      $anchor_id=0;
                     }
			...
			$action_video=$this->video_model->live_video_most($action_where,1);
                                    if(!empty($action_video)){
                                    	foreach ($action_video as $key => $value) {
                                    		$url=$this->get_web_url();
 				            $action_video[$key ]['anchor_photo']=$url.$value['anchor_photo'];
                                    	}
			...
			//正在直播的视频
			$action_where=" where  lr.live_status=1 ";		
			$action_video=$this->video_model->live_video_most($action_where,1);
			if(!empty($action_video)){
                                    	foreach ($action_video as $key => $value) {
                                    		$url=$this->get_web_url();
	 ...
	 */
      	function anchor_video_msg(){
			...
			$anchor_id=0;
      		...
      		//if($user_id>0){
		$follow_video=$this->video_model->follow_live_video($user_id,24);
		$outdata=array(
	 ...
	 * @author: xml
	 * @param: $anchor_id:主播id 
	 * @return:gift_sum:打赏,fans_video:粉丝,  video:视频 ,anchor_attr:我的标签,anchor_pic:我的图片,anchor:用户信息
	 */
            function direct_seeding(){
            	...
            	if($anchor_id>0){
            	         ...
            	         $anchor=$this->anchor_model->select_table('live_anchor',array('anchor_id'=>$anchor_id,'status'=>1));
                                 if(!empty($anchor[0])){
                                 	 	$url=$this->get_web_url();
                                 	 	if(!empty($anchor[0]['photo'])){
					$anchor[0]['photo']=$url.$anchor[0]['photo'];
				}
				if(!empty($anchor[0]['video_pic'])){
					$anchor[0]['video_pic']=$url.$anchor[0]['video_pic'];
				}

                                 		//主播获取的打赏
				$gift=$this->video_model->get_gift_record($anchor_id);
				$anchor[0]['gift_sum']=$gift['gift_sum'];
			           //主播的粉丝
			//	$fans_video=$this->video_model->live_anchor_fans(array('anchor_id'=>$anchor_id));

				//播放视频
				$order='addtime desc';
				$video=$this->video_model->select_Alltable('live_video',array('anchor_id'=>$anchor_id),'');
	...
	*/
	function  show_video_msg(){
		...
		if($room_id>0){
			...
			if(!empty($room_video)){
				$url=$this->get_web_url();
				foreach ($room_video as $key => $value) {
					$room_video[$key]['anchor_photo']=$url.$value['anchor_photo'];
				}
			}
			//打赏
...
/*          function get_video_room(){
			...
			$this->__errormsg('获取数据失败!');
          	...
          	/**
	*@method 人气主播和热门目的
	*@param cityid(当前的城市id )
	*@return array
	*/
	function get_dest_anchor(){
		//热门目的
		...
		//周边游
		$round_city=$this->video_model->get_round_city($cityid,'limit 4');
		//国内游
		$country_city=$this->video_model->get_country_city('limit 4');
		//出境游
		$abroad_city=$this->video_model->get_abroad_city('limit 4');
                      
		//人气主播
		$anchor=$this->video_model->get_top_anchor('limit 2');
	...
	*/
	function get_dest_most(){
		...
		//周边游
		$round_city=$this->video_model->get_round_city($cityid);
		//国内游
		$country_city=$this->video_model->get_country_city();
		//出境游
		$abroad_city=$this->video_model->get_abroad_city();
	 ...
	 */
          	function  search_video_data(){
                    	...
                    	//视频
                     	$video=$this->video_model->get_video_data($name);
                     	//直播
                     	$action_video=$this->video_model->get_action_video($name);
	 ...
	 * @return:
	 */
          	function  search_dest_video(){
          	         	$dest_id=$this->input->post('dest_id',true);
	         	$destination=$this->video_model->select_rowtable('bangu.u_dest_cfg',array('id'=>$dest_id));
	         	if(!empty($destination)){
	         	    	$name=$destination['kindname'];
	         	}else{
	         	  	$name=0;
	        	}
                     	//视频
	 ...
	 * @return:
	 */
            function  search_anchor_data(){
                     $name=$this->input->post('name',true);
                     $anchor=$this->video_model->get_anchor_data($name);
                     $outdata=array('anchor'=> $anchor);
	         $this->__outmsg($outdata);
          	}

        	/**
	 * @method:根据目的地,主播,视频  搜索相应的信息
	 * @param:$name 搜索条件
	 * @return: 
	 */
        	function search_data(){
        		$name=trim($this->input->post('name',true)); //搜索参数
        		//$name="房间";
        		//目的城市	
        		$dest=$this->video_model->get_dest($name);
        		//直播信息
        		$anchor=$this->video_model->get_anchor_data($name);
                    	//视频
                     	$video=$this->video_model->get_video_data($name);
        	...
        	*@param:$video_id
        	*/
        	function get_record_video(){
		$video_id=trim($this->input->post('video_id',true)); //视频id
		//$video_id=35;
		//录播视频信息
		$video=$this->video_model->select_rowtable('live_video',array('id'=>$video_id));
		//主播信息
		$anchor='';
		if(!empty($video['anchor_id'])){
			$anchor=$this->video_model->select_rowtable('live_anchor',array('anchor_id'=>$video['anchor_id']));
			if(!empty($anchor)){
				$url=$this->get_web_url();
 				$anchor['photo']=$url.$anchor['photo'];
			}	
		}
		//推荐线路
		$line=$this->video_model->get_recond_line();
