...
class Z extends APP_Controller {
	...
	public function getUserInfo()
	{
		...
		$anchorData = $this ->anchorModel ->getAnchorId($user_id ,$user_type);
		if (empty($anchorData))
		{
			$this->__nullmsg();
		}
	...
	/**
	 * @method 获取主播表(anchor)的ID
	 * @param unknown $user_id member表的会员ID
	 * @param unknown $user_type anchor表的会员类型
	 */
	protected function getAnchorID($user_id ,$user_type)
	{
		$this ->load_model('live/anchor_model' ,'anchorModel');
		$anchorData = $this ->anchorModel ->getAnchorId($user_id ,$user_type);
		if (empty($anchorData) || $anchorData['anchor_id'] < 1)
		{
			return false;
		}
		return $anchorData['anchor_id'];
	}
	...
	{
		$user_id = $this ->input ->post('user_id');//member表会员ID
		$user_type = intval($this ->input ->post('user_type'));//anchor表会员类型
		$money = floatval($this ->input ->post('money'));
// 		$user_id = 224;
// 		$money = 2;
// 		$user_type = 1;
		$anchor_id = $this ->getAnchorID($user_id, $user_type);
		if ($anchor_id == false)
		{
			$this ->__errormsg('会员账号不存在');
		}
	...
	{
		$user_id = intval($this ->input ->post('user_id')); //member表用户ID
		$user_type = intval($this ->input ->post('user_type'));//会员类型
...
//  	$gift_id = 1;
		...
		if ($user_id < 1 || $anchor_id < 1 || $room_id < 1 || $user_type < 0)
		{
		...
		//查询会员
		$userData = $this ->anchorModel ->getAnchorId($user_id ,$user_type);
		if (empty($userData))
		{
		...
		if ($userData['umoney'] < $giftUMoney)
		{
			...
			$money = round($recharge_u/sys_constant::URATE ,2);//不足的U币等值的人民币
			$dataArr = array(
					'user_id' =>$userData['anchor_id'],
					'money' =>$money,
					'status' =>0,
					'addtime' =>date('Y-m-d H:i:s' ,time())
			);
		...
		$buyGift = array(
				'gift_id' =>$gift_id,
				'anchor_id' =>$anchor_id,//当前直播的主播ID
				'room_id' =>$room_id,
				'room_code' =>$roomData['room_code'],
				'user_id' =>$userData['anchor_id'],//购买礼物的会员的主播表ID
				'num' =>$num,
				'worth' =>$giftUMoney,
				'addtime' =>date('Y-m-d H:i:s' ,time()),
				'pic' =>$giftData['pic']
		...
		$status = $this ->anchorModel ->userBuyGift($userData['anchor_id'] ,$buyGift);
		if ($status === FALSE)
		{
			...
			$contentArr = $this ->__dealData($contentArr);
			...
			//贡献排行缓存
			$userUmoney = $this->cache->redis->zscore ('rankingrl_'.$roomData['room_code'].$room_id ,$userData['anchor_id'].'#0#'.$userData['name']);
			$countUmoney = round($userUmoney + $giftUMoney);
			$this->cache->redis->zadd('rankingrl_'.$roomData['room_code'].$room_id ,$countUmoney ,$userData['anchor_id'].'#0#'.$userData['name']);
	...
	public function insertChat()
	{
		...
		$user_type = intval($this ->input ->post('user_type'));//用户类型
		...
		if ($room_id <1 || $user_id < 1 || $user_type <0 || empty($nickname) || empty($room_code))
		{
			$this ->__errormsg('缺少参数');
		}
		...
		if ($num > 0) {
			...
			//记录聊天人数
			$this->cache->redis->sadd('chat_people_'.$room_code.$room_id ,$user_id.$user_type);
...
// 		$user_id = 1;
// 		$user_type = 1;
		if ($user_id < 1 || $user_type < 0) {
			$this ->__errormsg('缺少用户参数');
		}
		$user_anchor_id = $this ->getAnchorID($user_id, $user_type);
		if ($user_anchor_id == false)
		{
			$this ->__errormsg('会员不存在');
		}
		$page = $page < 1 ? 1 :$page;
		$pagesize = $pagesize < 1 ? 10 :$pagesize;
		$this ->load_model('live/live_recharge_model' ,'rechargeModel');
		$rechargeData = $this ->rechargeModel ->getAppRechargeData($user_anchor_id ,$page ,$pagesize);
...
// 		$user_type = 1;
		...
		$recordData = $this ->recordModel ->getAppRecordData($user_anchor_id ,$page ,$pagesize);
		if (empty($recordData))
		{
			$this->__nullmsg();
		} else {
