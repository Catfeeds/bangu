...
class B extends APP_Controller {
	...
	public function E_expert_login() 
	{
		...
		if ($res_query) 
		{
			...
			if (md5 ( $password ) == $res_query ['password'] && $res_query ['status'] == "2") 
			{
				...
				if($registrationId)
				{
					$this->u_expert_model->update(array('equipment_id'=>$registrationId),array('id'=>$res_query['id']));
				}
	...
	public function E_customize_reply() {
		...
		$sql="SELECT
				...
				(select id as num from u_customize_answer where expert_id='{$e_id}' and customize_id=c.id limit 1) as cr_answer_id,
	...
	public function E_customize_list() {
		...
		$code_where="";
		if($code=="2") //已投标
			$code_where.="ca.expert_id={$e_id} AND c.user_type=0 AND ca.isuse=0 AND ISNULL(ca.replytime) = 0 AND ISNULL(ca.replytime) = 0";
		elseif($code=="3") //已中标
		...
		$sql="
		select * from (SELECT
		c.id,ca.addtime,c.startplace,c.endplace,(select GROUP_CONCAT(cityname) from u_startplace where FIND_IN_SET(id,c.startplace)>0) as startplace_CH,
...
/**
	...
	public function E_customize_answer()
	{
		...
		//3、回复、定制、方案关联表
		$one=$this->db->query("select id from u_customize_reply where customize_id={$c_id} and expert_id={$e_id} and is_allow=0 and status=1")->num_rows();
		if(empty($one))
		{
			$data3=array(
				'customize_id'=>$c_id,
		...
		if($this->db->trans_status() === TRUE){
			$this->db->trans_commit();
			$this->__data(array('msg'=>'回复成功','data'=>$data1));
		}else{
			$this->db->trans_rollback();
			$this->__errormsg(array('msg'=>'操作失败'));
		}
	...
	public function E_customize_answer_detail()
	{
		...
		$row=$this->db->query("
				...
				select 
						c.id as cid,c.member_id,c.startplace,s.cityname as startplace_ch,c.endplace,
						(select GROUP_CONCAT(kindname) from u_dest_cfg where FIND_IN_SET(id,c.endplace)>0) as endplace_ch,
