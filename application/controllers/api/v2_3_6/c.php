...
class C extends APP_Controller {
	...
	public function cfgm_home() {
		...
		$jw_line_sql = $line_sql . " and cx.dest_type=1 order by cx.id desc";
		$gn_line_sql = $line_sql . " and cx.dest_type=2 order by cx.id desc";
		$zb_line_sql = $line_sql . " and cx.dest_type=3 order by cx.id desc";  //合并
	...
	public function cfgm_find_startplace() 
	{
		...
		//$city="深圳";
		if (!$city) $this->__errormsg('城市名称不能为空');
	...
	public function cfgm_destinations() 
	{
		...
		//$type="2";
		//$cityid="235";
		if(empty($type))   $this->__errormsg('type参数缺失');
		if($type=="3")
		{
			if(empty($cityid))   $this->__errormsg('cityid参数缺失');
		}
	...
	public function cfgm_find_destinations()
	{
		...
		//$dest="t";
		//$type="2";
		if (empty($dest)) $this->__errormsg('目的地名称不能为空');
		if (empty($type)) $this->__errormsg('type参数缺失');
	...
	public function cfgm_hot_area() 
	{
		...
		if(empty($cityid))  $this->__errormsg('cityid不能为空');
		$page = empty ( $page ) ? 1 : $page;
	    //$page="1";
...
// 		$reDataArr ['vol_old'] = $query2->result_array (); // 优惠卷已过期
	...
	public function cfgm_user_register() 
	{
		...
		if (! preg_match ( "/1[34578]{1}\d{9}$/", $mobile ))  $this->__errormsg ( '手机号码格式错误' );
...
WHERE `tn`.`id` = {$eid} ORDER BY `tnr`.`addtime` desc      limit {$from},{$page_size}  ";
	...
	public function cfgm_my_customize_list() 
	{
		...
		$sql = "
				SELECT  
						...
						(SELECT COUNT(*) FROM u_customize_answer AS u WHERE u.customize_id=c.id and u.replytime is not null) AS answer_num,
						COUNT(u.expert_id) AS expert_num,c.addtime,
						 CASE 
	...
	public function cfgm_custoimze_detail() {
		...
		//$cid="735";
		$this->check_token ( $token );
		if(!$cid)   $this->__errormsg('定制单id不能为空');
		$data = $this->db->query ( "
				    SELECT  
						c.id,c.startplace,us.cityname, 
						(select GROUP_CONCAT(kindname) from u_dest_cfg where FIND_IN_SET(id,c.endplace) >0 )as dest , 
		...
		$data ['answer'] = $this->db->query ( "  
				SELECT 
						ca.id,ca.title,ca.price_description,c.id as customize_id,ca.expert_id,e.realname,e.nickname,e.small_photo,
						ca.addtime ,ca.isuse,ca.replytime,ca.plan_design,ISNULL(ca.replytime) as reply,
						CASE WHEN ISNULL(ca.replytime) > 0 THEN '未回复方案' WHEN ISNULL(ca.replytime) = 0 THEN '已回复方案' END 'reply_CH',
				        ca.is_allow
				FROM 
				...
				WHERE 
						ca.customize_id ={$cid} " )->result_array ();
		$this->__outmsg ( $data );
	}
	/**
	 * @name：定制属性：出行方式、多项服务
	...
	public function cfgm_customize_submit2()
	{
		...
		$code="2312";*/
		//验证
		if(empty($startplace_id))  $this->__errormsg('请选择出发城市');
		if(empty($dest_id))  $this->__errormsg('请选择目的地城市');
		if(empty($trip_way))  $this->__errormsg('请选择出游方式');
		if(empty($linkname))  $this->__errormsg('请填写联系人');
		
		if(empty($mobile))  $this->__errormsg('请填写手机号码');
		$this->load->helper ( 'regexp' );
		if (! regexp ( 'mobile', $mobile )) $this->__errormsg('请填写正确的手机号码');
		
		$islogin=false; //默认没有登录状态
		if($m_id=="0")  //没登录情况下做验证
		{
			
			if($mobile!=$sess_mobile)  $this->__errormsg('所填手机号和接收验证码的手机号不一致');
			if(empty($code))  $this->__errormsg('请填写短信验证码');
			if($code!=$sess_code)  $this->__errormsg('验证码错误');
	...
	public function cfgm_custoimze_answer_detail()
	{
		...
		//$answer_id="824";
		is_numeric ( $answer_id ) ? ($answer_id) : $this->__errormsg ( '回复id不能为空！' );
		//回复详情 （一条）
		$sql = "select 
		...
		foreach ( $answer ['plan_list'] as $key => $val ) 
		{	//用于转换数组，只因数组内嵌过多，导致图片过滤无法带上域名，而后转换域名。
	...
	public function cfgm_custoimze_answer_choose() 
	{
        ...
        if($m_id!=$reDataArr['member_id'])   $this->__errormsg('该方案不归属你，无法选择');
		$sta = $reDataArr ['status'];
		if ($sta == '-3')  $this->__errormsg('操作方案失败，该定制单已过期');
		if ($sta == '3')  $this->__errormsg('操作方案失败，该定制单已完成');
		if ($sta == '-2') $this->__errormsg('操作方案失败，该定制单已取消');
		if ($sta == '1') $this->__errormsg('操作方案失败，该定制单已确定方案，不能重复选择');
		if ($sta == '0') 
		{
			$data=array (
					'status' => 1,  //已确定方案
	...
	public function cfgm_custoimze_answer_contact()
	{
		...
		//$m_id="300";
		is_numeric ( $answer_id ) ? ($answer_id) : $this->__errormsg ( '方案id不能为空' );
		if($type!="0"&&$type!="1") $this->__errormsg ( 'type参数错误' );
		...
		$reDataArr = $this->db->query ( "    select status,member_id from u_customize where id ={$customize_id}" )->row_array ();
		if($m_id!=$reDataArr['member_id'])   $this->__errormsg('该方案不归属你，无法操作');
		$sta = $reDataArr ['status'];
		if ($sta == '-3')  $this->__errormsg('操作方案失败，该定制单已过期');
		if ($sta == '3')  $this->__errormsg('操作方案失败，该定制单已完成');
		if ($sta == '-2') $this->__errormsg('操作方案失败，该定制单已取消');
		...
		if ($sta == '0')
		{
			...
			$this->db->trans_begin();
			$this->u_customize_answer_model->update(array('is_allow'=>$type),array('id'=>$answer_id));  //方案被使用
			//事务
			if ($this->db->trans_status() === FALSE)
			{
				$this->db->trans_rollback();
 ...
 /**
     ...
     public function G_scenic_praise()
     {
        ...
        //$equipment_id="190e35f7e0457002de8";
     	...
     	if(empty($equipment_id)) $this->__errormsg('设备id不能为空');
     ...
     public function G_more_recommend()
     {
     	...
     	if(empty($spot))  $this->__errormsg('景点不存在');
     ...
     public function G_add_comment()
     {
		...
		$m_id=$this->F_get_mid($token);//根据token获取mid
		//$m_id="291";
		if(empty($spot_id)) $this->__errormsg('景点id不能为空');
		if(empty($content)) $this->__errormsg('评论内容不能为空');
		$one=$this->db->query("select * from scenic_spot where id='{$spot_id}'")->row_array();
		if(empty($one))   $this->__errormsg('景点不存在');
     ...
     public function G_nine_dest() 
     {
    	...
    	//景区封面
    	$reDataArr['dest_pic']=$reDataArr['rows'][0]['dest_pic'];
	 ...
	 public function G_nine_dest_id() 
     {
     	...
     	//$area_name="深圳";
     	if(!$area_name)  $this->__errormsg('目的地传入有误');
