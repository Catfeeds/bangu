...
// 		$reDataArr ['vol_old'] = $query2->result_array (); // 优惠卷已过期
	...
	public function cfgm_add_order() 
	{
		...
		//6、下订单前验证
		if(empty($line_id)) $this->__wapmsg('线路id不能为空',$callback);
		if(empty($expert_id)) $this->__wapmsg('管家id不能为空',$callback);
		if(empty($suitid)) $this->__wapmsg('套餐id不能为空',$callback);
		if(empty($indent_day)) $this->__wapmsg('出团日期不能为空',$callback);
		...
		if($now_date>$deadline)  $this->__wapmsg('线路需提前'.$linebefore.'天预订',$callback);  //  if($deadline>$date) echo "ok";
