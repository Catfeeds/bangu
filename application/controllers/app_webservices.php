 ...
 * 				     efmg 管家接口前缀
 * 					 cfmg 用户接口前缀
 *								代码能公用的已经公用了，不能公用的，有不能公用的限制，代码整合与优化。
 *		
 *						后续开发接口，少注释，壳子都是一样，核心是sql查询语句，所以少。
 *
...
class App_webservices extends CI_Controller {
	...
	public function cfgm_find_line() {
		...
		$order_by = "";
		$result = "";
		$areaid = intval ( $this->input->post ( 'areaid', true ) );		//出发ID
// 		$city_id = $this->input->post ( 'cityid', true );						//城市ID		此处两用，一用首页跳转，一用find line
		$price = intval ( $this->input->post ( 'price', true ) );			//价格
		$days = $this->input->post ( 'day', true );								//出游日
		$dest = $this->input->post ( 'dest_id', true );						//周边
		$dest_id = ltrim ( $dest, ',' );
		$order = intval ( $this->input->post ( 'sort', true ) );				//排序
		$label_id = $this->input->post ( 'label', true ); 						// 标签多选
		...
		if ($city_name) {
			...
			if ($city_data) {
				$startcity_id = $city_data ['id'];											    //出发城市定位之后
			}
		}
		...
		$all_redataid=isset($all_redata['id'])?$all_redata['id']:'391';																						//子站所需
// 		if (! empty ( $city_id )) {
// 			$where .= "  AND FIND_IN_SET({$city_id},l.overcity)>0     ";
// 		}
		...
		if (isset ( $price )) {
			$price_arr = $this->cfg_search_condition->row ( array (
					'id' => $price 
			) );
			if (isset ( $price_arr ['minvalue'] ))
				$where .= " and (l.lineprice >= " . $price_arr ['minvalue'] . " and l.lineprice <= " . $price_arr ['maxvalue'] . ")";
		...
		if (isset ( $days )) {
			$days_arr = $this->cfg_search_condition->row ( array (
					'id' => $days 
			) );
			if (isset ( $days_arr ['minvalue'] ))
				$where .= " and  (l.lineday >= {$days_arr['minvalue']} and l.lineday <= {$days_arr['maxvalue']})";
		...
		if ($label_id) {
 		...
 		if (! empty ( $order )) {			$wherekk="";			}else{			$wherekk="AND l.producttype = 0";		}			//内置条件，专防排序。
 		if(!empty($startcity_id)){
 			$where.="  and ( FIND_IN_SET({$startcity_id},ls.startplace_id)>0	";
 		}

	 	if(! empty($areaid))		{												
 			$wkere="	LEFT JOIN u_line_startplace AS ls ON l.id=ls.line_id";
 			$where .= "   and  FIND_IN_SET({$areaid},l.overcity)>0 ";
 			if(!empty($startcity_id)){
 				$where.=")";
 			}
 			...
 			$wkere=" LEFT JOIN u_line_startplace AS ls ON l.id=ls.line_id ";															//如果$areaid有值，则让这两个条件消失，除此之外的条件，全部带有。
 			$where .="  or FIND_IN_SET({$all_redataid},ls.startplace_id)>0";	
 			if(!empty($startcity_id)){
 			$where.=")";
 			}
 		}

 		$sql1=   "   SELECT l.id,l.linename,l.linetitle,l.mainpic,l.lineprice,satisfyscore,l.comment_count as comments,l.peoplecount as bookcount, 	(SELECT	COUNT(*) FROM u_member_order AS mo WHERE l.id = mo.productautoid AND mo. STATUS > 4 ) AS volume  	FROM u_line AS l  {$wkere}  WHERE l.STATUS = 2 and l.producttype =0 {$where}  {$wherekk}  GROUP BY ls.line_id  {$order_by}    	 LIMIT {$from},{$page_size}     ";
 		$query =  $this->db->query( $sql1 );
 		$data['line_list'] = $query->result_array ();
 

 		if(empty( $data['line_list'] )){  $this->__outmsg ($data['line_list']);}
 		$sql2= rtrim($sql1, "LIMIT {$from},{$page_size} ");
 		$query =  $this->db->query( $sql2 );
 		$data ['total_rows'] = $query->num_rows ();
 		
// 		$data ['line_list'] = $this->db->query ( "	 SELECT l.id,l.linename,l.linetitle,l.mainpic,l.lineprice,satisfyscore,l.comment_count as comments,l.peoplecount as bookcount, 	(SELECT	COUNT(*) FROM u_member_order AS mo WHERE l.id = mo.productautoid AND mo. STATUS > 4 ) AS volume  	FROM u_line AS l  {$wkere}  WHERE l. STATUS = 2  {$where}  {$wherekk}    {$order_by}    	 LIMIT {$from},{$page_size}    " )->result_array ();
...
// 		}
		...
		// 查询总记录数
// 		$result = $this->db->query ( "	             select count(*) as cont FROM (SELECT l.id,l.linename,l.linetitle,l.mainpic,l.lineprice,l.satisfyscore,(SELECT COUNT(*) FROM u_comment AS c WHERE c.line_id = l.id ) AS comments,(SELECT	SUM(mo.dingnum + mo.childnum+mo.childnobednum+mo.oldnum) FROM u_member_order AS mo WHERE	l.id = mo.productautoid AND mo. STATUS > 4) AS people,(SELECT	COUNT(*) FROM u_member_order AS mo WHERE l.id = mo.productautoid AND mo. STATUS > 4 ) AS volume FROM u_line AS l      {$wkere}  WHERE l. STATUS = 2  {$where}  {$wherekk}    {$order_by}      " )->result_array ();
...
// 		$reDataArr ['vol_old'] = $query2->result_array (); // 优惠卷已过期
	...
	/*
	 *
	 * 		do line Details Collection (ps : old) 线路详情		暂时不用
	 *
	 * 		by zhy
	 *
	 * 		at
	...
	/*
	 *
	 * 		do Line Details Collection 收藏线路详情
	 *
	 * 		by zhy
	 *
	 * 		at
	...
	public function cfgm_add_order() {
		...
		$data ['dingnum'] = $this->input->post ( 'adult_number' );								//总人数
		...
		$data ['order_price'] = $data ['dingnum'] * $data ['price'] + $data ['childnum'] * $data ['childprice'] + $data ['oldnum'] * $data ['oldprice'] + $data ['childnobednum'] * $data ['childnobedprice']+$insurance_price;
       ...
       逻辑有误
		...
		if(!empty($exp_id)){
			...
			if(	$expertDatapn['supplier_id']==$lineData ['supplier_id']	){
					...
					$data ['agent_fee']=round(($data ['total_price'] * $lineData ['agent_rate']) ,2);
			}else{
					$data ['agent_fee']= 0;
			}
		}else{
		...
		if ($update) // 送积分成功后，才存记录
{
	...
	/*
	 *
	 * 		do customize update status		定制单取消
	 *
	 * 		by zhy
	 *
...
WHERE `tn`.`id` = {$eid} ORDER BY `tnr`.`addtime` desc      limit {$from},{$page_size}  ";
	...
	/*
	 *
	 * 		do pay trip evaluate refund 	用户订单状态总条数
	 *
	 * 		by zhy
	 *
	 * 		at 2015年x月x日 x:x:x
	...
	public function cfgm_action_order() {
		...
		if ($pj) { // 评价
			...
			$this->order_status_model->update_order_status_cal($pj);
		}
		if ($ts) { // 投诉
			$log ['content'] = "投诉成功";
			$data ['order_id'] = $ts;
	...
	/*
	 *
	 * 		do Pay Preferential volume 支付宝支付
	 *
	 * 		by zhy
	 *
	 * 		at 2015年x月x日 x:x:x
	...
	public function cfgm_pay_order() {
		...
		$mid = $result [0] ['mid'];
		$reDataArr = $this->db->query ( "		SELECT `mo`.`ordersn`, `mo`.`productname`, `mo`.`dingnum` AS adult, `mo`.`childnum` AS child, mo.childnum,mo.childnobednum,mo.oldnum,mo.dingnum,		(select l.linetitle from u_line AS l left join u_member_order AS mo on mo.productautoid=l.id where mo.id={$order_id}) AS producttitle, 		mo.total_price as order_price,`mo`.`jifenprice`,mo.couponprice,mo.order_price as ctf,  	(select SUM(amount) from u_order_insurance as o WHERE o.order_id={$order_id}) as amount, 		(select count(*) from cou_member_coupon AS mcou left join cou_coupon AS cou on cou.id=mcou.coupon_id where cou.endtime < NOW() and cou.status=0 and cou.userid={$mid}) AS yhj_count  	FROM (`u_member_order` AS mo)  	WHERE `mo`.`id` =  {$order_id} AND `memberid` =  {$mid}  " )->result_array ();
		$this->__outmsg ( $reDataArr );
	}
	...
	public function cfgm_offline_pay() {
		...
		// 判断订单是否存在
		$order_info = $this->order_model->row ( array (
				'id' => $order_id 
		) );
		if (empty ( $order_info )) {
		...
		// 验证线路
		$line_info = $this->line_model->row ( array (
				'id' => $order_info ['productautoid'],
				'status' => 2 
		) );
		if (empty ( $line_info )) {
			$this->__errormsg ( "您选择的旅游线路不存在或已下架，请咨询您的旅游专家！" );
		}
		...
		switch ($order_info ['ispay']) {
			case 0 : // 没有支付
				if ($order_info ['status'] == 1) { // B1已留位可以支付
				                                  // 验证支付金额
				    if (($min_pay_money-$pay_amount)>0.001) { $this->__errormsg ( "支付金额最少为：￥{$min_pay_money}，请您重新填写支付金额" ); }
				  elseif (($pay_amount - $order_info ['total_price']) > 0.001) {
		...
		// $status =$this->order_model->insert_tabledata('u_order_detail',$insert_data);
		if ($status) {
	...
	/*
	 *
	 * 		do pay city		对应支付时候的城市插件数组
	 *
	 * 		by zhy
	 *
	 * 		at 2016年3月2日 10:54:00
	 *
	 */
	...
	/*
	 *
	 * 		do show user data 展示用户信息
	 *
	 * 		by zhy
	 *
	 * 		at	2016年3月15日 15:25:09
	 *
	 */
	...
	/*
	 *
	 * 		do modify password 修改密码
	 *
	 * 		by zhy
	 *
	 * 		at 2015年11月24日 16:41:51
	 *
	 */
	...
	/*
	 *
	 * 		do updata password 更新密码
	 *
	 * 		by zhy
	 *
	 * 		at 2015年10月24日 16:41:51
	 *
	 */
	...
	public function cfgm_line_sort() {
		...
		// }
		// if ($dest_more) {
		// $sql = "select sec.name as sec_name,sec.id AS sec_id,a.id as th_id,a.name as th_dest from cfg_index_kind_dest as sec left join cfg_index_kind as f on f.id=sec.index_kind_id left join u_area as a on a.pid=sec.id where a.level=3 and f.id={$dest_id}";
		// $query = $this->db->query ( $sql );
		// $reDataArr = $query->result_array ();
		// if ($reDataArr) {
		// foreach ( $reDataArr as $key => $val ) {
		...
		// $this->__outmsg ( $reDataArr );
		// } else {
		// $this->db->select ( 'f.id AS f_id,f.name AS f_name,th.id AS th_id,d.kindname AS th_dest' );
		// $this->db->from ( 'cfg_index_kind AS f' );
		// $this->db->join ( 'cfg_index_kind_dest AS sec', 'f.id=sec.index_kind_id', 'left' );
		// $this->db->join ( 'cfg_index_kind_dest_line AS th', 'sec.id=th.index_kind_dest_id', 'left' );
		// $this->db->join ( 'u_line AS l', 'l.id=th.line_id', 'left' );
		// $this->db->join ( 'u_dest_cfg AS d', 'd.id=l.overcity', 'left' );
		// $where = array(
		// 'f.id' => $dest_id
		// );
		// $this->db->where ( $where );
	...
	/*
	 *
	 * 		do Show Experience Division 管家展示信息
	 *
	 * 		by zhy
	 *
	 * 		at 2016年3月14日 18:08:50
	 *
	 */
	...
	/*
	 *
	 * 		do Experience Division detail 管家展示信息
	 *
	 * 		by zhy
	 *
	 * 		at 2016年3月14日 18:08:45
	 *
	 */
	...
	/*
	 *
	 * 		do Experience Division info 管家展示信息糅合游记
	 *
	 * 		by zhy
	 *
	 * 		at
	...
	/*
	 *
	 * do museum message 照相馆
	 *
	 * by zhy
	 *
	 * at 2015年12月04日 16:35:59
	 *
	 */
	...
	/*
	 *
	 * 		do Verification 管家注册验证第一页
	 *
	 * 		by zhy
	 *
	 * 		at 2015年12月05日 08:32:43
	 *
	 */
	public function cfgm_expert_register_p1() {
		...
		$code = $this->input->post ( 'code', true ); // 验证码
		                                                   // $reads = $this->input->post ( 'reads', true ); //用户阅读协议，勾选为1，不勾为空
	...
	/*
	 *
	 * 		do Verification 验证		管家注册验证第一页
	 *
	 * 		by zhy
	 *
	 * 		at 2015年12月05日 09:32:43
	 *
	 */
	public function cfgm_expert_register_p2() {
		...
		// if (empty($museum)) { $this->__errormsg ('请选择免费照相馆！');}
	...
	/*
	 *
	 * 		do expert register 管家注册
	 *
	 * 		by zhy
	 *
	 * 		at 2015年12月05日 12:32:43
	 *
	 */
	public function cfgm_expert_register() {
		...
		$code = $this->input->post ( 'code', true ); // 验证码
		 // page2
		$phone = $this->input->post ( 'phone', true ); // 验证码
		$nickname = $this->input->post ( 'nickname', true ); // 用户昵称
		$sex = $this->input->post ( 'sex', true ); // 性别
		...
		if ($reg_type == 1) {
			...
			$code_mobile = $this->session->userdata ( 'mobile' );
			$code_number = $this->session->userdata ( 'code' );
			if (! preg_match ( "/1[34578]{1}\d{9}$/", $login_name )) 							{	$this->__errormsg ( '手机号码错误！' ); }else{
				$result_user = $this->db->query ( "select  status  from u_expert  where mobile={$login_name}  ORDER BY `id` desc " )->result_array ();
				if (! empty ( $result_user [0] ['status'] )) {
					if ($result_user [0] ['status'] == 1) {
						$this->__errormsg ( '该手机号正在注册中，请不要重复注册！' );
					} elseif ($result_user [0] ['status'] == 2) {
						$this->__errormsg ( '该手机号已经注册成功，请勿重复注册！' );
			...
			if (empty ( $code_number ) && $noCheckCode == false) 						{	$this->__errormsg ( '请获取验证码! ' );  }
		} else {
			$kk = $this->session->userdata ( 'email_code' );
			$code_number = $kk ['code'];
			$code_mobile = $kk ['email'];
			...
			if (empty ( $code_number ) && $noCheckCode == false) {
				$this->__errormsg ( '请获取验证码!' );
			}
		}
		...
		if ($noCheckCode == false && ($code_mobile != $login_name || $code_number != $code)) // 需要进行验证码验证，且验证码是正确的
{
			$this->__errormsg ( '验证码错误！' );
		} else {
			$this->db->trans_begin ();
			...
			$data = array (
					...
					'isstar' => 0,
					'nickname' => $nickname,
					'idcardtype' => isset ( $idcardtype ) ? $idcardtype : '' 
			);
			...
			/*
			 * else{
			 * $this->__errormsg('荣耀证书有误！');
			 * }
			 */
			// 照片二维码
			if (! empty ( $museum )) { // 管家关联相馆表
	...
	/*
	 *
	 * 		do all city domestic abroad ... 全部城市
	 *
	 * 		by zhy
	 *
	 * 		at
	...
	/*
	 *
	 * 		do b2expert login 管家登录
	 *
	 * 		by zhy
	 *
	 * 		at 2015年12月14日 16:30:46
	 *
	 */
		...
		if ($res_query) {
			...
			if (md5 ( $password ) == $res_query ['password'] && $res_query ['status'] == "2") {
				...
				exit ();
				  ...
				  echo json_encode(array('code'=>1,'msg' =>'平台终止了与您的合作，请联系客服!',));
			} elseif (md5 ( $password ) == $res_query ['password'] && $res_query ['status'] == "1") {
				echo json_encode(array('code'=>1,'msg' =>'该帐号在审核中，请耐心等待!',));
			} elseif (md5 ( $password ) == $res_query ['password'] && $res_query ['status'] == "3") {
				echo json_encode(array('code'=>1,'msg' =>'该帐号审核被拒绝，请联系客服!',));
			} else {
	 ...
	 *
	 */
	public function cfgm_order_list() {
		$token = $this->input->post ( 'number', true );
		$this->expert_check_token ( $token );
		$this->load->model ( 'common/u_expert_access_token_model', 'eat_model' );
		...
		$sql = "SELECT mo.id AS mo_id,e.id AS e_id,mo.ordersn,mo.productname,(mo.dingnum+mo.childnum+mo.childnobednum+mo.oldnum) AS  people,mo.usedate,mo.total_price,l.mainpic,mo.ispay,mo.status
FROM u_member_order AS mo  LEFT JOIN u_expert AS e ON mo.expert_id=e.id LEFT JOIN u_line AS l ON mo.productautoid=l.id WHERE mo.expert_id={$m_id} {$where} ORDER BY mo.id desc";
		$query = $this->db->query ( $sql );
		$reDataArr = $query->result_array ();
		if ($reDataArr) {
			foreach ( $reDataArr as $key => $val ) {
	...
	public function cfgm_order_detail() {
		...
		$sql = "SELECT mo.id,mo.ordersn,mo.productname,s.cityname,mo.oldnum,mo.childnobednum,mo.usedate,mo.dingnum,mo.childnum,mo.total_price,mo.agent_fee,mo.linkman,mo.linkmobile,	CASE WHEN mo.isneedpiao=1 THEN '是' WHEN mo.isneedpiao=0 THEN '否' END AS is_fp,		mo.ispay as pay_status,mo.status  as syatus	FROM u_member_order AS mo LEFT JOIN u_line AS l ON mo.productautoid=l.id LEFT JOIN u_startplace AS s ON l.startcity=s.id WHERE mo.id={$mo_id}";
		$query = $this->db->query ( $sql );
		$result = $query->result_array ();
	...
	/*
	 *
	 * 		do order tourist message 订单中游客信息
	 *
	 * 		by zhy
	 *
	 * 		at
	...
	public function cfgm_customize_manage() {
		...
		$e_id = $user [0] ['eid'];  
		$sql = "		  SELECT   e.id AS e_id, e.status AS e_status,CASE WHEN c.isshopping=1 THEN '有购物' WHEN c.isshopping=0 THEN '无购物' END shop_status,
    (SELECT   ca.id FROM  u_customize_answer AS ca  WHERE ca.customize_id = c.id    AND ca.expert_id = 1     AND ca.enquiry_grab_id = 0   ORDER BY ca.addtime DESC   LIMIT 1) AS ca_id,
    ca.expert_id,   c.id AS c_id,   c.startdate AS startdate,   c.estimatedate AS estimatedate, c.budget AS budget,(select GROUP_CONCAT(kindname) from u_dest_cfg where FIND_IN_SET(id,c.endplace) >0 )as endplace,
    (SELECT    st.cityname    FROM     u_startplace AS st   WHERE st.id = c.startplace) AS startplace,
    (SELECT     GROUP_CONCAT(d.kindname)   FROM    u_dest_cfg AS d   WHERE FIND_IN_SET(d.id, c.endplace) > 0    AND d.level = 3) AS dest,
    c.days AS days,   c.total_people AS total_people,   c.trip_way AS trip_way,   c.another_choose AS another_choose,   c.custom_type AS custom_type,   c.addtime AS ADDTIME
  FROM   u_customize AS c 
    LEFT JOIN u_customize_answer AS ca 
      ON (
        c.id = ca.customize_id 
        AND ca.expert_id = {$e_id}
      ) 
    LEFT JOIN u_customize_dest AS cd 
      ON c.id = cd.customize_id 
    LEFT JOIN u_enquiry AS e 
      ON (
        c.id = e.customize_id 
        AND e.expert_id = {$e_id}
      ) 
  WHERE (
      c.status = 0 
  ...
  GROUP BY c.id";
		...
		foreach ( $result as $key => $val ) {
			foreach ( $val as $k => $v ) {
				if ($k == "endplace") {
					if (strstr ( $v, ',' )) {
						$ar = explode ( ',', $v );
						$result ['endpl'] = array_slice ( $ar, 1, 6 );
						$result [$key] ['endplaces'] = implode ( "/", $result ['endpl'] );
						unset ( $result ['endpl'] );
						unset ( $result ['end'] );
						unset ( $result [$key] ['endplace'] );
					} else {
						$result [$key] ['endplaces'];
					}
				}
			}
		}
	...
	public function cfgm_expert_info() {
		...
		$e_id = $expert [0] ['eid'];
		$sql = "	SELECT e.id AS e_id,e.small_photo,e.realname,e.mobile,e.expert_dest as expert_destid,e.sex as sexid,
...
(select GROUP_CONCAT(kindname SEPARATOR '、') from u_dest_cfg where FIND_IN_SET(id,e.expert_dest) >0 )as expert_dest
	...
	/*
	 *
	 * do expert line list (ps : unclear purpose) 线路列表
	 *
	 * by zhy
	 *
	 * at
	...
	/*
	 *
	 * do modify expert data 修改管家信息
	 *
	 * by zhy
	 *
	 * at 2015年11月11日 13:26:31
	 *
	 */
...
*/
	...
	/*
	 *
	 * 		do expert password (ps : unclear purpose ) 修改密码
	 *
	 * 		by zhy
	 *
	 * 		at
	...
	public function get_expert_list() {
		...
		if (is_numeric ( $lat ) && is_numeric ( $lon ) && is_numeric ( $len )) {			//根据经纬度查询并且匹配最近管家
...
// 			$order_by = 'order by e.online desc,e.satisfaction_rate desc,e.people_count desc,e.id desc';
			$order_by = 'order by e.online desc,e.total_score desc';
		...
		if (! empty ( $city_name ) && ! empty ( $city_id )) {
			$temp_where .= " and e.city in ({$city_id})";
		}
		...
		if ($dest_id || $qy_id) {
			$label_where = "   1=1 {$sub_where}  {$temp_where} and e.status=2";
		} else {
			$label_where = "  1=1  {$temp_where} and e.status=2 ";
		}
		$sql = "	SELECT e.id AS expert_id,e.small_photo,e.sex,e.online,e.nickname,e.realname,e.comment_count, CASE WHEN e.isstar=1 THEN '明星专家' WHEN e.grade=1 THEN '管家' WHEN e.grade=2 THEN '初级专家' WHEN e.grade=3 THEN '中级专家' WHEN e.grade=4 THEN '高级专家' END grade, e.expert_theme AS expert_dest, e.satisfaction_rate AS satisfaction_rate , e.total_score,e.people_count AS volume, ua.name AS cityname, (select GROUP_CONCAT(d.kindname SEPARATOR '、') as expert_dest from u_dest_cfg as d where FIND_IN_SET(d.id, substring_index  (expert_dest,',',3))) as expert_dest  FROM u_expert AS e  LEFT JOIN u_area AS ua ON e.city=ua.id  LEFT JOIN u_line_apply AS la ON la.expert_id=e.id  LEFT JOIN u_line AS l ON l.id=la.line_id  LEFT JOIN u_expert_location AS uel ON e.id=uel.eid   WHERE     {$expands_where}  {$label_where}          group by e.id    {$order_by}     LIMIT {$from},{$page_size}	";
		$query = $this->db->query ( $sql );
		$data1 = $query->result_array ();
 

		foreach ( $data1 as $key => $val ) {									//转换积分
			foreach ( $val as $k => $v ) {
				if ($k == "satisfaction_rate") {
					if ($v) {
						$val [$k] = round ( $v * 100 );
	...
	/*
	 *
	 * 		do customize city list		定制沿用城市数据。
	 *
	 * 		by zhy
	 *
	...
	/*
	 *
	 * 		do expert_sell_line 管家产品
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月5日 15:10:19
	 *
	 */
	public function cfmg_expert_sell_line() {
		...
		$sql = "select la.id,la.expert_id,(CASE WHEN la.grade=1 THEN '管家' WHEN la.grade=2 THEN '初级专家' WHEN la.grade=3 THEN '中级专家' WHEN la.grade=4 THEN '高级专家' END) AS grade,l.id AS line_id,l.linename,l.linetitle,l.mainpic,l.lineprice,l.satisfyscore,l.all_score AS total_score,    	l.peoplecount  as sell_num,     (select sum(avgscore2) from u_comment as c where c.line_id=l.id and c.expert_id={$eid}) as expert_total_score       from u_line_apply AS la left join u_line AS l on l.id=la.line_id     where la.status=2 and l.status='2'  and la.expert_id= {$eid} GROUP BY l.id limit {$from},{$page_size}    ";
	...
	/*
	 *
	 * 		do custiome expert list 管家定制
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月5日 15:43:41
	 *
	 */
	...
	public function cfmg_expert_comment_list() {
		...
		is_numeric ( $eid ) ? ($eid) : $this->__errormsg ( '管家标识不能为空！' );
		$sql = "select     uc.score5,uc.score6,uc.expert_content as content,uc.reply1,uc.reply2,uc.addtime,um.nickname as nickname,mo.productautoid as productautoid,l.linename as line_name,l.linetitle as line_title       from u_comment as uc left join u_member as um on uc.memberid=um.mid        left join u_member_order as mo on uc.orderid=mo.id    left join u_line as l on l.id=mo.productautoid        where uc.status=1	and uc.expert_id={$eid}      {$where} 	order by uc.addtime  	desc limit {$from},{$page_size}     ";
		$query = $this->db->query ( $sql );
		$data ['line_list'] = $query->result_array ();
		if (empty ( $data ['line_list'] )) {
			$this->__outmsg ( $data ['line_list'] );
	...
	/*
	 *
	 * 		do expert Consultation 管家咨询
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月5日 16:34:45
	 *
	 */
	...
	/*
	 *
	 * 		do expert travels		管家游记
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月5日 16:37:38
	 *
	 */
	...
	/*
	 *
	 * 		do lot lat		日志插入，用于pc网站记录
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月12日 18:25:17
	 *
	 */
	...
	public function cfmg_user_comment_list() {
		...
		$sql = " select uc.score1,uc.score2,uc.score3,uc.score4    ,   uc.addtime,uc.reply1,uc.reply2,uc.content ,um.mobile ,um.nickname ,uc.pictures as pic from u_comment as uc left join u_member as um on uc.memberid=um.mid where uc.status=1   and uc.line_id={$eid}      {$where} 	order by uc.addtime  	desc limit {$from},{$page_size}     ";
		$query = $this->db->query ( $sql );
		$datas = $query->result_array ();
		if (empty ( $datas )) {
			$this->__outmsg ( $datas );					//空输出
		}
		foreach ( $datas as $key => $val ) {			//为图片加域名
			foreach ( $val as $k => $v ) {
				if ($k == "pic") {
					if ($v) {
						$val [$k] = explode ( ";", $v );
	...
	/*
	 *
	 * 		do expert collect		管家收藏	又称		管家点赞
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月21日 17:07:58
	 *
	 */
	...
	/*
	 *
	 * 		do good tag 点赞		用户为管家游记点赞
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月18日 14:36:15
	 *
	 */
	...
	/*
	 *
	 * 		do expert comment submit 管家评论提交
	 *
	 * 		by zhy
	 *
	 * 		at 2016年3月2日 18:27:32
	 *
	 */
	...
	/*
	 *
	 * 		do my customize list 我的定制
	 *
	 *			by zhy
	 *
	 * 		at 2016年1月28日 15:54:19
	 *
	 */
	public function cfmg_my_customize_list() {
		...
		$this->__outmsg ( $datas, $dat );
	}
	
	
	/*
	 *
	 * 		do custoimze detail row 定制 详情 行
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月28日 16:26:01
	 *
	 */
	...
	/*
	 *
	 * 		do custoimze expert detail row 管家抢单
	 *
	 * 		by zhy
	 *
	 * 		at 2016年1月28日 16:45:02
	 *
	 */
	...
	/*
	 *
	 * 		do expert apply 管家未申请线路
	 *
	 * 		by zhy
	 *
	 * 		at 2016年2月27日 11:01:55
	 *
	 */
...
/* 		elseif (! empty ( $startcity )) {
		...
		$sql = "         SELECT `l`.`id` AS line_id, `l`.`linecode` AS line_sn, `l`.`linename` AS line_title,     l.mainpic as pic ,`l`.`all_score` AS all_score, GROUP_CONCAT(`st`.`cityname`) AS start_city, `l`.`agent_rate` AS agent_rate, `l`.`satisfyscore` AS satisfyscore, `l`.`comment_count` AS comment_count, `l`.`peoplecount` AS peoplecount, `l`.`sell_direction` AS sell_direction, `s`.`company_name` AS company_name, `s`.`mobile` AS mobile   		 FROM (`u_line` as l)     LEFT JOIN `u_supplier` as s ON `l`.`supplier_id` =`s`.`id`   left join u_line_startplace as ls on l.id=ls.line_id  LEFT JOIN `u_startplace` as st ON `ls`.`startplace_id`=`st`.`id`       where `l`.`id` NOT IN( SELECT line_id FROM u_line_apply as la WHERE la.expert_id ={$m_id} and la.status=2)  {$where}   AND `l`.`status` = 2    GROUP BY l.id   ORDER BY `l`.`addtime` desc limit {$from},{$page_size}";
		$query = $this->db->query ( $sql );
	...
	/*
	 *
	 * 		do expert no apply 管家已申请线路
	 *
	 * 		by zhy
	 *
	 * 		at 2016年2月27日 12:14:18
	 *
	 */
...
/* 		elseif (! empty ( $startcity )) {
		...
		*/
		...
		$sql = "      SELECT `l`.`id` AS line_id, `l`.`linecode` AS line_sn, `l`.`linename` AS line_title, l.mainpic as pic, `s`.`company_name` AS supplier_name,     	(CASE WHEN `la`.`grade`=1 THEN '管家' WHEN `la`.`grade`=2 THEN '初级专家' WHEN `la`.`grade`=3 THEN '中级专家' WHEN `la`.`grade`=4 THEN '高级专家' END) AS grade  ,  	GROUP_CONCAT(`st`.`cityname`) AS cityname, 	`l`.`agent_rate` AS agent_rate, `l`.`satisfyscore` AS satisfyscore, `l`.`comment_count` AS comment_count, `l`.`peoplecount` AS peoplecount, `l`.`lineprice` AS lineprice, `s`.`mobile` AS mobile, `l`.`status`   	FROM (`u_line_apply` AS la)   	LEFT JOIN `u_line` AS l ON `la`.`line_id` = `l`.`id`   LEFT JOIN `u_supplier` as s ON `l`.`supplier_id` =`s`.`id`   left join u_line_startplace as ls on l.id=ls.line_id  LEFT JOIN `u_startplace` as st ON `ls`.`startplace_id`=`st`.`id`      	WHERE `la`.`status` = 2 	{$where}	AND `l`.`producttype` = 0 	AND `la`.`expert_id` = {$m_id}	 GROUP BY l.id  ORDER BY `la`.`addtime` desc     limit {$from},{$page_size}";
		$query = $this->db->query ( $sql );
		$line_apply_list = $query->result_array ();
	...
	/*
	 *
	 * 		do expert apply supplier		供应商信息
	 *
	 * 		by zhy
	 *
	 * 		at 2016年2月27日 13:55:31
	 *
	 */
	...
	/*
	 *
	 * 		do line detial apply
	 *
	 * 		by zhy
	 *
	 * 		at 2016年2月27日 15:25:55
	 *
	 */
	...
	/*
	 *
	 * 		do show line detial 查看
	 *
	 * 		by zhy
	 *
	 * 		at 2016年2月27日 16:13:58
	 *
	 */
...
// 		$this->expert_check_token ( $token );
	...
	/*
	 *
	 * 		do apply line update status		管家申请线路
	 *
	 * 		by zhy
	 *
	 * 		at 2016年3月1日 17:31:48
	 *
	 */
	...
	/*
	 *
	 * 		do coupon 优惠卷		通过用户ID寻找优惠卷
	 *
	 * 		by zhy
	 *
	 * 		at 2016年2月29日 11:42:57
	 *
	 */
	...
	/*
	 *
	 * 		do my prize detail 我的礼品详情
	 *
	 * 		by zhy
	 *
	 * 		at 2016年2月29日 12:04:15
	 *
	 */
	...
	/*
	*
	*				do	expert	api		用于聊天api
	*
	*				by 	zhy
	*
	*				at		2016年3月15日 14:35:58
	*
	*/
	...
	/*
	*
	*				do	update		order  price		管家修改订单价格
	*
	*				by 	zhy
	*
	*				at		2016年3月15日 16:59:39
	*
	*/
...
//---------------------------------------------------------------------------------------------------------------------public----------------------------------------------------------------------------------------------------------------------
	 ...
	 * 		by zhy
	 *
	 * 		at 2015年09月24日 16:41:14
	 *
	 */
	public function send_message() {
		$this->load->library ( 'session', true );
		$mobile = $this->input->post ( 'mobile', true );
		$is = intval ( $this->input->post ( 'is', true ) ); // 短信模板标识码
		$usertype = intval ( $this->input->post ( 'usertype', true ) ); // 短信模板标识码
			...
			switch ($is) {
				case 1 : // 会员注册
					...
					if ($usertype == '1') {
						...
						if (! empty ( $member )) 					{		$this->__errormsg ( '用户手机号已存在' );		}
					}
				...
				case 4 : // 管家注册
					...
					// 验证手机号状态
					if ($usertype == '1') {
						$result_user = $this->db->query ( "select  status  from u_expert  where mobile={$mobile}  ORDER BY `id` desc " )->result_array ();
						if (! empty ( $result_user [0] ['status'] )) {
							if ($result_user [0] ['status'] == 1) {
								$this->__errormsg ( '该手机号正在注册中，请不要重复注册！' );
							} elseif ($result_user [0] ['status'] == 2) {
								$this->__errormsg ( '该手机号已经注册成功，请不要重复注册！' );
							} elseif ($result_user [0] ['status'] == - 1) {
								$this->__errormsg ( '该手机号已经被终止合作，请联系系统管理员 ！' );
							}elseif ($result_user [0] ['status'] == 3) {
								$this->__errormsg ( '该手机号审核被拒绝，请联系系统管理员 ！' );
	...
	/*
	 *
	 * 		do page inside , just customize registered ,once ..... 短信内部掉用，故简
	 *
	 * 		by zhy
	 *
	 * 		at 2016年3月7日 18:01:07
	 *
	 */
	...
	/*
	 *
	 * 		do 一对一消息写入 content 消息内容 $msg_type 接收人实体类型 （0:用户,1:B2,2:B1） $receipt_id 接收人ID $title 消息标题
	 *
	 * 		by zhy
	 *
	 * 		at
	...
	/*
	 *
	 * do expert mobile 验证管家手机是否唯一
	 *
	 * by zhy
	 *
	 * at 2016年3月10日 14:54:55
	 *
	 */
	...
	/*
	 *
	 * do public upload （ps : cautious） 公共上传
	 *
	 * by zhy
	 *
	 * at 2015年12月064日 08:37:28
	 *
	 */
	...
	/*
	 *
	 * do get_qrcodes for expert_register 二维码上传以及生成
	 *
	 * by zhy
	 *
	 * at 2015年12月14日 16:20:23
	 *
	 */
	...
	/*
	 *
	 * do array_unique_two 祛重二维数组
	 *
	 * by zhy
	 *
	 * at 2016年2月25日 17:49:12
	 *
	 */
		...
		if ($couActionData [0] ['isrepeat'] == 0) // 不可以重复参与
{
	...
	*		by 	zhy
	*
	*		at		2016年3月16日 17:27:33
	*
	*/
	
	private function __outmsg($reDataArr, $len = 0) {
		$lastData ['rows'] = "";
