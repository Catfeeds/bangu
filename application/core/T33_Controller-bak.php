...
class T33_Controller extends MY_Controller {
		...
		(
					    CASE  
					    	WHEN mo.ispay=0 AND TIMESTAMPDIFF(HOUR,mo.addtime,NOW())>24 
					    		THEN '已取消'  
					   	    WHEN mo.id IN (SELECT r.order_id FROM u_order_refund AS r WHERE r.status>=0 and r.status<4) AND (mo.status=-3 or mo.status=-4) 
					   	    	THEN '退款审核中'  
					   	    WHEN mo.id IN (SELECT r.order_id FROM u_order_refund AS r WHERE r.status=4) AND (mo.status=-3 or mo.status=-4) 
					   	    	THEN '退款成功'  
					   	    WHEN mo.id IN (SELECT r.order_id FROM u_order_refund AS r WHERE r.status<0) AND (mo.status=-3 or mo.status=-4) 
					   	    	THEN '退款失败'  
					   	    WHEN mo.status = -4 
					   	    	THEN '已取消'  
					   	    WHEN mo.status = -3 
	...
	* */
			...
			foreach ($order_list as $k=>$v)
			{
				...
				if($v['order_id']!="0")  //有订单的交款
				{
					if($v['money']>0)  //交的是正数
					{
						//有未还的信用额度
						$no_pay=0;//剩余未还款的金额
						if(!empty($expert_limit)&&$v['way']!="账户余额")
						{
						...
						if($reback>0&&$credit_money>0&&$v['way']!="账户余额")
						{
	...
	protected function reback_debit($item_id)
	{
		...
		$receive=$this->u_order_receivable_model->all(array('order_id'=>$one['order_id'],'way !='=>'账户余额','status'=>'2','money >'=>'0'));
		$re_money=0;//该订单总共交的钱
		if(!empty($receive))
		{
			foreach ($receive as $n=>$m)
