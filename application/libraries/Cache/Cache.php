...
class CI_Cache extends CI_Driver_Library {
	...
	{
		return $this->{$this->_adapter}->select($db);
	}
	public function hGet($key, $field)
	{
		$hashdata = $this->{$this->_adapter}->hGet( $key, $field );
		return $hashdata;
	}
	public function hSet($key, $field, $value){
		$this->{$this->_adapter}->hSet ( $key, $field, $value );
	}
	public function sAdd($key,$member){
		$this->{$this->_adapter}->sAdd( $key, $member );
	}
	public function sRem($index_key, $id){
		$this->{$this->_adapter}->sRem( $index_key, $id );
	}
	public function sMembers($key){
		$array = $this->{$this->_adapter}->sMembers($key);
		return $array;
	}
	// ------------------------------------------------------------------------
