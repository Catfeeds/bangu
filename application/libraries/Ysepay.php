...
class Ysepay
{
	...
	{
		$this->param = array ();
		$this->param['usercode']            = 'baokao360';
		$this->param['merchantname']		= '中国报考信息网';
		$this->param['pfxpath']             = './application/libraries/yinsheng/key/baokao360.pfx';
		$this->param['businessgatecerpath'] = './application/libraries/yinsheng/key/businessgate.cer';
		$this->param['pfxpassword']         = '111111';
		$this->param['noticepg_url']        = '';
		$this->param['noticebg_url']        = '';
	 ...
	 * @return url
	 */
	function S3001_ysepay($config)
	{
		$this ->param['noticepg_url'] = $config['noticepg_url']; 
		$this ->param['noticebg_url'] = $config['noticebg_url'];
		...
		$param=array(
			"Ver"=>"1.0",
			"MsgCode"=>$MsgCode,
			"datetime_string"=>self::datetime2string(date('YmdHis' ,$time)),
			"busicode"=>$config['busicode'],
			"orderid"=>$config['orderid'],
			"cur"=>"CNY",
			"date"=>date('Ymd',$time),
			"amount_units_fen"=>(double)$config['amount'],
			"note"=>$config['ordernote'],
			"timeout"=>60*24*7,
...
// 		var_dump($this ->param);exit;
		...
		$url = 'http://'.$this->param['xmlpage_url']."?".http_build_query(array(
			...
			'msgCode' => $MsgCode,
			'msgId'   => sprintf("%014d",$config['orderid']),
...
// 		fclose($file);
	 ...
	 * @return success
	 */
	function unsign_crypt_return($input)
	{
		$check = $input['Check'];
		$msg = $input['Msg'];
