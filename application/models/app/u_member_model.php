...
class U_member_model extends MY_Model {
	...
	function get_custom($where,$page = 1, $num = 10){
		...
		$query_sql ='SELECT (CASE WHEN ca.isuse=1 THEN l.id ELSE NULL END) AS line_id,l.status AS line_status,c.estimatedate,';
	...
	function get_scheme_data($id){
		...
		$query_sql .= "CASE WHEN ISNULL(ca.replytime) > 0 THEN '管家还未回复方案，请耐心等待。。。' ";
		$query_sql .= " WHEN ISNULL(ca.replytime) = 0 THEN	ca.plan_design END 'solution' ";
		$query_sql .= "FROM u_customize_answer AS ca ";
		$query_sql .= "LEFT JOIN u_customize AS c ON ca.customize_id = c.id  ";
		$query_sql .= "LEFT JOIN u_expert AS e ON ca.expert_id = e.id WHERE c.id =".$id." and isnull(ca.replytime)=0 ";
