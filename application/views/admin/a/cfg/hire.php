...
<div class="page-content">
	...
	<div class="tab-content">
		<form action="<?php echo site_url('admin/a/cfg/hire/getHireData')?>" id='search_condition' class="form-inline clear" method="post">
...
$("#addSubmitForm").submit(function(){
	$.post("/admin/a/cfg/hire/addSubmitForm",$("#addSubmitForm").serialize(),function(json){
		var data = eval("("+json+")");
		if (data.code == 2000) {
			alert(data.msg);
			location.reload();
...
function see_detail(id ,type) {
	$.post("/admin/a/cfg/hire/getHireDetail",{id:id},function(json){
		if (json == false) {
			return false;
		}
		var data = eval("("+json+")");
...
$(".hireRelease").click(function(){
	...
	$.post("/admin/a/cfg/hire/hireRelease",{id:id},function(json){
		var data = eval("("+json+")");
		if (data.code == 2000) {
...
$(".hireCancel").click(function(){
	var id = $("input[name='hireId']").val();
	$.post("/admin/a/cfg/hire/hireCancel",{id:id},function(json){
		var data = eval("("+json+")");
		if (data.code == 2000) {
			$(".hire_Detail,.modal-backdrop").hide();
			change_status();
...
$("#editSubmitForm").submit(function(){
	$.post("/admin/a/cfg/hire/editHire",$("#editSubmitForm").serialize(),function(json){
		var data = eval("("+json+")")
		if (data.code == 2000) {
