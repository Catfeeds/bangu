...
<div class="page-content">
	<ul class="breadcrumb">
		...
		<li class="active"><span>/</span>首页体验师</li>
	</ul>
	<div class="page-body">
	...
	<div class="fb-content">
		...
		<div class="fb-form">
			<form method="post" action="#" id="add-data" class="form-horizontal" >
				...
				<div class="form-group">
					<div class="fg-title">游记：<i>*</i></div>
					<div class="fg-input">
						<input type="text" readonly="readonly" id="clickChoiceNote" />
...
<?php echo $this->load->view('admin/a/choice_data/choice_travel_note.php');  ?>
...
var columns = [ {field : 'nickname',title : '姓名',width : '120',align : 'center'},
                {field : 'title',title : '游记标题',width : '150',align : 'center' ,length:15},
...
$("#dataTable").pageTable({
	columns:columns,
	url:'/admin/a/cfg/index_experience/getExperienceData',
	pageNumNow:1,
...
//选择游记
$("#clickChoiceNote").click(function(){
	var name = $("#clickChoiceMember").val();
	var id = $('input[name=member_id]').val();
	if (id < 1 && name.length <1) {
		alert('请选择体验师');
	} else {
		$('.cb-prompt').html(name);
		$('#cb-search-form-1').find('input[name=expertience_id]').val(id);
		createNoteHtml();
	}
})
$(".note-submit").click(function(){
	var activeObj = $(".db-data-1").find('.db-active');
	$("#clickChoiceNote").val(activeObj.attr('data-title'));
	$('input[name=note_id]').val(activeObj.attr('data-val'));
	$(".uploadImg").remove();
	$("#uploadFile").after("<img class='uploadImg' src='" + activeObj.attr('data-pic') + "' width='80'>");
	$('input[name=pic]').val(activeObj.attr('data-pic'));
	$(".choice_note").hide();
})
//添加弹出层
...
$("#add-data").submit(function(){
	...
	var url = id > 0 ? '/admin/a/cfg/index_experience/edit' :'/admin/a/cfg/index_experience/add';
	$.ajax({
		url:url,
		type:'post',
		dataType:'json',
...
function edit(id) {
	$.ajax({
		url:'/admin/a/cfg/index_experience/getDetailjson',
		type:'post',
		dataType:'json',
		data:{id:id},
		success:function(data){
			if (!$.isEmptyObject(data)){
				...
				$('#clickChoiceNote').val(data.title);
				formObj.find('input[name=member_id]').val(data.member_id);
				formObj.find('input[name=note_id]').val(data.travel_note_id);
...
function del(id) {
	if (confirm("您确定要删除吗?")) {
		$.post("/admin/a/cfg/index_experience/delExperience",{id:id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				$("#dataTable").pageTable({
					columns:columns,
