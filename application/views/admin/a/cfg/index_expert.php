...
<div class="page-content">
	<ul class="breadcrumb">
		...
		<li class="active"><span>/</span>首页管家配置</li>
	</ul>
	<div class="page-body">
		<div class="tab-content">
			...
			<form action="#" id='search-condition' class="search-condition" method="post">
				<ul>
					...
					<li class="search-list">
						<span class="search-title">专家类型：</span>
						<span >
...
var columns = [ {field : 'realname',title : '管家姓名',width : '120',align : 'center'},
		{field : false,title : '类型',width : '120',align : 'center' ,formatter:function(item){
				if (item.location == 1) {
					return '首页中间';
		...
		{field : false,title : '所在城市',width : '140',align : 'center' ,formatter:function(item){
				return item.pname+item.cityname;
			}
		},
...
$("#dataTable").pageTable({
	columns:columns,
	url:'/admin/a/cfg/index_expert/getIndexExpertJson',
	pageNumNow:1,
...
$("#add-data").submit(function(){
	...
	var url = id > 0 ? '/admin/a/cfg/index_expert/edit' :'/admin/a/cfg/index_expert/add';
	$.ajax({
		url:url,
		type:'post',
		dataType:'json',
...
function del(id) {
	if (confirm("您确定要删除吗?")) {
		$.post("/admin/a/cfg/index_expert/delete",{id:id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				$("#dataTable").pageTable({
					columns:columns,
...
function edit(id){
	$.ajax({
		url:'/admin/a/cfg/index_expert/getDetailjson',
		type:'post',
		dataType:'json',
		data:{id:id},
		success:function(data){
			if (!$.isEmptyObject(data)){
				...
				formObj.find('select[name=location]').val(data.location);
