...
<div class="page-content">
	<ul class="breadcrumb">
		...
		<li class="active"><span>/</span>首页畅销线路</li>
	</ul>
	<div class="page-body">
			...
			<form action="#" id='search-condition' class="search-condition" method="post">
				<ul>
					<li class="search-list">
						...
						<span ><input class="search-input" type="text" placeholder="线路名称" name="name" /></span>
					</li>
					<li class="search-list">
...
<script>
var columns = [ {field : 'linename',title : '线路名称',width : '300',align : 'center'},
                {field : 'cityname',title : '始发地',width : '130',align : 'center'},
...
$("#dataTable").pageTable({
	columns:columns,
	url:'/admin/a/cfg/index_hot_line/getHotLineData',
	pageNumNow:1,
...
$("#add-data").submit(function(){
	...
	var url = id > 0 ? '/admin/a/cfg/index_hot_line/edit' :'/admin/a/cfg/index_hot_line/add';
	$.ajax({
		url:url,
		type:'post',
		dataType:'json',
...
function edit(id) {
	$.ajax({
		url:'/admin/a/cfg/index_hot_line/getDetailJson',
		type:'post',
		dataType:'json',
		data:{id:id},
		success:function(data){
...
function del(id) {
	if (confirm("您确定要删除吗?")) {
		$.post("/admin/a/cfg/index_hot_line/delHotLine",{id:id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				$("#dataTable").pageTable({
					columns:columns,
