		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="bootbox-body">
					...
					<div class="form-group">
						<label for="inputEmail3" class="col-sm-2 control-label no-padding-right col_lb">活动目的地<span class="input-must">*</span></label>
						<div class="col-sm-10 col_ts">
							<select name="act_dest_id" id="act_dest_id" onchange="init_line(this)">
								<option value="">请选择</option>
								<?php foreach ($act_city_dest as $k => $vl): ?>
									<option data-dest="<?php echo $vl['dest_id']?>" value="<?php echo $vl['id']?>"><?php echo $vl['name']?></option>
								<?php endforeach; ?>
							</select>
						</div>
					</div>
...
var columns = [
		...
		{field : 'sad_name',title : '活动目的地名字',width : '100',align : 'center'},
...
$("#dataTable").pageTable({
	columns:columns,
	url:'/admin/a/cfg_act/activity_dest_line/getDestLineData',
	searchForm:'#search_condition',
});

function init_line(obj){
...
$("#addFormData").submit(function() {
	...
	if (id.length > 0) {
		var url = "/admin/a/cfg_act/activity_dest_line/edit";
	} else {
		var url = "/admin/a/cfg_act/activity_dest_line/add";
	}
...
function edit(id) {
	$.post("/admin/a/cfg_act/activity_dest_line/getOneData" ,{id:id} ,function(data) {
		var data = eval("("+data+")");
		$("input[name='linename']").val(data.linename);
		$("input[name='line_id']").val(data.line_id);
		$("input[name='id']").val(data.id);
		$("input[name='showorder']").val(data.showorder);
		$("select[name='act_dest_id']").val(data.sc_activity_dest_id);
...
function del(id) {
	if (confirm("您确定要删除吗?")) {
		$.post("/admin/a/cfg_act/activity_dest_line/delete",{id:id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				alert(data.msg);
				location.reload();
