...
<div class="page-content">
    <ul class="breadcrumb">
        ...
        <li class="active"><span>/</span>优惠劵帮助信息配置</li>
    </ul>
    <div class="page-body">
        <div class="tab-content">			
            <a id="add-button" href="javascript:void(0);" class="but-default" >添加优惠劵帮助信息</a>
        ...
        <div class="fb-form">
            <form method="post" action="#" id="add-data" class="form-horizontal" >
                <div class="form-group">
                    <div class="fg-title">优惠劵标题：<i>*</i></div>
                    <div class="fg-input">
                        <input type="text" name="title" />
                    </div>
                </div> 
                <div class="form-group">
                    <div class="fg-title">优惠劵说明内容：<i>*</i></div>
                    <div class="fg-input">
...
<script type="text/javascript">
                            var columns = [{field: 'title', title: '优惠劵标题', width: '100', align: 'center'}, 
                                           {field: 'content', title: '优惠劵说明内容', width: '200', align: 'center'},                             
                            ...
                            $("#dataTable").pageTable({
                                columns: columns,
                                url: '/admin/a/coupon_manage/coupon_help/get_data',
                                pageNumNow: 1,
                            ...
                            $("#add-data").submit(function () {
                                ...
                                var url = id > 0 ? '/admin/a/coupon_manage/coupon_help/edit' : '/admin/a/coupon_manage/coupon_help/add';
                                $.ajax({
                                    url: url,
                                    type: 'post',
                                    dataType: 'json',
    ...
    function edit(id) {
        $.ajax({
            url: '/admin/a/coupon_manage/coupon_help/get_edit_data',
            type: 'post',
            dataType: 'json',
            data: {id: id},
            success: function (data) {
                if (!$.isEmptyObject(data)) {
                    ...
                    formObj.find('input[name=content]').val(data.content); 
    ...
    function del(id) {
        if (confirm("您确定要删除吗?")) {
            $.post("/admin/a/coupon_manage/coupon_help/del", {id: id}, function (json) {
                var data = eval("(" + json + ")");
                if (data.code == 2000) {
                    $("#dataTable").pageTable({
                        columns: columns,
