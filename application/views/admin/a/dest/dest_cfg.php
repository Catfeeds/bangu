...
var columns = [ {field : 'name',title : '名称',width : '120',align : 'center'},
	        	...
	        	{field : 'kindname',title : '绑定目的地',align : 'center', width : '140'},
...
function getData(page) {
	...
	$("#dataTable").pageTable({
		columns:columns,
		url:'/admin/a/dest/dest_cfg/getDestCfgData',
		pageSize:10,
...
function disable(id) {
	layer.confirm('您确定要停用此目的地？', {btn:['确认' ,'取消']},function(){
		$.ajax({
			url:'/admin/a/dest/dest_cfg/disable',
			type:'post',
			dataType:'json',
			data:{'id':id},
...
function enable(id) {
	layer.confirm('您确定要启用此目的地？', {btn:['确认' ,'取消']},function(){
		$.ajax({
			url:'/admin/a/dest/dest_cfg/enable',
			type:'post',
			dataType:'json',
			data:{'id':id},
			success:function(result) {
...
function edit(id) {
	window.top.openWin({
		  ...
		  fix: true, //不固定
		  maxmin: true,
		  content: "<?php echo base_url('admin/a/dest/dest_cfg/edit_view');?>?id="+id
	});
}
//添加
$('#add-button').click(function(){
	window.top.openWin({
		  ...
		  fix: true, //不固定
		  maxmin: true,
		  content: "<?php echo base_url('admin/a/dest/dest_cfg/add_view');?>"
	});
})
</script>
</html>
