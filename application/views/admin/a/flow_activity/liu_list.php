...
<div class="page-content">
	...
	<!-- Page Body -->
						...
						<div class="widget-header ">
							<span class="widget-caption">流量活动分享列表</span>
						...
						<div class="widget-body">
							<div role="grid" id="simpledatatable_wrapper"
								...
								<div id="simpledatatable_filter" >
									<label>
										<form class="form-inline" role="form"  method="get" action="<?php echo site_url('admin/a/flow_activity/liu_list')?>">
									...
									注册人数：<?php echo $all_num_total;?>，实际激活人数<?php echo $num_total;?>，累计获得流量数：<?php echo $flownums_total;?>M，已经兑换流量数：<?php echo $congzhi_total;?>M
								</div>
								...
								<table
									...
									<thead>
										<tr role="row">
											...
											<th style="text-align:center" >流量数</th>											
									...
									<?php foreach ($pageData as $item): ?>
       									 <tr data-mobile="<?php echo $item['mobile']?>">
            								    ...
            								    <td class="mobile_line" style="text-align:center"><?php echo $item['mobile']?>[<span class="belong_place"></span>]</td>
            								    <td  style="text-align:center"><?php echo $pep_all[$item['member_id']];?></td>												 
            								    <td  style="text-align:center"><?php echo $item['flownums']?></td>												 
            								     <td  style="text-align:center"><?php echo date("Y-m-d H:i:s", $item['addtime'])?></td>									 
									    <td class=" " style="text-align:center">
										<?php if($item['flownums']>=50 &&$item['status']==1 ) { ?>
									    <a href="javascript:void(0);" onclick="del('<?php echo $item['id'];?>');"  class="btn btn-default btn-xs purple">充值流量</a></td>
										<?php } ?>
									  </tr>
									<?php endforeach;?>
									</tbody>
		...
		$.ajax({
			...
			success:function(data){
				if(data.carrier){
					...
					obj.find(".mobile_line .belong_place").html(data.carrier);
				}else{
					obj.find(".mobile_line .belong_place").html("手机号码有误");
				}
...
function del(id) {
	if (confirm("您确定要充值吗?")) {
		$.post("/admin/a/flow_activity/congzhi",{id:id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				alert(data.msg);
				location.reload();
