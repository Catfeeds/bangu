	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">深窗首页轮播模块配置</h4>
			</div>
			<div class="modal-body">
				<div class="bootbox-body">
					...
					<div class="form-group">
						...
						<div class="col-sm-10 col_ts">
							<input class="form-control" id="link" placeholder="请输入正确的链接地址 (例:/line/index?id=3)" name="link" type="text">
						</div>
					</div>
					<div class="form-group">
...
var columns = [
		...
		{field : 'ck_name',title : '类别',width : '100',align : 'center'},
...
$("#dataTable").pageTable({
	columns:columns,
	url:'/admin/a/sc_cfg/sc_index_roll_pic/getDataList',
...
$("#addFormData").submit(function() {
	...
	if (id.length > 0) {
		var url = "/admin/a/sc_cfg/sc_index_roll_pic/edit";
	} else {
		var url = "/admin/a/sc_cfg/sc_index_roll_pic/add";
	}
...
function edit(id) {
	$.post("/admin/a/sc_cfg/sc_index_roll_pic/getOneData" ,{id:id} ,function(data) {
		var data = eval("("+data+")");
		$("input[name='id']").val(data.id);
		$("#index_kind_id").val(data.kind_dest_id);
...
function del(id) {
	if (confirm("您确定要删除吗?")) {
		$.post("/admin/a/sc_cfg/sc_index_roll_pic/delete",{'id':id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				alert(data.msg);
				location.reload();
