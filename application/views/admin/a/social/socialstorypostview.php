...
//            }
            ...
            var url = id > 0 ? '/admin/a/social/socialstorypost/edit_post_by_id' : '/admin/a/social/socialstorypost/add_post';
                $.ajax({
                url: url,
                type: 'post',
                dataType: 'json',
    ...
    function edit(id) {
        $('.url_title').remove();
        $.ajax({
            url: '/admin/a/social/socialstorypost/get_post_by_id',
            type: 'post',
            dataType: 'json',
            data: {post_id: id},
            success: function (data) {
...
function del(id) {
	if (confirm("您确定要删除吗?")) {
		$.post("/admin/a/social/socialstorypost/del",{post_id:id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				alert(data.msg);
				window.location.reload(true);
