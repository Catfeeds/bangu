...
<div class="page-content">
    <ul class="breadcrumb">
        ...
        <li class="active"><span>/</span>心情</li>
    </ul>
    <div class="page-body">
...
<script type="text/javascript">
                            var columns = [{field: 'id', title: '主题编号', width: '25', align: 'center'},
                                ...
                                {field :false,title : '主题背景图片',align : 'center', width : '100',formatter:function(item){
                        			return '<img src="<?php echo trim(base_url(''),'/');?>'+item.icon+'"  height="100" />';
                        		}},
...
//                                        }
                                        ...
                                        button += '<br/>';
                                        button += '<a href="<?php echo site_url('admin/a/social/socialtheme/get_posts_by_theme_id?theme_id=')?>'+item.id+'" class="tab-button but-blue">心情动态</a>';
                                        return button;
                                    }
                                }
                            ];
    ...
    function indext(id,status) {
        if(status==1){
        	if (confirm("您确定要取消app首页推荐吗?")) {
        		$.post("/admin/a/social/socialtheme/indexVideoInfo",{id:id},function(json){
        			var data = eval("("+json+")");
        			if (data.code == 2000) {
        				alert(data.msg);
        				location.reload();
