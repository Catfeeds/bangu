        ...
        <div class="fb-form">
            <form method="post" action="#" id="add-data" class="form-horizontal" >
                <div class="form-group">
                    <div class="fg-title">问题标题：<i>*</i></div>
                    <div class="fg-input">
                        <input type="text" name="title"/>
                    </div>
                </div>
                <div class="form-group">
                    <div class="fg-title">问题正文：<i>*</i></div>
                    <div class="fg-input">
								...
								<table 
									...
									<tbody>
                                                                            <?php foreach ($posts as $item): ?>
                                                                                <tr>
                                                                                    <td class="sorting_1" style="text-align:center;vertical-align: middle"><?php echo $item['id']; ?></td>
                                                                                    <td class=" " style="text-align:center;vertical-align: middle"><?php echo $item['title']; ?></td>
...
//            }
            ...
            var url = id > 0 ? '/admin/a/social/socialtopicquestion/edit_post_by_id' : '/admin/a/social/socialtopicquestion/add_post';
                $.ajax({
                url: url,
                type: 'post',
                dataType: 'json',
    ...
    function edit(id) {
        $('.url_title').remove();
        $.ajax({
            url: '/admin/a/social/socialtopicquestion/get_post_by_id',
            type: 'post',
            dataType: 'json',
            data: {post_id: id},
            success: function (data) {
...
function del(id) {
	if (confirm("您确定要删除吗?")) {
		$.post("/admin/a/social/socialtopicquestion/del",{post_id:id},function(json){
			var data = eval("("+json+")");
			if (data.code == 2000) {
				alert(data.msg);
				window.location.reload(true);
