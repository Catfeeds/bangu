...
jQuery(document).ready(function(){
	...
	page0=new jQuery.paging({renderTo:'#list',record:jQuery.parseJSON(data),url : "<?php echo base_url()?>admin/b1/product/product_indexData",form : '#listForm',// 绑定一个查询表单的ID
				columns : [
	...
	jQuery('#list').on("click", 'a[name="edit0"]',function(){
		...
		if(producttype==1){ //定制团
			window.open("<?php echo base_url()?>admin/b1/products/toLineEdit?id="+id+'&style=1');
				//window.location.href="<?php echo base_url()?>admin/b1/products/toLineEdit?id="+id+'&style=1';
		}else{
				//window.location.href="<?php echo base_url()?>admin/b1/products/toLineEdit?id="+id+'&style=0';
	...
	jQuery('#list').on("click", 'a[name="commit0"]',function(){
		...
		layer.confirm('您确定要上线？该线路如也在帮游产品,也同步到审核中', {btn:['确认' ,'取消']},function(){
	...
	var page1 = null;
	function initTab1(){
		page1=new jQuery.paging({renderTo:'#list1',url : "<?php echo base_url()?>admin/b1/product/product_indexData",form : '#listForm1',// 绑定一个查询表单的ID
			columns : [
				{field : 'linecode',title : '产品编号',width : '90',align : 'center'},
	...
	var page2 = null;
	function initTab2(){
		page2=new jQuery.paging({renderTo:'#list2',url : "<?php echo base_url()?>admin/b1/product/product_indexData",form : '#listForm2',// 绑定一个查询表单的ID
			columns : [
				{field : 'linecode',title : '产品编号',width : '90',align : 'center'},
			...
			/*	{field : 'linkman',title : '核准人',align : 'center', width : '70',
				...
				{field : '',title : '操作',align : 'center',width : '180',
					formatter : function(value,	rowData, rowIndex){
						return '<a target="_blank" href="/admin/b1/products/line_stock?id='+rowData.id+'" title="如需修改价格,请下架后再修改">库存</a>&nbsp;&nbsp;<a data="'+rowData.id+'" union_id="'+rowData.union_id+'"  name="offline" href="##">下线</a>&nbsp;&nbsp;<a target="_blank" href="/admin/b1/product/train_expert?id='+rowData.id+'"  >管家培训</a>&nbsp;&nbsp;<a href="##" name="copy0"  type="'+rowData.producttype+'" data="'+rowData.id+'">复制</a>';
					}
				}
			]
		});
	...
	jQuery('#list2').on("click", 'a[name="offline"]',function(){
	    var id=jQuery(this).attr('data');
	    var union_id=jQuery(this).attr('union_id');
	    layer.confirm('您确定要下线？该线路如在帮游产品上线,也同一起下线', {btn:['确认' ,'取消']},function(){
			var param = {"id":id,'type':1,'union_id':union_id}
	...
	var page3 = null;
	function initTab3(){
		page3=new jQuery.paging({renderTo:'#list3',url : "<?php echo base_url()?>admin/b1/product/product_indexData",form : '#listForm3',// 绑定一个查询表单的ID
			columns : [
				{field : 'linecode',title : '产品编号',width : '90',align : 'center'},
		...
		/*		{field : 'status',title : '状态',align : 'center', width : '80',
				...
				{field : '',title : '操作',align : 'center',width : '180',
					formatter : function(value,	rowData, rowIndex){
						...
						var string='<a href="##" data="'+rowData.id+'"  type="'+rowData.producttype+'"  name="edit0" >修改</a>&nbsp;&nbsp;<a href="##" data="'+rowData.id+'" name="commit0" union_id="'+rowData.union_id+'" >再递</a>&nbsp;&nbsp;<a href="##" name="copy0" data="'+rowData.id+'"  type="'+rowData.producttype+'" >复制</a>&nbsp;&nbsp;<a href="##" data="'+rowData.id+'" name="del3" union_id="'+rowData.union_id+'" >删除</a>';
						if(rowData.status==4){
	...
	jQuery('#list3').on("click", 'a[name="del3"]',function(){
		//updateStatus("del",jQuery(this).attr('data'),"#searchBtn3");
		var id=jQuery(this).attr('data');
 		var union_id=jQuery(this).attr('union_id');
 		layer.confirm('您确定要删除该线路?删除后将无法恢复了', {btn:['确认' ,'取消']},function(){
			var param = {"id":id,'type':1,'union_id':union_id}
	...
	var page4 = null;
	function initTab4(){
		page4=new jQuery.paging({renderTo:'#list4',url : "<?php echo base_url()?>admin/b1/product/product_indexData",form : '#listForm4',// 绑定一个查询表单的ID
			columns : [
				{field : 'linecode',title : '产品编号',width : '90',align : 'center'},
	...
	var page5 = null;
	function initTab5(){
		page5=new jQuery.paging({renderTo:'#list5',url : "<?php echo base_url()?>admin/b1/product/product_indexData",form : '#listForm5',// 绑定一个查询表单的ID
			columns : [
				{field : 'linecode',title : '产品编号',width : '90',align : 'center'},
