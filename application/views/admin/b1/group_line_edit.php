			...
			<div class="tabbable">
				...
				<span id="theme_arr" data="<?php  if(!empty($themeid)){ echo $themeid[0]['id'];} ?>" name="ds-lable"  <?php if(!empty($themeid)){ echo 'style="color: #fff;"'; }?> data-val="<?php  if(!empty($themeid)){ echo $themeid[0]['id'];} ?>">
                                    ...
                                    <tr height="34" class="form_group">
                                        ...
                                        <td>
                                            <input type="text"  id="depart_id" class="form_input w_180 fl"  autocomplete="off" onfocus="showAllExpert(this.id);" >
						...
						<div class="lineDayDiv">
							<div class="form-group">
								...
								<input name="daynum" value="<?php echo $num; ?>" id="daynum" type="hidden" />
									<div name="title" class="title_div text-width" contenteditable="true" ><?php if(!empty($rout[$i])){ echo $rout[$i]['title']; }?> </div>
							...
							<div class="form-group">
								<label class="col-sm-2 control-label no-padding-right label-width col_lb" for="inputEmail3">住宿</label>
									<div class="form-inline col_ts">
										<div class="form-group col_ts" style="margin: 0px;">
										     	<div class="col-sm-10 col_ts">
												<input type="text"  style="width: 700px;" id="hotel<?php echo $i;?>"
							...
							<div class="form-group">
								...
								<textarea id="editor_id<?php echo $i; ?>"  name="jieshao[<?php echo $i; ?>]" style="width: 50%; height:180px; padding:2px 0px 0px 5px;"><?php if(!empty($rout[$i])){ echo $rout[$i]['jieshao'];} ?></textarea>	
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label no-padding-right label-width col_lb"
						...
						id="lineLabelForm" novalidate="novalidate" onsubmit="return updateLabel();" >
							<div class="widget-body">
								<div class="form-group" style="padding:0px 0px 10px 0px;">					
										<div class="col-sm-2 control-label no-padding-right label-width col_xl" style="float: none;width:100%;max-width: 810px;" for="inputEmail3" >请点击以下标签进行选择：（客人根据兴趣寻找心仪的旅游线路时，会使用关键字标签进行搜索，若您不设标签将脱离于客人视线之外）</div>
...
jQuery('#sb_linetrain,#next_inetrain').click(function(){
	 ...
	 var index=$(this).index();　
		jQuery.ajax({ type : "POST",data : jQuery('#lineTrainForm').serialize(),async:false,url : "<?php echo base_url()?>admin/b1/product/updateTrain", 
			success : function(response) {
			 	 if(response!='' && response!='0'){	 	 
					alert( '保存成功！' );
                                                  //   alert(index);
