		...
		<div class="widget-main ">
			<div class="tabbable">
				...
				<div class="tab-content tabs-flat">
					...
					<form action="<?php echo base_url()?>admin/b1/product/updateLine" accept-charset="utf-8" method="post" 
                                ...
                                <td>
                                       ...
                                       <?php  if(!empty($package)){ 
                                        ...
                                        ?>
                                            ...
                                            <input type="hidden"  id="expert_id" value="<?php echo $value['expert_id']; ?>" name="expert_id[]">
                                            <input type="hidden"  id="departId" value="<?php echo $value['depart_id']; ?>" name="departId[]">
                                         <?php if($value['expert_id']==0){ echo $value['name']; }else{echo $value['nickname']; }  ?>
                                         <span class="delPlugin" onclick="delExperID(this ,<?php echo $value['expert_id']; ?>);">×</span>
                                         </span>
                                    
                                       <?php } }?> 
    
                                          <?php  if(!empty($expert['nickname'])){  //echo  $expert['nickname']; ?>
                                                <span class="selectedContent">
                                                <input id="b_expert_id" value="<?php echo  $expert['id']; ?>" name="b_expert_id" type="hidden">
                                               <?php echo  $expert['nickname']; ?>
                                                <span class="delPlugin" onclick="delExperID(this ,<?php echo  $expert['id']; ?>);">×</span>
                                                </span>                 
                                         <?php    } ?>
              ...
              jQuery.ajax({ type : "POST",data :"id="+lineid,url : "<?php echo base_url()?>admin/b1/product/getLineRoutData",
                     success : function(result) {    
                               ...
                               function chk(obj){
                                 ...
                                 var route_obj = null;
                                $("#rout_line").on("click", ".title_div",function(){
                                       clearTimeout(routeTimeout);
                                       route_obj = null;
...
function show_line_train(lineid){
     ...
     jQuery.ajax({ type : "POST",data :"id="+lineid+'&type_train='+type_train+'&type='+type,url : "<?php echo base_url()?>admin/b1/product/showLineTrain",
             success : function(result) {    
...
//function CheckLine(){
 jQuery('#sb_line,#next_line').click(function(){
	...
	jQuery.ajax({ type : "POST",async:false,data : jQuery('#lineInfo').serialize()+"&type="+1,url : "<?php echo base_url()?>admin/b1/product/updateLine", 
         beforeSend:function() {//ajax请求开始时的操作
