...
<body>
	<div class="Cellphone_header">
    	<div class="Cellphone_main">
        	...
        	<div class="Cellphone_Reg"><a href="<?php echo site_url('admin/b1/register/index');?>">免费注册</a></div>
        	<div class="Cellphone_land"><a href="<?php echo site_url('admin/b1/index')?>">供应商登录</a></div>
        </div>
    </div>
        		...
        		<div class="chunk_box">
           		...
           		<div class="eject_yanzheng">
            		...
            		<div class="Cellphone_ejc_box">
               		...
               		<button class="tijiao submit_code">发送</button>
                	<button class="guanbi">关闭</button>
              	</div>
              	<div class="chunk_box">
             		<span class="magin_top">密码</span>
           			<input class="magin_top" name="password" style=" width:337px;" maxlength="15" type="password" placeholder="设置登录密码" onKeyUp=pwStrength(this.value) onBlur=pwStrength(this.value)>
            	</div> 
...
function submit_form() {
	$.post(
		"<?php echo site_url('admin/b1/retrieve_pass/do_retrieve_pass')?>",
		$('#retrieve_password').serialize(),
		function(data) {
			var data = eval('('+data+')');
			if (data.code == 2000) {
				alert(data.msg);
				location.href="/admin/b1/index";
			} else {
				alert(data.msg);
			}
		}
...
$('.submit_code').click(function(){
	...
	$.post( 
		...
		{'mobile':mobile,'verifycode':vcode,'type':7},
		function(data) {
			var data = eval('('+data+')');
			if (data.code == 2000) {
				alert(data.msg);
				...
				alert(data.msg);
				location.href="/admin/b1/register/index";
			} else {
				alert(data.msg);
				$("#vcode").trigger("click");
			}
...
　　//设置button效果，开始计时
    ...
    $("#btnSendCode").html(curCount + "秒重新获取");
