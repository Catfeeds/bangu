...
<script src="<?php echo base_url() ;?>assets/js/500px/bootbox.js"></script>
<style type="text/css">
    .customBody{ width: 1200px; margin: 0 auto; border: 1px solid #dedede;}
    .widget-main { overflow: hidden; padding-top: 5px;}
    .form-group img{ padding: 0 5px;}
...
.widget{ margin: 0;}
.col-sm-10{ height: 30px; line-height: 30px;;}
...
.nav-tabs>li.active>a, .nav-tabs>li.active>a:hover, .nav-tabs>li.active>a:focus{ box-shadow: none; border-left: 1px solid #ddd; border-right: 1px solid #ddd;}
.widget-body .form-group label{ text-align: right; height: 30px; line-height: 30px; margin: 0;}
.nav-tabs>li.active>a, .nav-tabs>li.active>a:hover, .nav-tabs>li.active>a:focus{border-top: 2px solid #09c; background: #fff;}
.page-breadcrumbs{ border-bottom:1px solid #ddd;padding-left:10px;}
...
<div class="customBody">
									...
									<div class="widget-body">
										<div id="registration-form">
											<div class="form-group">
												...
												<div class="col-sm-10"><?php if(!empty($data['brand'])){ echo $data['brand'] ; ?><span style="font-size: 14px;font-family:宋体;color:navy">·</span><?php }?><?php echo $data['linename'];?></div>
											</div>
												...
												<div class="form-inline">
													<div class="checkbox " style="padding-top: 0px;">
														<label style="padding: 0px; text-align: left; width: auto; padding-right: 10px;">
															早餐/<?php if(!empty($rout[$i])){ if($rout[$i]['breakfirsthas']==1){ if(!empty($rout[$i]['breakfirst'])){echo $rout[$i]['breakfirst'];}else{ echo '有';} }else{ echo '不含';}}else{echo '不含'; } ?>
														</label>
													</div>
													<div class="checkbox" style="padding-top: 0px;">
														<label	style="padding: 0px; text-align: left; width: auto; padding-right: 10px;">
															午餐/<?php if(!empty($rout[$i])){ if($rout[$i]['lunchhas']==1){ if(!empty($rout[$i]['lunch'])){ echo $rout[$i]['lunch']; }else{ echo '有'; }}else{echo '不含';}}else{echo '不含';} ?>
														</label>
													</div>
													<div class="checkbox" style="padding-top: 0px;">
														<label style="padding: 0px; text-align: left; width: auto; padding-right: 10px;">
															晚餐 /<?php if(!empty($rout[$i])){ if($rout[$i]['supperhas']==1){ if(!empty($rout[$i]['supper'])){echo $rout[$i]['supper'];}else{echo '有'; }}else{echo '不含';}}else{echo '不含';} ?>
														</label>
													</div>
												</div>
											</div>
								...
								<form action="" accept-charset="utf-8"
									...
									<div class="widget-body" style="padding: 0;">
										...
										<!-- 		  	<div style="margin-top:20px;"><span>成人佣金:</span>
											<input style="padding-left: 5px;width:60px; background: #ebebe4" class="price_input" type="text" readonly disabled="disabled"  value="<?php //echo $data['agent_rate_int'];?>" id="agent_rate" class="form-control text-width" name="agent_rate"  />
											<span>元/人份</span>
										</div>
										<div style="margin-top:20px;"><span>小童佣金:</span>
											<input style="padding-left: 5px;width:60px; background: #ebebe4" class="price_input" type="text" readonly disabled="disabled"  value="<?php //echo $data['agent_rate_child'];?>" id="agent_rate" class="form-control text-width" name="agent_rate_child"  />
											<span>元/人份</span>
										</div> -->
													...
													<div style="margin-top: 20px;">
														<span class=" col_price">儿童占床说明 :</span> <span
															id="child_description"><?php if(!empty($suits[0]['child_description'])){ echo $suits[0]['child_description']; }else{echo '暂无内容';}?></span>
													</div>


													<div style="margin-top: 20px;">
														<span class=" col_price">儿童不占床说明 :</span> <span
															id="child_nobed_description"><?php if(!empty($suits[0]['child_nobed_description'])){ echo $suits[0]['child_nobed_description']; }else{echo '暂无内容';}?></span>
													</div>

													<div style="margin-bottom: 30px;">
														<span class=" col_price">特殊人群说明:</span> <span
															id="special_description"><?php if(!empty($suits[0]['special_description'])){ echo $suits[0]['special_description'];}else{ echo '暂无内容';} ?></span>
													</div>
									...
									<div class="widget-body">
										<div id="registration-form">
											...
											<div class="form-group">
												<!-- 费用包含 -->
												<label class="col-sm-2 control-label no-padding-right "
													style="width: 120px;" for="inputEmail3">费用包含：</label>
												<div class="col-sm-10">
									...
									<div class="widget-body">
										<div id="registration-form">
											<div class="form-group">
												...
												<!-- 特别约定 -->
												<label class="col-sm-2 control-label no-padding-right "
													style="width: 120px; padding-top: 0px;" for="inputEmail3">特别约定
													：</label>
												<div class="col-sm-10">
								...
								<div class="widget-body">
									<div id="registration-form">
										...
										<div class="form-group">
											<!-- 线路属性 -->
											<label class="col-sm-2 control-label no-padding-right "
												style="width: 120px; padding-top: 0px; height: 40px; line-height: 40px;" for="inputEmail3">产品标签：</label>
											<div class="col-sm-10" style="width: 70%;">
											<?php
	...
	<script
		src="<?php echo base_url('assets/js/admin/jquery.b2_date.js')?>"></script>
...
(function(){
	...
	if($("#read_profile").attr('checked')){
		//alert($("#read_profile").attr('checked'));
		$.post(
		"<?php echo base_url('admin/b2/line_apply/apply_line_operator');?>",
		{'apply_line_id':<?php if(!empty($data['id'])){echo $data['id']; }?>},
		function(data) {
		data = eval('('+data+')');
		if (data.status == 200) {
		alert(data.msg);
...
function switchover(suitId){
	jQuery.ajax({ type : "POST",data :'suitId='+suitId,url : "<?php echo base_url()?>admin/b2/line_apply/switchover_desc",
