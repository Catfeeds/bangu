...
.senBg{ clear: both; padding-top: 20px; }
.senBg .mingtou {height: 30px; line-height: 30px;}
.fileBtn{ float: left; }
.fileBtn .fileBox { position: relative; cursor: pointer; }
.fileBtn .fileBox img{ width:100px; position: relative; top:5px; left:90px;}
.fileBtn .fileBox span{ position: absolute; left: 0; top: 5px; }
.fileBtn .fileBox input{ width: 80px; overflow: hidden; height: 24px; position: relative; z-index: 10; opacity: 0;filter:alpha(opacity=0); cursor: pointer; }
</style>
...
<form class="jingnei_box" <?php if (!empty($expert) && $expert['type'] != 1){echo 'style="display:none;"';}?>>
    <div class="reg_conment">
        ...
        <div class="conmen_zhong">
                ...
                <div class="goog_goin">
                    ...
                    <ul class="generated_city">
                        <?php
                        	if (!empty($dest)) {
                        		foreach($dest as $val){
                        			echo '<li value="'.$val['id'].'">'.$val['name'].'<span>X</span></li>';
                        		}
                        	}
                        ?>
                    </ul>
            ...
            <!-- 背景图 -->
            <div class="senBg">
                <div class="mingtou"><i>*</i>设置背景图</div>
                <div class="fileBtn">
                    <div class="fileBox">
                    	<span onclick="uploadImgFile(this,4);">选择背景图</span>
                        <input type="hidden" name="big_photo">
                    </div>
                </div>
            </div>
        ...
        <div class="take_photos">
          ...
          <div class="conmen_top" style=" padding-top:5px;">
            <ul class="relative">
                <li>
                	<span class="titl_head" style="width:64px;">
                		<i>*</i>验证码:
                	</span>
                	<input type="text" name="code" maxlength="18" style="width:150px;"/>
                	<div class="huoqu getMobileCode" id="btnSendCode1">获取验证码</div>
                </li>
	             <div class="huoqu_hidden" id="yanzheng">
	            	...
	            	<div style="float:right; height:40px;width:91px;">
	               	...
	               	<div class="tijiao getMobileCode" id="submitSendCode">提交</div>
    ...
    <div class="reg_conment">
        ...
        <div class="personal_details">
            ...
            <ul>
                ...
                <li class="relative" style=" width: 285px;">
                	<span><i>*</i>手机号码:</span>
                    <input type="text" name="mobile" value="<?php if(!empty($expert)){echo $expert['mobile'];}?>" style="width:175px;" />
          ...
          <div class="conmen_top" style=" padding-top:5px;">
            <ul class="relative">
                <li><span class="titl_head" style="width:64px;">
                	<i>*</i>验证码:</span>
                	<input type="text" name="code" maxlength="18" style="width:150px;"/>
                	<div class="huoqu" id="btnSendCode2">获取验证码</div>
                </li>
...
function submitForm(obj) {
	...
	if (expert_id > 0) {
		...
		var url = "/admin/b2/register/expertRegister";
	}
...
$(".getMobileCode").click(function(){
	...
	try{
		...
		var vcode = ($(this).parents("form").find("input[name='vcode']").val());
		if (!isMobile(mobile)) {
			throw   new  Error( "请填写正确的手机号" );
		}
		var eid = <?php if(!empty($expert)){echo $expert['id'];}else{echo 0;}?>;
		$.post("/send_code/registerExpertCode",{mobile:mobile,type:4,verifycode:vcode,eid:eid},function(json){
			var data = eval("("+json+")");
			codeStatus = true;
			if (data.code == 2000) {
...
//上传文件
var imgProportion = {1:"5:3",2:"360x360",3:"5:3",9:'5:3',4:'332x222'};
var xiuBox = {1:'xiuxiu_box1',2:'xiuxiu_box2',3:"xiuxiu_box3",9:"xiuxiu_box9",4:'xiuxiu_box4'};
var xiuxiuEditor = {1:'xiuxiuEditor1',2:'xiuxiuEditor2',3:"xiuxiuEditor3",9:"xiuxiuEditor9",4:'xiuxiuEditor4'};
		...
		xiuxiu.onUploadResponse = function (data)
		{
			...
			if (data.code == 2000) {
				...
				if (type == 3) {
					...
					buttonObj.next("input").val(data.msg);
					if(buttonObj.nextAll('img').length) {
						buttonObj.nextAll('img').attr('src' ,data.msg);
					} else {
						buttonObj.parent().append('<img src="'+data.msg+'" />');
					}
				}
				closeXiu(type);
			} else {
