...
function searchKeyword1(obj ,zNodes ,event) {
	...
	if (keyword.length == 0) {
		...
		$.each(zNodes ,function(k ,v) {
			dataArr[v.id] = v;
			if (v.name.indexOf(jQuery.trim(keyword)) != -1 || v.enname.indexOf(jQuery.trim(keyword)) != -1) {
				idStr = idStr+v.id+',';
				pidStr = pidStr + v.pId+',';
			}
		})
...
//国内游和出境游的目的地插件
function showDestBaseTree(obj) {
	treeInputObj = obj;
	var url = '/common/get_data/getDestBaseData';
...
//国内游目的地
function showGNDestTree(obj) {
	treeInputObj = obj;
	var url = '/common/get_data/getGLDestBaseData';
	var zNodes = commonTree(obj ,url);
...
//出境游目的地
function showCJDestTree(obj) {
	treeInputObj = obj;
	var url = '/common/get_data/getJLDestBaseData';
	var zNodes = commonTree(obj ,url);
...
//周边游目的地
function showZBDestTree(obj ,cityid) {
	treeInputObj = obj;
	var url = '/common/get_data/getTripDestAll';
...
//出境，国内，周边游目的地
function showCGZDestTree(obj ,cityid) {
...
function treeInitCommon(obj ,dataArr) {
	...
	$("#treeDemo").css("width",width); //等于input的长
