...
.edit_btn:focus,.edit_btn:hover{ color: #fff; background:#2dc3e8 }
...
.table>thead>tr>th, .table>tbody>tr>td{ padding: 10px 0}
.input-sm {padding: 0;}
...
.DTTTFooter { background-color: #fff;background-image: none;border: none;padding: 15px 15px 0 15px;}
.tab-content{ box-shadow: none;}
.btn-info, .btn-info:focus{ color: #fff !important; background: #09C !important; border-color: #09C !important; margin: 0 3px; }
.nav-tabs>li.active>a, .nav-tabs>li.active>a:hover, .nav-tabs>li.active>a:focus{ box-shadow: none !important;}
...
.tab-content { padding:15px 00px !important;}
 </style>
...
<div class="page-body" id="bodyMsg">
	   <div class="widget">
	   	<div class="flip-scroll">
				<div class="tabbable" style="background:#fff; padding:10px;">
					...
					<div class="tab-pane active" id="tab_content0"><!--询价单:数据开始-->
						<div class="div_inquiry_list0">
							...
							<a style="padding:5px 8px;background:#e74b37;border-radius:2px; color:#fff; " target="_blank" href="<?php echo base_url('admin/b2/inquiry_sheet/show_add_inquiry')?>">添加询价单</a>
							<form style="margin-top:16px;" action="<?php echo base_url();?>admin/b2/inquiry_sheet/inquiry_sheet_list" id='inquiry_sheet0' name='inquiry_sheet0' method="post">
...
<script type="text/javascript">
	...
	// 询价单数据
	var inquiry_sheet_columns_0=[ {field : 'eid',title : '询价单编号',width : '100',align : 'center',},
			...
			{field : 'endplace',title : '目的地',align : 'center', width : '200'},
			...
			{field : 'ce_id',title : '操作',align : 'center', width : '200',
				formatter : function(value,	rowData, rowIndex){
		      	 var html = "<a target='_blank' href='<?php echo base_url()?>admin/b2/inquiry_sheet/show_inquiry?c_id="+value+"&ca_id="+rowData['ca_id']+"&e_id="+rowData['eid']+"' class='edit' data-val='"+value+"'>"+"详情</a>";
		      	 if(rowData['e_status']==2){
		      	 	html += "<a target='_blank' href='<?php echo base_url()?>admin/b2/inquiry_sheet/edit_inquiry?c_id="+value+"&ca_id="+rowData['ca_id']+"&e_id="+rowData['eid']+"&again=1' class=''>再次发单</a>";
		      	 }else{
		      	 	html += "<a target='_blank' href='<?php echo base_url()?>admin/b2/inquiry_sheet/edit_inquiry?c_id="+value+"&ca_id="+rowData['ca_id']+"&e_id="+rowData['eid']+"' class=''>编辑</a>";
		      	 }
			...
			return html;
         	...
         	// 已回复数据
	var inquiry_sheet_columns_1=[ {field : 'eid',title : '询价单编号',width : '100',align : 'center'},
	        {field : 'c_id',title : '定制单编号',width : '100',align : 'center',},
			{field : 'startdate',title : '出行日期',width : '200',align : 'center',
	           	formatter : function(value,rowData, rowIndex){
		        	if(rowData['startdate']!=''&& rowData['startdate']!=null && rowData['startdate']!='0000-00-00'){
		...
		/* 	 {field : 'estimatedate',title : '预估日起',align : 'center', width : '200'}, */
			...
			{field : 'ce_id',title : '操作',align : 'center', width : '250',
			formatter : function(value,rowData, rowIndex){
		      	return "<a target='_blank'  onclick='show_inquiry_sheet(this)' class='edit' data-val='"+rowData['eid']+"|"+value+"'>选方案</a><a class='edit' target='_blank' href='<?php echo base_url()?>admin/b2/inquiry_sheet/edit_inquiry?c_id="+value+"&ca_id="+rowData['ca_id']+"&e_id="+rowData['eid']+"&again=1'>再次发单</a><a onclick='cancle_inquiry_sheet(this)' class='delete' data-val='"+rowData['eid']+"''>取消</a> ";
				}
			}
		         ];
         //已完成数据
	var inquiry_sheet_columns_2=[ {field : 'eid',title : '询价单编号',width : '100',align : 'center'},
	        {field : 'c_id',title : '定制单编号',width : '100',align : 'center',},
			{field : 'startdate',title : '出行日期',width : '200',align : 'center',
	           	formatter : function(value,rowData, rowIndex){
		        	if(rowData['startdate']!=''&& rowData['startdate']!=null && rowData['startdate']!='0000-00-00'){
			...
			{field : 'ce_id',title : '操作',align : 'center', width : '250',
			formatter : function(value,rowData, rowIndex){
		      	return "<a target='_blank' href='<?php echo base_url()?>admin/b2/inquiry_sheet/show_supplier_plan?eid="+rowData['eid']+"&sid="+rowData['s_id']+"&c_id="+value+"' class='delete' data-val='"+value+"'>详情</a>";
				}
			}
		         ];
     	//已取消数据
         var inquiry_sheet_columns_3=[ {field : 'eid',title : '询价单编号',width : '100',align : 'center'},
    {field : 'c_id',title : '定制单编号',width : '100',align : 'center',},
	{field : 'startdate',title : '出行日期',width : '200',align : 'center',
       	formatter : function(value,	rowData, rowIndex){
	        	if(rowData['startdate']!=''&& rowData['startdate']!=null && rowData['startdate']!='0000-00-00'){
...
function show_inquiry_sheet(obj){
	var id = $(obj).attr('data-val').split('|');
	var html="";
		...
		function(data){
			...
			if(data.length==0 || data[0]['id']==null ){
				...
				$.each(data,function(index,val){
					...
					html +="<div class='supplier col_fy fl'><span>联系人姓名：</span>"+val['realname']+"</div>";
					...
					html +="<div class='operation'><a target='_blank' href='<?php echo base_url()?>admin/b2/inquiry_sheet/show_supplier_plan?eid="+val['eid']+"&sid="+val['s_id']+"&c_id="+id[1]+"'>供应商方案详情</a><button onclick='choose_this_one(this)' class='supplierON' data-val="+val['id']+"|"+val['eid']+"|"+val['s_id']+"|"+val['supplier_price']+"|"+val['childprice']+"|"+val['childnobedprice']+">选择此方案</button></div>";
