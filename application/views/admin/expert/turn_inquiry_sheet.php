...
.w_80 { width:72px !important;}
.form_input { height: 26px;line-height: 26px;padding: 5px 8px;margin:2px 0;display:inline-block;width:250px;}
...
.input_list_hidden li{ padding-left: 8px; position: relative;}
...
#destButton{ height:32px; line-height:32px; float:left; width: 700px; margin:8px; display:none; margin-bottom: 5px;}
...
.form_input{ padding: 2px 5px; margin:0px; height: 24px; line-height: 24px;}
.select_two_dest i {position: absolute; right: 5px; top: 10px; width: 20px; height: 20px; background: url(<?php echo base_url();?>static/img/ok_msu.png) no-repeat;}
...
.to_play_input input, .to_play_input{ height: 30px; line-height: 30px;}
.input_list_hidden{ top: 30px;}
.input_list_hidden li:hover{ background: #09c; color: #fff;}
.project_number_title{ background: #09c;}
.project_number_info{ border: 1px solid #09c; border-radius: 0 5px 5px 0; color: #09c;}
.form_select .search_select .show_select{ height: 20px; line-height: 20px; padding: 4px 8px;}
.form_select .search_select .select_list li.select{ background: #09c; color: #fff;}
input,select,textarea,.form_select, { margin:5px !important;}
.textWlp{ margin: 5px 0;}
.traffic_content{ margin: 0 5px;}
select.form_select{ height: 20px; line-height: 20px; padding: 4px 8px;}
.expert_offer_price div input{ width: 40px;}
.price_explain_title{ color: #09c  ;}
.travel_arrange .travelDay{ padding: 20px 0;}
...
<body class="iframe_body">
    ...
    <div class="page-body shadow" id="bodyMsg">
        ...
        <div class="order_detail">
            ...
            <form id="turn_inquiry_form" method="post" action="#">
            <!-- ===============管家需求============ -->
                ...
                <div class="project_number"><span class="project_number_title">编号:</span><span class="project_number_info"><?php echo $c_id?></span></div>
                 <table class="order_info_table table_td_border" border="1" width="100%" cellspacing="0">
                    <tr height="40">
                        ...
                        <input type="text" id="startcity" name="startcity" class="form_input" value="<?php echo $startplace_arr[0]?>" disabled placeholder="请选择出发城市"/>
                    ...
                    <tr>
                        ...
                        <td colspan="3" style=" padding: 5px;">
                        	<input type="hidden" name="endplace" value="<?php echo $grab_custom_data['endplace']?>" />
                            <!--
                            <div class="to_play_input">
                            							<div class="mudidi_input">
                                        		            	...
                                        		            	<ul class="input_list_hidden row_ree_one"></ul>
                                        				<div class="mudidi_input">
                                            <?php
                                           ...
                                           $count_endplace_two =  count($endplace_two_arr);
                                            ?>
                                        			         <input type="text" id="destTwo" name="destTwo" value="<?php echo '已选择'.$count_endplace_two.'项';?>" readonly="" class="mudidi_two" placeholder="选择省份(二级目录)" autocomplete="off">
                    ...
                    <tr height="40">
                        ...
                        <td colspan="3" style=" padding: 5px;">
                            ...
                            <label data-val="1" onclick="click_this_type(this);"><input type="radio" name="travel_data" checked="checked"/>出行日期</label>
                        	...
                        	<input type="text" data-val="2" name="estimatedate" class="form_input" placeholder="只能手写输入" style="display:none;width:682px;" value="<?php echo $grab_custom_data['estimatedate']?>"/>
                        </td>
                    </tr>
                    <tr height="40">
                    ...
                    <tr height="40">
                        ...
                        <td colspan="3"><textarea  name="service_range" class="form_textarea noresize textWlp"  placeholder="请安排多预留购物时间，行程安排不要太紧迫了..."><?php echo $grab_custom_data['service_range']?></textarea>
		</td>
                    </tr>
                    <tr height="40">
                ...
                <div class="travel_content" id="rout_line">
                         <!-- Start 详细行程开始-->
                            ...
                            <?php $i=1;foreach ($custom_trip_data_list as $item): ?>
                                 ...
                                 <?php $i++; endforeach;?>
                               <span class="btn btn_blue"  data-val="<?php echo $i;?>" id="xysTimeDay" onclick="add_travel(this)">+第<i><?php echo $i;?></i>天</span>
                                    </div>
                                <?php else:?>
                                    <!-- *************************如果没有回复方案的时候**********************-->
                                          <div class="travel_arrange">
                                    <div class="travelDay shadow">
 ...
 * @type {String}
 */
var select_supplier_id = '<?php echo $grab_custom_data['supplier_id']?>';
var select_supplier_name = '<?php echo $grab_custom_data['s_name']?>';
...
function clickDestTwo(obj ,data) {
    ...
    var name = $(obj).html();
            if($(obj).hasClass('select_two_dest')){
                $(obj).removeClass('select_two_dest');
                $(obj).find('i').remove();
                delThreeDest(data,id);
            }else{
                $(obj).addClass('select_two_dest');
                $(obj).append("<i></i>");
            }
            //$(obj).parent
	//$("#pop_city_expert_dest,#suggest_city_expert_dest").remove();//删除之前的第三级目的地选择插件
	...
	if (typeof data[id] != 'undefined') {
		...
		var data_dest_three = getChoiceDestData(data);
                         var pluginArr = {0:{name:$("input[name='destOne']").val(),two:data_dest_three}};
		createChoicePlugin({
			data:pluginArr,
			nameId:'expert_dest',
...
function getDestTwoSelected() {
    ...
    if (maxnum != 'none' && $('.row_ree_two').find('.select').length > maxnum) {
        alert('二级目的地最多选择'+maxnum+'项');
        return false;
    }
    var ids = '';
    var num = 0;
    $.each($('.row_ree_two').find('.select_two_dest') ,function() {
...
//删除选择的第三级目的地
function delThreeDest(destData,towId) {
...
function init_endplace(data){
              ...
              //默认有数据的时候
            if($("#destOne").val()!=0 && $("#destOne").val()!='' && $("#destOne").val()!=undefined){
                var choosed_id_arr = $("#destTwoId").val().split(",");
                ...
                if ($("#destOne").val() == '周边游') {
                    ...
                    $.post("/line/line_custom/getRoundTrip",{cityId:$("#startcityId").val()},function(json) {
                        var tripArr = eval("("+json+")");
                        $.each(tripArr ,function(k ,v) {
                            if($.inArray(v.id, choosed_id_arr)>-1){
                                dest_html += "<li data-val='"+v.id+"' class='select_two_dest'>"+v.name+"<i></i></li>";
                            }else{
                ...
                //点击第二层目的地获取地三层目的地
                    ...
                    $.each(data[dest_type] ,function(k ,v) {
                        if($.inArray(v.id, choosed_id_arr)>-1){
                            ...
                            //console.log($.inArray(v.id, choosed_id_arr));
                               // console.log(dest_html);
                        //dest_html += "<li data-val='"+v.id+"'>"+v.name+"</li>";
                    });
                    $(".row_ree_two").html(dest_html);
                    $(".row_ree_two").find("li").click(function(){
                   ...
                   // console.log(dest_two_type);
                          if (typeof dest_two_type != 'undefined' && dest_two_type != '') {
                                var data_dest_three = getChoiceDestData(data);
                                var pluginArr = {0:{name:$("input[name='destOne']").val(),two:data_dest_three}};
                                createChoicePlugin({
                                    data:pluginArr,
...
//获取第三级目的地数据
function getChoiceDestData(destData) {
    var data = {};
    var key = 0;
    $.each($('.row_ree_two').find('.select_two_dest') ,function() {
...
function submit_form(obj){
        ...
        var supplier_select_name = $("#supplier_select_id").val();
        if(supplier_select_name!="" && supplier_id==""){
            alert("请选择供应商");
            return false;
        }*/
        if(submit_name=='keep_data'){
          $("#submit_type").val('1');//暂存
        }else{
         $("#submit_type").val('2'); //提交
        }
        return true;
}
...
function cancle_pic(obj){
  ...
  $('#turn_inquiry_form').submit(function(){
      $("#keep_data").attr('disabled',true);
       $("#go_submit").attr('disabled',true);
      $.post(
        "<?php echo site_url('admin/b2/grab_custom_order/turn_inquiry');?>",
        $('#turn_inquiry_form').serialize(),
        function(data) {
            console.log(data);
          data = eval('('+data+')');
          if (data.status == 200) {
...
function close_xiuxiu(){
    ...
    if(select_supplier_id!="" && select_supplier_id!=undefined){
        $("#supplier_select_id").val(select_supplier_name);
        $("#supplier_id").val(select_supplier_id);
    }
});
