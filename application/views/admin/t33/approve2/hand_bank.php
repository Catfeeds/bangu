                        ...
                        <form class="search_form" method="post" action="">
                            <div class="search_form_box clear" style="padding-right:0;">
                               <div class="search_group">
                                    <label>申请部门：</label>
                                    <input type="text" id="depart_id" onfocus="showMenu(this.id);" class="search_input" data-id="" style="width:178px;">
                               ...
                               <div class="search_group">
                                ...
                                <div class="search_group" style="margin-right:150px;">
                               ...
                               <!-- 搜索按钮 -->
                                ...
                                <div class="search_group">
                                    <input type="button" id="btn_submit" name="submit" class="search_button" style="margin-left: 0px;" value="搜索"/>
                                     <?php if($action!="2"):?>
                                    <span class="a_add btn btn_green" style="margin:3px 0px 0 5px;">新增认款</span>
                                     <?php endif;?>
                                </div>
...
<script type="text/javascript">
	...
	object = {
        init:function(){ //初始化方法
            ...
            //接口数据
            var post_url="<?php echo base_url('admin/t33/sys/approve2/api_hand_bank')?>";
        	...
        	laypage({
        	    ...
        	    jump: function(ret){
		        	...
		        	//分情况
		        	if(type_value=="1")
			        	$(".tr_rows").html("<th>操作</th><th>认款金额</th><th>销售姓名</th><th>营业部门</th><th>交款方式</th><th>备注</th><th>申请时间</th><th>状态</th>");
		        	else if(type_value=="2")
		        		$(".tr_rows").html("<th>操作</th><th>认款金额</th><th>销售姓名</th><th>营业部门</th><th>交款方式</th><th>备注</th><th>申请时间</th><th>状态</th><th>审核时间</th><th>审核人</th><th>审核意见</th>");
        ...
        pageData:function(curr,page_size,data){  //生成表格数据
        	    ...
        	    for(var i = 0; i <= last; i++)
        	    {
        	        ...
        	        var class_str="";
         	        ...
         	        var input_str="<input type='checkbox' style='margin-left:10px;margin-top:4px;' class='input_check "+ class_str +"' "+disabled_str+" data-id='"+data[i].id+"' />";

         	       if(data[i].status=="1")
         	       {
         	        //str +=     "<td style='width:40px;'>"+input_str+"</td>"; 
         	       }
            	    ...
            	    do_str +="<font style='color:red;font-size:10px;'>"+is_urgent+"</font>";
         	        str +=     "<td width='100px'>"+do_str+"</td>";
   ...
   $("#btn_submit").click(function(){
	...
	$("body").on("click",".a_detail",function(){
		...
		window.top.openWin({
		  ...
		  fix: true, //不固定
		  maxmin: true,
		  content: "<?php echo base_url('admin/t33/sys/approve2/hand_bank_detail');?>"+"?id="+id+"&action="+action
		});
	});
	//打印
...
// 		  content: ""
// 		});
		var win1 = window.open("<?php echo base_url('admin/t33/sys/approve2/hand_bank_print');?>"+"?id="+id,'print','height=1090,width=765,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		win1.focus();
	});
	//添加按钮
	$("body").on("click",".a_add",function(){
		window.top.openWin({
		  ...
		  fix: true, //不固定
		  maxmin: true,
		  content: "<?php echo base_url('admin/t33/sys/approve2/hand_bank_add');?>"
		});
	});
