...
<li ><a class="abb"  href="<?php echo base_url('base/invite_service/complete_service')?>" >已完成</a> </li>
...
<table  class="common-table" style="position:relative;" border="0" cellpadding="0" cellspacing="0">
                         ...
                         <?php if($row){foreach ($row as $k=>$v){?>
                            <tr>
                                ...
                                <td >已完成服务</td>
                                <td ><?php echo  $v['service_time']?></td>
                                <td ><?php echo  $v['score']?></td>
                                <td  class="service_text" style="position: relative;width:30px;height:30px;line-height:18px;cursor: pointer;">
                                <?php echo  $str = mb_strimwidth($v['comment'], 0,20, '...', 'utf8'); ?>
                                	  <div class="info_text" id="info_text" style="width:260px;text-align:left;border:2px solid #aaa;line-height:28px;background:#fff;z-index:1;position:absolute;right:40px;top:15px;display:none;max-height:185px;overflow-y:auto;">
									   <p><?php if(!empty($v['comment'])){ echo $v['comment'];}else{echo '暂无';}  ?></p>
									   </div>			
                                </td>
                            </tr>
		<?php } }else{?>
...
$(document).on('mouseover', '.service_text', function(){
	$(this).find(".info_text").show();
})
$(document).on('mouseout', '.service_text', function(){
	$(".info_text").hide();
});
</script>
