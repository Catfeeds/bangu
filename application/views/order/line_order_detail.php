	...
	<div class="content w_1200 clear">
		...
		<div class="travel_main clear">
			<div class="travel_Position fl">
				您当前的位置是&nbsp;:&nbsp;<a href="/">首页</a>><a href="<?php echo base_url() ?>order_from/order/line_order"">会员中心</a>><a
			...
			<div class="travel_customer clear">
				...
				<div id="im_st">
					...
					<div class="travel_information clear">
						<h4>订单信息<span style="margin-left:50px;">
                         <a href="javascript:void(0);" id="downloadContract" style="font-size:12px;color:#2E29EF; width:80px; display: inline-block;">
                         			<i style=" width:15px; height:15px; float:left; background:url(../../../static/img/xiazai_slee.png)"></i>下载合同</a>
                          		</span>
                        </h4>
		...
		<div class="travel_main">
			<div class="travel_Product clear">
				...
				<div class="travel_Contact clear">
					<ul>
						<h4>
							联系人信息<a href="#form1" class="various1">[修改]</a>
						</h4>
                    ...
                    <div id="form1" style="display:none;">
          				...
          				<?php  foreach ($concact_info as $key => $value) {?>
          					<input  type="hidden" id="next1" name="id" value=" <?php echo $value['id']; ?>">
               				<p><span>联系人姓名：</span><input type="text" name="linkman" value="<?php echo $value['linkman']; ?>" onkeyup="this.value=this.value.replace(/[^\w\u4e00-\u9fa5]+/g,'')"/></p>
				...
				<div class="travel_travel_r clear">
						<h4>
							出游人信息<!-- <a href="#form2" class="various2">[修改]</a> -->
						</h4>
						<!--出境游人修改-->
						 <?php $overArr=explode(',', $product['overcity']) ; if(in_array(1,$overArr)){ ?>
							 <div id="form2" style="display:none;">
	          					<form class="form-horizontal" role="form" method="post" id="lineinformationForm"  onsubmit="return CheckInformation(this);" action="<?php echo base_url();?>order_from/order/save_information">
	               					<input type="hidden" name="tyle" value="updata">
	               					<?php  if(!empty($travel_info)){ foreach ($travel_info as $k=>$v){?>
	               					<input type="hidden"  name="id[<?php echo $k; ?>]" value="<?php echo $v['id']; ?>">
	               					<p>第<?php echo $k+1;?>位游客</p>
	               					<p><span>中文姓名：</span><input type="text" id="name_info<?php echo $k; ?>" name="name[<?php echo $k; ?>]" value="<?php echo $v['name']; ?>"  onkeyup="this.value=this.value.replace(/[^\w\u4e00-\u9fa5]+/g,'')"/></p>
	               					<p><span>英文姓名：</span><input type="text" id="enname_info<?php echo $k; ?>" name="enname[<?php echo $k; ?>]" value="<?php echo $v['enname']; ?>" /></p>
	                    			<p><span>性别：</span>
	                    				<select name="sex[<?php echo $k; ?>]" style="margin-right:100px;">
	            						<option value="1" <?php if($v['sex']=='1'){ echo 'selected="selected"';} ?>>男</option>
	            						<option value="0" <?php if($v['sex']=='0'){ echo 'selected="selected"';} ?>>女</option>
	            						</select>
	                    			</p>
	                    			<p><span>证件类型：</span>
	                    			<select name="certificate_type[<?php echo $k; ?>]" style="margin-right:100px;" id="certificate_type<?php echo $k; ?>">
	                    			<?php foreach ($tyle as $key=>$val){ ?>
	            					<option value="<?php echo $val['dict_id']; ?>" <?php if($v['certificate_type']==$val['description']){ echo 'selected="selected"';} ?>><?php echo $val['description'];?></option>
	            				    <?php }?>
	            					</select></p> 				
	                    			<p><span>证件号码：</span><input type="text" id="certificate_no<?php echo $k; ?>" name="certificate_no[<?php echo $k; ?>]" value="<?php echo $v['certificate_no']; ?>"/></p>
	                    			<p><span>出生日期：</span><input class="Wdate input_cx_data" type="text" id="birthday<?php echo $k; ?>" name="birthday[<?php echo $k; ?>]" onClick="WdatePicker()" id="sj" value="<?php echo $v['birthday']; ?>"/></p>
	                    			<p><span>签发地：</span><input  type="text" name="sign_place[<?php echo $k; ?>]"  value="<?php if(!empty($v['sign_place'])){ echo  $v['sign_place'];}?>"/></p>
	                    		    <p><span>签发日期：</span><input class="Wdate input_cx_data" type="text" id="sign_time<?php echo $k; ?>" name="sign_time[<?php echo $k; ?>]" onClick="WdatePicker()" id="sj" value="<?php echo $v['sign_time']; ?>"/></p>
	                    			<p><span>有效期：</span><input class="Wdate input_cx_data" type="text" name="endtime[<?php echo $k; ?>]" onClick="WdatePicker()" id="endtime<?php echo $k; ?>" value="<?php echo $v['endtime']; ?>"/></p>		
	                    			<p><span>手机：</span><input type="text" id="telephone<?php echo $k; ?>" name="telephone[<?php echo $k; ?>]" value="<?php echo $v['telephone']; ?>" /></p>
	                    			 <?php }}else{ echo "怎无游客信息";}?>
	                    			 	<input type="hidden" name="num" value="<?php if(!empty($travel_info)){ echo count($travel_info);}else{ echo 0;} ?>">
	                    			 	<input type="hidden" name="userdatetime" id="userdatetime" value="<?php if(!empty($order_info ['usedate'])){echo $order_info ['usedate']; }?>" >
	                    			<p><span></span><input type="submit" name="submit" value="修改" id="update"/></p>
	                			</form>
	          				</div>
						 <?php }else{?>
							<!--国内出游人修改-->
	                        <div id="form2" style="display:none;">
	          					<form class="form-horizontal" role="form" method="post" id="lineinformationForm"  onsubmit="return CheckInformation(this);" action="<?php echo base_url();?>order_from/order/save_information">
	               					...
	               					<?php  if(!empty($travel_info)){ foreach ($travel_info as $k=>$v){?>
	               					 <input type="hidden"  name="id[<?php echo $k; ?>]" value="<?php echo $v['id']; ?>">
	               					<p>第<?php echo $k+1;?>位游客</p>
	               					<p><span>游客姓名：</span><input type="text" id="name_info<?php echo $k; ?>" name="name[<?php echo $k; ?>]" value="<?php echo $v['name']; ?>"  onkeyup="this.value=this.value.replace(/[^\w\u4e00-\u9fa5]+/g,'')"/></p>
	                    			<p><span>性别：</span>
	                    				<select name="sex[<?php echo $k; ?>]" style="margin-right:100px;">
	            						<option value="1" <?php if($v['sex']=='1'){ echo 'selected="selected"';} ?>>男</option>
	            						<option value="0" <?php if($v['sex']=='0'){ echo 'selected="selected"';} ?>>女</option>
			...
			<div class="travel_travel clear">
				...
				<?php foreach ($travel_info as $key => $value) { ?>
					<ul>
						...
						<li class="travel_trave_100"><?php if(!empty($value['sign_place'])){ echo $value['sign_place'];} ?></li>
                        ...
                        <li class="travel_trave_100" ><a href="javascript:void(0)" onclick="update_travel(<?php echo $value['id']; ?>,<?php if(!empty($product['lineid'])){ echo $product['lineid'];} ?>)">修改</a></li> 
                    </ul>
					<?php } ?>
                </div>
				</div>
               ...
               <?php }?> 
				...
				<div class="travel_travel_r clear">
					<ul>
						<h4>
							发票信息<a href="#form3" class="various3">[修改]</a>
						</h4>
					
						<!--发票信息修改-->
                        <div id="form3" style="display:none;">
          					 <form class="form-horizontal" role="form" method="post" id="invoiceForm"  onsubmit="return CheckInvoice(this);" action="<?php echo base_url();?>order_from/order/save_information">
               			     <input type="hidden" name="tyle" value="updata">
               			     <input type="hidden" name="order_id" value="<?php echo $order_id; ?>">
       					        <input type="hidden" name="id" value="<?php if(!empty($invoice_info[0]['id'])){ echo $invoice_info[0]['id'];} ?>">
               					<p><span>发票抬头：</span><input type="text" name="invoice_name" id="invoice_name" value="<?php if(!empty($invoice_info[0]['invoice_name'])){echo $invoice_info[0]['invoice_name'];} ?> "/></p>
                    			<p><span>配送地址：</span><input type="text" name="address" value="<?php if(!empty( $invoice_info[0]['address'])){ echo $invoice_info[0]['address']; }?>" /></p>
                    			<p><span>收件人：</span><input type="text" name="receiver" id="receiver" value="<?php if(!empty( $invoice_info[0]['receiver'])){ echo $invoice_info[0]['receiver']; }?>" onkeyup="this.value=this.value.replace(/[^\w\u4e00-\u9fa5]+/g,'')"/></p>
                    			<p><span>手机：</span><input type="text" name="telephone" value="<?php if(!empty( $invoice_info[0]['telephone'])){ echo $invoice_info[0]['telephone']; }?>" /></p>
                    			<p><span>金额：</span><?php if(!empty( $order_info)){ echo $order_info['total_price'];} ?></p>
                    			<p><span></span><input type="submit" name="submit" value="修改" id="update"/></p>
                			</form>
          				</div>
					</ul>
		...
		<div class="travel_main">
			<div class="travel_invoice clear">
				...
				<div class="travel_invo_careful">
					...
					<p>2.出游前填写的发票将于出游归来后5日内开具,请注意查收.</p>
...
function CheckInformation(obj){
    ...
    var userdatetime=$('#userdatetime').val();
    var endtime=$("#endtime<?php echo $k; ?>").val();
    var birthday=$("#birthday<?php echo $k; ?>").val();

    var date =  new Date(endtime);
    var time_str = date.getTime().toString();

    var date1 =  new Date(userdatetime);
    var time = date1.getTime().toString();

 	var date2 =  new Date(birthday);
    var birthtime = date2.getTime().toString();
    var nowtime = new Date();

    if(time_str.substr(0, 10)<time.substr(0, 10)){
          alert('证件日期不能小于出游时间');
          return false;
     }
      if(nowtime.getTime()<birthtime){
    	alert('生日日期不能等于大于现在日期');
        return false;
    }
    //去掉空格
...
/* 	   if(telephone==''){
   ...
   if(certificate_type==37){
	...
	jQuery.ajax({ type : "POST",data : jQuery('#lineinformationForm').serialize(),url : "<?php echo base_url()?>order_from/order/update_traver",
		success : function(response) {
		 	 if(response){
				alert( '保存成功！' );
                location.reload();
...
//修改出游人信息
function update_travel(id,lineid){
        ...
        layer.open({
              ...
              fix: false, //不固定
              maxmin: true,
              content: "<?php echo base_url('order_from/order/get_member_travel');?>"+"?id="+id+"&lineid="+lineid
        });
}
