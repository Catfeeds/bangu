...
jQuery.priceDate.prototype = {
	...
	initDate : function(tab_body,currentMonth,index){
		...
		var table = jQuery('<table border="0" cellspacing="0" width="1000" cellpadding="0" class="priceDate" id="'+currentMonth+'" style="display:none;"></table> ').appendTo( tab_body );
...
//		content = content + '>' + ( this.dayFormatter ? this.dayFormatter(tablSettings,this.comparable(dateStr)) : "" ) + '</td>';
		...
		if(tab_data){
			...
			if(rowData){
			//	content = content +  ' style="background:#FFDF64"; ';
				content = content + ( disabled ?   ' class="disable" '  :  ' style="background:#E5F2FD"; ' );
			}
		}
