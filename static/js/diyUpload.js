...
(function( $ ) {
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			var div = '<div class="diyButton"> \
						<a class="diyStart" href="javascript:void(0)" onclick=mun(this)>开始上传</a> \
						<a class="diyCancelAll" href="javascript:void(0)">全部取消</a> \
					</div>';
		...
		//父容器宽度;
		var $width = $('.fileBoxUl>li').length * 400;
		...
		//根据图片的数量 发生事件
		//fileNumLimit 
		//var onMUc = $.extend({}, defaluts, objId);
		//$startButton.one('click',function(Mun){
			//var Mun=$parentFileBox.children('.fileBoxUl').find('li').length;
			// console.log( Mun );
			//if(Mun<fileNumLimit){
			//	alert('图片上传超过数量')	
			//}
