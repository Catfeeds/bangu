...
(function( $ ) {
    ...
    $.fn.extend({
        diyUpload:function( opt, serverCallBack ) {
			...
			if ( !WebUploader.Uploader.support() ) {
				...
				return false;
       			...
       			webUploader.on('filesQueued', function( files ) {
       				if($fileInput.parent().nextAll('.reply_deposit2').children('.show_img').length>0){
					if($fileInput.parent().nextAll('.reply_deposit2').children('.show_img').children('li').length+files.length>6){
						alert('上传文件数量超过限制');
						$fileInput.nextAll('.parentFileBox').children('.fileBoxUl').children().remove();
						webUploader.reset();
					}
				}else{
					if($fileInput.parent().nextAll('.parentFileBox').children('.fileBoxUl').children('li').length+files.length>6){
						alert('上传文件数量超过限制asd');
						$fileInput.nextAll('.parentFileBox').children('.fileBoxUl').children().remove();
						webUploader.reset();
					}
			...
			//绑定文件加入队列事件;限制总文件个数只能是六个****************修改过*************
			webUploader.on('fileQueued', function( file ) {
			/*	if($fileInput.nextAll('.parentFileBox').children('.fileBoxUl').children('li').length<6 && ($fileInput.nextAll('.reply_deposit2').children('.show_img').length>0 && $fileInput.nextAll('.reply_deposit2').children('.show_img').children('li').length<6)){
					createBox( $fileInput, file ,webUploader);
				}else{
					alert('上传文件数量超过限制');
				}*/
				if($fileInput.parent().nextAll('.reply_deposit2').children('.show_img').length>0){
					if($fileInput.parent().nextAll('.reply_deposit2').children('.show_img').children('li').length+$fileInput.nextAll('.parentFileBox').children('.fileBoxUl').children('li').length+1>6){
						alert('上传文件数量超过限制');
						$fileInput.nextAll('.parentFileBox').children('.fileBoxUl').children().remove();
						webUploader.reset();
					}else{
					...
					if($fileInput.parent().nextAll('.parentFileBox').children('.fileBoxUl').children('li').length+1>6){
						alert('上传文件数量超过限制');
						$fileInput.nextAll('.parentFileBox').children('.fileBoxUl').children().remove();
						webUploader.reset();
					}else{
			...
			webUploader.on('uploadFinished', function(){
				$fileInput.nextAll('.parentFileBox').children('.diyButton').remove();
				if($fileInput.parent().nextAll('.reply_deposit2').children('.show_img').length>0){
					//只有在转询价单的时候会有这个修改的需求,所以判断显示图片的方框是否存在
					$fileInput.nextAll('.parentFileBox').children('.fileBoxUl').children().remove();
					webUploader.reset();
				}
			});
			...
			webUploader.on('uploadSuccess',function( file, response ){
				...
				if ( successCallBack ) {
					...
					var line_photo_url='<li style="float:left;list-style:none;margin:0 20px 0px 0px" class="url_li">';
					 line_photo_url+='<div class="img_main"><div id="del_img" class="float_div" onclick="del_imgdata(this,-1);"  style="height:20px;width:12px; font-size:24px;cursor:pointer;">×</div><img received="" file="" src="'+response.url+'"  style="width:180px; height:160px;">';
					...
					$fileInput.prev('.div_url_val').children('.line_pic_val').val($fileInput.prev('.div_url_val').children('.line_pic_val').val()+response.url+';');
	
					//.attr('src',$fileInput.nextAll('.div_url_val').children('.url_val').val()+response.url);
/*					if($fileInput.parent().nextAll('.reply_deposit2').children('.show_img').length>0){
						//只有在转询价单的时候会有这个修改的需求,所以判断是否存在
						if($fileInput.parent().nextAll('.reply_deposit2').children('.show_img').children('li').length<6){

							var html = "<li ><img src="+response.url+"><span onclick='cancle_pic(this)'><img src='/assets/img/x_alt.png'></span></li>";
							$fileInput.parent().nextAll('.reply_deposit2').children('.show_img').append(html);
						}else{
							alert('上传文件数量超过限制');
						}
					}*/
	...
	function showDiyProgress( progress, $diyBar, text ) {
		var div = '<div class="diyButton"> \
						<a class="diyStart" style="right: 615px; top: -13px;" href="javascript:void(0)">开始上传</a> \
						<a class="diyCancelAll" style="right: 284px; top: 24px;display: none;"  href="javascript:void(0)">全部取消</a> \
					</div>';
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		//父容器宽度;
		var $width = $('.fileBoxUl>li').length * 800;
